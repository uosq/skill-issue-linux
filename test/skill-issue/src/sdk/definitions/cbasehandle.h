...
template <class T> CHandle<T> CHandle<T>::FromIndex(int index)
{
	CHandle<T> ret;
	ret.m_Index = index;
	return ret;
...
template <class T> T *CHandle<T>::Get() const
{
...
template <class T> bool CHandle<T>::operator==(T *val) const
{
	return Get() == val;
}
...
template <class T> const CBaseHandle &CHandle<T>::operator=(const T *val)
{
	Set(val);
	return *this;
}
