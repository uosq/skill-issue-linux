...
// yes, I pasted this from amalgam
// i am not going to make all this stuff from scratch
...
class Vec2
{
	...
	{
		return fabsf(x) < flEpsilon && fabsf(y) < flEpsilon;
	}
};
using Vector2D = Vec2;
...
class Vec3
{
	...
	inline bool IsZero(float flEpsilon = 0.001f) const
	{
		return fabsf(x) < flEpsilon && fabsf(y) < flEpsilon && fabsf(z) < flEpsilon;
	}

	inline Vec3 ToAngle() const noexcept
	{
		return {RAD2DEG(atan2(-z, hypot(x, y))), RAD2DEG(atan2(y, x)), 0.f};
	}
	inline Vec3 FromAngle() const noexcept
	{
		return {static_cast<float>(cos(DEG2RAD(x)) * cos(DEG2RAD(y))),
			static_cast<float>(cos(DEG2RAD(x)) * sin(DEG2RAD(y))), static_cast<float>(-sin(DEG2RAD(x)))};
	}
};
using Vector	  = Vec3;
using QAngle	  = Vec3;
...
struct Color_t
{
	...
	inline Color_t HueShift(float flShift)
	{
		...
		GetHSV(flH, flS, flV);
		Color_t tOut;
		tOut.SetHSV(fmodf(flH + flShift, 360.f), flS, flV, a);
		return tOut;
	}
	...
	inline Color_t Lerp(Color_t to, float t, LerpEnum::LerpEnum eLerp = LerpEnum::All) const
	{
		...
		{
		default:
			return {byte(r + (to.r - r) * t), byte(g + (to.g - g) * t), byte(b + (to.b - b) * t),
				byte(a + (to.a - a) * t)};
		case LerpEnum::NoAlpha:
			return {byte(r + (to.r - r) * t), byte(g + (to.g - g) * t), byte(b + (to.b - b) * t), byte(a)};
		case LerpEnum::Alpha:
			return {r, g, b, byte(a + (to.a - a) * t)};
		}
	}
	...
	inline bool IsColorBright(float flValue = 175, float flRed = 0.299f, float flGreen = 0.587f,
				  float flBlue = 0.114f) const
	{
		return Brightness(flRed, flGreen, flBlue) > flValue;
	}

	inline bool IsColorDark(float flValue = 80, float flRed = 0.299f, float flGreen = 0.587f,
				float flBlue = 0.114f) const
	{
		return Brightness(flRed, flGreen, flBlue) < flValue;
	}
...
struct Chams_t
{
	...
	inline bool operator()(bool bVisibleOnly = false) const
	{
		return bVisibleOnly
			   ? !Visible.empty()
			   : Visible != std::vector<std::pair<std::string, Color_t>>{{"Original", Color_t()}} ||
				 !Occluded.empty();
	}
};
