...
class CHudTexture
{
	...
	void DrawSelfCropped(int x, int y, int cropx, int cropy, int cropw, int croph, int finalWidth, int finalHeight,
			     Color clr) const;
...
class CHud
{
	...
	CHudElement *FindElement(const char *pName)
	{
		...
		{
			if (strcmp(m_HudList[i]->GetName(), pName) == 0)
				return m_HudList[i];
		}
	...
	void DrawIconProgressBar(int x, int y, CHudTexture *icon, CHudTexture *icon2, float percentage, Color &clr,
				 int type);
