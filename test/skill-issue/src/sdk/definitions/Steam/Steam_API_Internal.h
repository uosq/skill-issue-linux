...
#define STEAM_DEFINE_USER_INTERFACE_ACCESSOR(type, name, version)                                                      \
	STEAM_DEFINE_INTERFACE_ACCESSOR(                                                                               \
	    type, name, SteamInternal_FindOrCreateUserInterface(SteamAPI_GetHSteamUser(), version), "user", version)
#define STEAM_DEFINE_GAMESERVER_INTERFACE_ACCESSOR(type, name, version)                                                \
	STEAM_DEFINE_INTERFACE_ACCESSOR(                                                                               \
	    type, name, SteamInternal_FindOrCreateGameServerInterface(SteamGameServer_GetHSteamUser(), version),       \
	    "gameserver", version)
	...
	{                                                                                                              \
		...
		{                                                                                                      \
			...
			thisclass *pOuter = reinterpret_cast<thisclass *>(                                             \
			    reinterpret_cast<char *>(this) - offsetof(thisclass, m_steamcallback_##func));             \
...
template <class T, class P> inline void CCallResult<T, P>::Set(SteamAPICall_t hAPICall, T *p, func_t func)
{
	if (m_hAPICall)
...
inline CCallback<T, P, bGameserver>::CCallback(T *pObj, func_t func) : m_pObj(nullptr), m_Func(nullptr)
{
	if (bGameserver)
	{
...
template <class T, class P, bool bGameserver> inline void CCallback<T, P, bGameserver>::Register(T *pObj, func_t func)
{
	if (!pObj || !func)
		return;
...
template <class T, class P, bool bGameserver> inline void CCallback<T, P, bGameserver>::Unregister()
{
	// SteamAPI_UnregisterCallback removes k_ECallbackFlagsRegistered
	SteamAPI_UnregisterCallback(this);
}

template <class T, class P, bool bGameserver> inline void CCallback<T, P, bGameserver>::Run(void *pvParam)
{
...
#define STEAM_OUT_ARRAY_CALL(COUNTER, FUNCTION, PARAMS)                                                                \
	STEAM_CLANG_ATTR("out_array_call:" #COUNTER "," #FUNCTION "," #PARAMS ";")
...
class CSteamGameServerAPIContext
{
	...
	inline bool
	Init(); // NOTE: This is defined in Steam_Gameserver.h, to avoid this file having to include everything
