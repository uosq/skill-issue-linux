...
// not on Windows so those don't do anything
...
void bf_write::WriteBitCoordMP(const float f, bool bIntegral, bool bLowPrecision)
{
	...
	else
	{
		...
		if (intval)
		{
			...
			--intval;
			bits = intval * 8 + signbit * 4 + 2 + bInBounds;
			bits += bInBounds ? (fractval << (3 + COORD_INTEGER_BITS_MP))
					  : (fractval << (3 + COORD_INTEGER_BITS));
			numbits = 3 + (bInBounds ? COORD_INTEGER_BITS_MP : COORD_INTEGER_BITS) +
...
void bf_write::WriteLong(uint32_t val)
{
	WriteSBitLong(val, sizeof(uint32_t) << 3);
}
...
int bf_read::CompareBitsAt(int offset, bf_read *RESTRICT other, int otherOffset, int numbits) RESTRICT
{
	...
	uint32_t *pData1	= (uint32_t *)m_pData + (offset >> 5);
	uint32_t *pData2	= (uint32_t *)other->m_pData + (otherOffset >> 5);
	uint32_t *pData1End	= pData1 + ((offset + numbits - 1) >> 5);
	uint32_t *pData2End	= pData2 + ((otherOffset + numbits - 1) >> 5);

	while (numbits > 32)
	{
		x = LoadLittleDWord((uint32_t *)pData1, 0) >> iStartBit1;
		x ^= LoadLittleDWord((uint32_t *)pData1, 1) << (32 - iStartBit1);
		x ^= LoadLittleDWord((uint32_t *)pData2, 0) >> iStartBit2;
		x ^= LoadLittleDWord((uint32_t *)pData2, 1) << (32 - iStartBit2);
		if (x != 0)
		{
			return x;
		}
	...
	x = LoadLittleDWord((uint32_t *)pData1, 0) >> iStartBit1;
	x ^= LoadLittleDWord((uint32_t *)pData1End, 0) << (32 - iStartBit1);
	x ^= LoadLittleDWord((uint32_t *)pData2, 0) >> iStartBit2;
	x ^= LoadLittleDWord((uint32_t *)pData2End, 0) << (32 - iStartBit2);
