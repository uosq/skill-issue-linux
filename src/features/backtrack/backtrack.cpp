...
bool LagCompRecord::IsValid ()
{
	...
	float delta
	    = correct - (interfaces::GlobalVars->curtime - m_flSimTime);
...
float Backtrack::GetLatency ()
{
	...
	if (auto netchan = interfaces::Engine->GetNetChannelInfo ())
		flLatency += netchan->GetLatency (FLOW_INCOMING)
			     + netchan->GetLatency (FLOW_OUTGOING);
...
bool ShouldPrioritizeHead (CTFWeaponBase *pWeapon)
{
	...
	if (pWeapon->IsSniperRifle ()
	    && static_cast<CTFSniperRifle *> (pWeapon)->m_flChargedDamage ()
		   > 50.0f)
		return true;
...
void Backtrack::Run (CTFPlayer *pLocal, CTFWeaponBase *pWeapon, CUserCmd *pCmd)
{
	...
	BacktrackMode mode
	    = static_cast<BacktrackMode> (Settings::Misc.backtrack);
	if (mode == BacktrackMode::NONE || mode >= BacktrackMode::MAX
	    || mode <= BacktrackMode::INVALID)
		return;
	...
	{
		CTFPlayer *entity = static_cast<CTFPlayer *> (
		    interfaces::EntityList->GetClientEntity (index));
		if (entity == nullptr)
			continue;
		...
		CBaseAnimating *pAnimating
		    = reinterpret_cast<CBaseAnimating *> (entity);
		...
		for (auto &record : records)
		{
			...
			Vector aimPos;
			if (!pAnimating->GetHitboxCenter (
				record.m_Bones,
				bPriotizeHead ? HITBOX_HEAD : HITBOX_PELVIS,
				aimPos))
				continue;
	...
	pCmd->tick_count
	    = TIME_TO_TICKS (pBestRecord->m_flSimTime + GetInterp ());
}

void Backtrack::Reset () { m_records.clear (); }

float Backtrack::GetInterp ()
{
	static ConVar *cl_interp = interfaces::Cvar->FindVar ("cl_interp");
	static ConVar *cl_updaterate
	    = interfaces::Cvar->FindVar ("cl_updaterate");
	static ConVar *cl_interp_ratio
	    = interfaces::Cvar->FindVar ("cl_interp_ratio");

	float lerp = std::max (cl_interp->GetFloat (),
			       cl_interp_ratio->GetFloat ()
...
void Backtrack::CleanRecords ()
{
	...
	{
		CTFPlayer *entity = static_cast<CTFPlayer *> (
		    interfaces::EntityList->GetClientEntity (it->first));
...
void Backtrack::Store (CTFPlayer *pLocal, const EntityListEntry &entry)
{
	...
	if (!records.empty ()
	    && records.front ().m_flSimTime == pEntity->m_flSimulationTime ())
		return;

	matrix3x4 bones[MAXSTUDIOBONES];
	if (!pEntity->SetupBones (bones, MAXSTUDIOBONES, BONE_USED_BY_ANYTHING,
				  pEntity->m_flSimulationTime ()))
		return;

	records.emplace_front (
	    bones, pEntity->m_flSimulationTime (), pEntity->GetCenter (),
	    pEntity->m_angEyeAngles (), pEntity->EstimateAbsVelocity ());
}

void Backtrack::Init ()
{
	m_mat = MaterialManager::CreateMaterial (
	    "Hello", "UnlitGeneric\n"
		     "{\n"
		     "	$basetexture \"white\"\n"
		     "}");
...
bool Backtrack::GetRecords (CTFPlayer *pEntity,
			    std::vector<LagCompRecord> &out)
{
	auto records = m_records[pEntity->GetIndex ()];
	...
	BacktrackMode mode
	    = static_cast<BacktrackMode> (Settings::Misc.backtrack);
	if (mode >= BacktrackMode::MAX || mode <= BacktrackMode::INVALID
	    || mode == BacktrackMode::NONE)
	{
		LagCompRecord &front = records.front ();
		out.emplace_back (front.m_Bones, front.m_flSimTime,
				  front.m_vecAbsCenter, front.m_vecViewAngles,
				  front.m_vecVelocity);
		return true;
	}
