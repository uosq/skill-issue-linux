...
class CPrediction
{
	...
	int TryPlayerMove (Vector *vecFirstDest	   = nullptr,
			   CGameTrace *tFirstTrace = nullptr,
	...
	void TryTouchGround (const Vector &start, const Vector &end,
			     const Vector &mins, const Vector &maxs,
			     unsigned int fMask, ITraceFilter &filter,
			     CGameTrace &pm);
	void StayOnGround ();
	int ClipVelocity (Vector &in, Vector &normal, Vector &out,
			  float overbounce, float flRedirectCoeff);
	void TracePlayerBBox (const Vector &start, const Vector &end,
			      unsigned int fMask, ITraceFilter &filter,
			      CGameTrace &trace);
