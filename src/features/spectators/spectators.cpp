...
bool Spectators::IsSpectated (CTFPlayer *pTarget,
			      std::vector<CTFPlayer *> &out)
{
	for (const auto &entry : EntityList::GetEntities ())
	{
		...
		CBaseEntity *pSpectatedTarget
		    = HandleAs<CBaseEntity *> (pPlayer->m_hObserverTarget ());
...
bool Spectators::IsLocalPlayerSpectated () { return s_bSpectated; }
...
void Spectators::OnLevelShutdown () { Reset (); }