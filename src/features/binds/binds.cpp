...
bool Binds::IsActive (const Hotkey *hk) const { return hk->m_bState; }
...
bool Binds::RenderHotkey (const char *label, Hotkey *hk)
{
	...
	const char *keyName
	    = hk->m_bCapturing ? "Press key..." : GetKeyName (hk);
	...
	if (ImGui::BeginPopupContextItem ("HotkeyModeMenu"))
	{
		constexpr const char *modes[]
		    = { "Off", "Hold", "Toggle", "Hold Off", "Always" };
	...
	for (const auto &bind : m_hotkeys)
	{
		...
		if (bind->m_iMode != HotkeyMode::Off)
		{
			ImVec4 color = ImGui::ColorConvertU32ToFloat4 (
			    bind->m_bState ? IM_COL32 (100, 255, 100, 255)
					   : IM_COL32 (255, 255, 255, 255));

			// F4 - Aimbot Key - Toggle
			ImGui::TextColored (color, "%s - %s [%s]", strKeyName,
					    strName.c_str (), strModeName);
		} else
		{
			ImGui::TextDisabled ("%s - %s [%s]", strKeyName,
					     strName.c_str (), strModeName);
		}
	}
...
{
	return hk->m_iType == InputType::VirtualKey
	       && hk->m_iMode != HotkeyMode::Off;
}