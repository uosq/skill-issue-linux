...
inline void Radar::Run()
{
	int size = g_Settings.radar.size;

	m_iRange = g_Settings.radar.range;
...
inline Vec2 Radar::WorldToRadar(const Vector& localPos, const Vector& enemyPos, float viewAnglesYaw)
{
...
inline float Radar::GetRadius()
{
	return m_flRadius;
}

inline int Radar::GetRange()
{
	return m_iRange;
}

inline void Radar::DrawContents()
{
	int size = g_Settings.radar.size;
	...
	int iconSize = g_Settings.radar.icon_size;
	...
	for (const auto& entry : EntityList::GetEntities())
	{
		...
		if (entry.flags & EntityFlags::IsPlayer && (!g_Settings.radar.players || !static_cast<CTFPlayer*>(entry.ptr)->IsAlive()))
			continue;

		if (entry.flags & EntityFlags::IsProjectile && !g_Settings.radar.projectiles)
			continue;
