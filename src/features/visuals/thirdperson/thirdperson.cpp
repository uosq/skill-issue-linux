...
void Thirdperson::OverrideView (CTFPlayer *pLocal, CViewSetup *pView)
{
	...
	if (interfaces::CInput->CAM_IsThirdPerson ())
	{
		...
		Math::AngleVectors (vecViewAngles, &vecForward, &vecRight,
				    &vecUp);

		float flScale
		    = bActive ? Settings::Misc.thirdperson_offset[3] : 1.0f;
		...
		Vector vecEnd = pLocal->GetEyePos ()
				+ (vecForward * vecCamOffset.x * flScale)
				+ (vecRight * vecCamOffset.y * flScale)
				+ (vecUp * vecCamOffset.z * flScale);

		Vector vecHullMin, vecHullMax;
		vecHullMin.Set (-CAM_HULL_OFFSET, -CAM_HULL_OFFSET,
				-CAM_HULL_OFFSET);
		vecHullMax.Set (CAM_HULL_OFFSET, CAM_HULL_OFFSET,
				CAM_HULL_OFFSET);

		helper::engine::TraceHull (pView->origin, vecEnd, vecHullMin,
					   vecHullMax, MASK_SHOT_HULL, &filter,
					   &trace);
...
{
	return pLocal->IsTaunting () || pLocal->IsGhost ()
	       || pLocal->InCond (TF_COND_HALLOWEEN_KART)
	       || (pLocal->InCond (TF_COND_STUNNED)
		   && (pLocal->m_iStunFlags ()
		       & (TF_STUN_CONTROLS | TF_STUN_LOSER_STATE)));
}

bool Thirdperson::IsThirdPerson (CTFPlayer *pLocal)
{
	return Settings::Misc.thirdperson_key->IsActive ()
	       || ShouldIgnoreBind (pLocal);
}

Vector Thirdperson::GetCameraOffset ()
{
	if (Settings::Misc.thirdperson_key->IsActive ())
		return { Settings::Misc.thirdperson_offset[0],
			 Settings::Misc.thirdperson_offset[1],
