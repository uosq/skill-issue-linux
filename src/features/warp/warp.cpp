...
namespace Warp
{
	...
	int GetMaxTicks ()
	{
		static ConVar *sv_maxusrcmdprocessticks
		    = interfaces::Cvar->FindVar ("sv_maxusrcmdprocessticks");
			    ...
			    CUserCmd *pCmd)
	{
		...
		if (interfaces::EngineVGui->IsGameUIVisible ()
		    || interfaces::EngineVGui->IsConsoleVisible ())
			return;
		...
		if (Settings::AntiAim.warp_recharge_key->IsActive ()
		    && Warp::m_iStoredTicks < GetMaxTicks ())
		{
	...
	void DrawContents ()
	{
		...
		float ratio
		    = static_cast<float> (m_iStoredTicks) / GetMaxTicks ();
		...
		if (m_iStoredTicks > 0)
			draw->AddRectFilled (
			    min, max, IM_COL32 (0, 119, 200, 255), 2.0f);

		char chrText[12];
		sprintf (chrText, "%i/%i", Warp::m_iStoredTicks,
			 GetMaxTicks ());
		ImVec2 textSize = ImGui::CalcTextSize (chrText);

		draw->AddText (
		    ImVec2 (pos.x + width / 2.0f - textSize.x / 2.0f,
			    pos.y + height / 2.0f - textSize.y / 2.0f),
