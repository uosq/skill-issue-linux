...
float GetFloat (IGameEvent *event, const std::string &keyName,
		float defaultValue)
{
	if (!event)
		return 0;
...
std::string GetString (IGameEvent *event, const std::string &keyName,
		       const std::string &defaultValue)
{
	return event
		   ? event->GetString (keyName.c_str (), defaultValue.c_str ())
		   : "Invalid Event";
}
...
void SetString (IGameEvent *event, const std::string &keyName,
...
void GameEvent_RegisterClass (asIScriptEngine *engine)
{
	...
	engine->RegisterObjectBehaviour (
	    GAMEEVENT_CLASSNAME, asBEHAVE_ADDREF, "void f()",
	    asFUNCTION (GameEvent_AddRef), asCALL_CDECL_OBJLAST);
	engine->RegisterObjectBehaviour (
	    GAMEEVENT_CLASSNAME, asBEHAVE_RELEASE, "void f()",
	    asFUNCTION (GameEvent_Release), asCALL_CDECL_OBJLAST);

	engine->RegisterObjectMethod (GAMEEVENT_CLASSNAME, "string GetName()",
				      asFUNCTION (GetName),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (GAMEEVENT_CLASSNAME, "bool IsLocal()",
				      asFUNCTION (IsLocal),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    GAMEEVENT_CLASSNAME, "bool IsEmpty( const string &in keyName)",
	    asFUNCTION (IsEmpty), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    GAMEEVENT_CLASSNAME,
	    "bool GetBool(const string &in keyName, bool defaultValue)",
	    asFUNCTION (GetBool), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    GAMEEVENT_CLASSNAME,
	    "int GetInt(const string &in keyName, int defaultValue)",
	...
	engine->RegisterObjectMethod (
	    GAMEEVENT_CLASSNAME,
	    "void SetBool(const string &in keyName, bool value)",
	    asFUNCTION (SetBool), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    GAMEEVENT_CLASSNAME,
	    "void SetInt(const string &in keyName, int value)",
	    asFUNCTION (SetInt), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    GAMEEVENT_CLASSNAME,
	    "void SetFloat(const string &in keyName, float value)",
