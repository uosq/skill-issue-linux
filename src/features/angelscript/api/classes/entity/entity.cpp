...
int GetHealth (CBaseEntity *ent)
{
	...
	{
		CTFRobotDestruction_Robot *pRobot
		    = static_cast<CTFRobotDestruction_Robot *> (ent);
...
int GetMaxHealth (CBaseEntity *ent)
{
	...
	if (ent->IsPlayer ())
	{
		...
		return pResources ? pResources->m_iMaxHealth (ent->GetIndex ())
				  : 0;
	} else if (ent->IsBuilding ())
	{
		CBaseObject *pObj = static_cast<CBaseObject *> (ent);
...
template <typename T>
T *GetProp (CBaseEntity *ent, const std::string &className,
...
template <typename T>
void SetProp (CBaseEntity *ent, const std::string &className,
	      const std::string &prop, T value)
{
	...
	if (it == netvars.end ())
	{
		...
		engine->WriteMessage ("Entity::SetProp", 0, 0, asMSGTYPE_ERROR,
				      "Couldn't find netvar");
		return;
	}
...
int GetPropInt (CBaseEntity *ent, const std::string &cls,
		const std::string &prop)
{
	if (ent == nullptr)
		return 0;
...
float GetPropFloat (CBaseEntity *ent, const std::string &cls,
		    const std::string &prop)
{
	if (ent == nullptr)
		return 0;
...
bool GetPropBool (CBaseEntity *ent, const std::string &cls,
		  const std::string &prop)
{
	if (ent == nullptr)
		return 0;
...
Vector GetPropVector (CBaseEntity *ent, const std::string &cls,
		      const std::string &prop)
{
	if (ent == nullptr)
		return {};
...
CBaseEntity *GetPropEntity (CBaseEntity *ent, const std::string &cls,
			    const std::string &prop)
{
	if (ent == nullptr)
		return nullptr;
...
void SetPropInt (CBaseEntity *ent, const std::string &cls,
...
void SetPropFloat (CBaseEntity *ent, const std::string &cls,
		   const std::string &prop, float value)
{
	if (ent == nullptr)
		return;
	SetProp (ent, cls, prop, value);
...
void SetPropVector (CBaseEntity *ent, const std::string &cls,
		    const std::string &prop, Vector value)
{
	if (ent == nullptr)
		return;
	SetProp (ent, cls, prop, value);
...
void SetPropBool (CBaseEntity *ent, const std::string &cls,
...
void SetPropEntity (CBaseEntity *ent, const std::string &cls,
		    const std::string &prop, CBaseEntity *entity)
{
	if (ent == nullptr)
		return;
...
void Entity_RegisterClass (asIScriptEngine *engine)
{
	...
	engine->RegisterObjectBehaviour (
	    ENTITY_CLASSNAME, asBEHAVE_ADDREF, "void f()",
	    asFUNCTION (Entity_AddRef), asCALL_CDECL_OBJLAST);
	engine->RegisterObjectBehaviour (
	    ENTITY_CLASSNAME, asBEHAVE_RELEASE, "void f()",
	    asFUNCTION (Entity_Release), asCALL_CDECL_OBJLAST);

	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "int GetHealth() const", asFUNCTION (GetHealth),
	    asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "int GetMaxHealth() const",
	    asFUNCTION (GetMaxHealth), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "bool IsPlayer() const",
	    asMETHOD (CBaseEntity, IsPlayer), asCALL_THISCALL);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "bool IsWeapon() const",
	    asMETHOD (CBaseEntity, IsWeapon), asCALL_THISCALL);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "int GetIndex() const",
				      asMETHOD (CBaseEntity, GetIndex),
				      asCALL_THISCALL);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "bool IsPlayer()",
				      asMETHOD (CBaseEntity, IsPlayer),
				      asCALL_THISCALL);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "bool IsBuilding()",
				      asMETHOD (CBaseEntity, IsBuilding),
				      asCALL_THISCALL);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "int GetClassID() const",
	    asMETHOD (CBaseEntity, GetClassID), asCALL_THISCALL);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "bool IsRobot()",
				      asMETHOD (CBaseEntity, IsRobot),
				      asCALL_THISCALL);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "Vector3 EstimateAbsVelocity() const",
	    asMETHOD (CBaseEntity, EstimateAbsVelocity), asCALL_THISCALL);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "bool IsProjectile()",
				      asMETHOD (CBaseEntity, IsProjectile),
				      asCALL_THISCALL);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME,
				      "int GetPropInt(const string &in "
				      "className, const string &in propName)",
				      asFUNCTION (GetPropInt),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME,
				      "float GetPropFloat(const string &in "
				      "className, const string &in propName)",
				      asFUNCTION (GetPropFloat),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME,
				      "bool GetPropBool(const string &in "
				      "className, const string &in propName)",
				      asFUNCTION (GetPropBool),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME,
				      "Vector3 GetPropVector(const string &in "
				      "className, const string &in propName)",
				      asFUNCTION (GetPropVector),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME,
				      "Entity@ GetPropEntity(const string &in "
				      "className, const string &in propName)",
				      asFUNCTION (GetPropEntity),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME,
	...
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "Vector3 GetAbsOrigin()",
	    asFUNCTION (GetAbsOrigin), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "Vector3 GetAbsAngles()",
	    asFUNCTION (GetAbsAngles), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "bool ShouldDraw()",
				      asFUNCTION (ShouldDraw),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "Entity@ GetFirstMoveChild()",
	    asFUNCTION (GetFirstMoveChild), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "Entity@ GetNextMovePeer()",
	    asFUNCTION (GetNextMovePeer), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "void DrawModel(int flags)",
	    asFUNCTION (DrawModel), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "int GetTeamNumber()",
				      asFUNCTION (GetTeamNumber),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "bool IsAlive()",
				      asFUNCTION (Ent_IsAlive),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "int GetMoveType()",
				      asFUNCTION (GetMoveType),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "bool InCond(int cond)", asFUNCTION (InCond),
	    asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "string GetClassName()",
	    asFUNCTION (GetClassName), asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (ENTITY_CLASSNAME, "string GetName()",
				      asFUNCTION (GetName),
				      asCALL_CDECL_OBJFIRST);
	engine->RegisterObjectMethod (
	    ENTITY_CLASSNAME, "Entity@ GetEntityFromLoadoutSlot(int index)",
