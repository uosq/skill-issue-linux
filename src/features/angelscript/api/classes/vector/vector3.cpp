...
{
	engine->RegisterObjectType(VECTOR_CLASSNAME, sizeof(Vector), asOBJ_VALUE | asOBJ_POD | asGetTypeTraits<Vector>());
	...
	engine->RegisterObjectMethod(VECTOR_CLASSNAME, "Vector3 FromAngle()", asMETHOD(Vector, FromAngle), asCALL_THISCALL);
}