...
{
	return static_cast<CClientState *> (interfaces::ClientState)
	    ->chokedcommands;
}
...
void ForceFullUpdate ()
{
	...
	static ForceFullUpdateFn original_ForceFullUpdate
	    = reinterpret_cast<ForceFullUpdateFn> (
		sigscan_module ("engine.so", "83 BF B8 01 00 00 FF 74 ? 55"));
	if (original_ForceFullUpdate == nullptr)
		return Logs::Error ("ForceFullUpdate is null!");

	original_ForceFullUpdate (
	    reinterpret_cast<CClientState *> (interfaces::ClientState));
}
...
void ClientState_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->SetDefaultNamespace ("ClientState");
	{
		engine->SetDefaultAccessMask (
		    ScriptAccessMask::SCRIPT_MASK_ALLOW_CLIENTSTATE);
		engine->RegisterGlobalFunction ("int GetChokedCommands()",
						asFUNCTION (GetChokedCommands),
						asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "int GetLastOutgoingCommand()",
		    asFUNCTION (GetLastOutgoingCommand), asCALL_CDECL);
		engine->RegisterGlobalFunction ("int GetLastCommandAck()",
						asFUNCTION (GetLastCommandAck),
						asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "int GetClientSignonState()",
		    asFUNCTION (GetClientSignonState), asCALL_CDECL);
		engine->RegisterGlobalFunction ("int GetDeltaTick()",
						asFUNCTION (GetDeltaTick),
						asCALL_CDECL);
		engine->RegisterGlobalFunction ("void ForceFullUpdate()",
						asFUNCTION (ForceFullUpdate),
						asCALL_CDECL);
	}
