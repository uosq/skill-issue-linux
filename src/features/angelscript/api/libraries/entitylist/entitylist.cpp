...
{
	return static_cast<CBaseEntity *> (
	    interfaces::EntityList->GetClientEntity (index));
}

int EL_GetMaxEntities () { return interfaces::EntityList->GetMaxEntities (); }
...
{
	CBaseEntity *pEntity = static_cast<CBaseEntity *> (
	    interfaces::EntityList->GetClientEntity (index));
...
CScriptArray *EL_GetEntities ()
{
	...
	{
		CBaseEntity *ent = static_cast<CBaseEntity *> (
...
void Interface_EntityList_Register (asIScriptEngine *engine)
{
	...
	engine->SetDefaultAccessMask (
	    ScriptAccessMask::SCRIPT_MASK_ALLOW_ENTITYLIST);

	engine->RegisterGlobalFunction (
	    "Entity@ GetClientEntity(int entindex)",
	    asFUNCTION (EL_GetClientEntity), asCALL_CDECL);

	engine->RegisterGlobalFunction ("int GetMaxEntities()",
					asFUNCTION (EL_GetMaxEntities),
					asCALL_CDECL);

	engine->RegisterGlobalFunction ("Entity@ GetLocalPlayer()",
					asFUNCTION (EL_GetLocalPlayer),
					asCALL_CDECL);

	engine->RegisterGlobalFunction ("array<Entity@>@ GetEntities()",
					asFUNCTION (EL_GetEntities),
