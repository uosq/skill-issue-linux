...
void Common_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->RegisterGlobalFunction ("void print(const string &in text)",
					asFUNCTION (Print), asCALL_CDECL);
	engine->RegisterGlobalFunction ("void warn(const string &in text)",
					asFUNCTION (Warn), asCALL_CDECL);
	engine->RegisterGlobalFunction ("bool Execute(const string &in text)",
					asFUNCTION (Execute), asCALL_CDECL);
}