...
void Input_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->SetDefaultNamespace ("Input");
	{
		...
		engine->RegisterGlobalFunction (
		    ...
		    asFUNCTION (Input_IsButtonDown), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool IsButtonPressed(int button)",
		    asFUNCTION (Input_IsButtonPressed), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool IsButtonReleased(int button)",
		    asFUNCTION (Input_IsButtonReleased), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetCursorAlwaysVisible(bool state)",
		    asFUNCTION (Input_SetCursorAlwaysVisible), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool IsCursorVisible()",
		    asFUNCTION (Input_IsCursorVisible), asCALL_CDECL);
		engine->RegisterGlobalFunction ("int GetPollTick()",
						asFUNCTION (Input_GetPollTick),
						asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool IsButtonValid()", asFUNCTION (Input_IsButtonValid),
		    asCALL_CDECL);
	}
...
int Input_GetPollTick () { return interfaces::InputSystem->GetPollTick (); }

bool Input_IsButtonValid (int button)
{
	return button <= ButtonCode_t::BUTTON_CODE_INVALID
	       || button >= ButtonCode_t::BUTTON_CODE_COUNT;
}