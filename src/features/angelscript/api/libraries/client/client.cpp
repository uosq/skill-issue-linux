...
void ClearConsole () { interfaces::EngineVGui->ClearConsole (); }
...
void Client_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->SetDefaultNamespace ("Client");
	{
		...
		engine->RegisterGlobalFunction (
		    "ConVar@ GetConVar(const string &in name)",
		    asFUNCTION (GetConVar), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void Command(const string &in text, bool unrestricted = "
		    "false)",
		    asFUNCTION (RunCommand), asCALL_CDECL);
		engine->RegisterGlobalFunction ("void ClearConsole()",
						asFUNCTION (ClearConsole),
						asCALL_CDECL);
	}
