...
ITexture *CreateTextureNamedRenderTarget (const std::string &name, int width,
					  int height)
{
	return MaterialManager::CreateTextureNamedRenderTarget (name, width,
								height);
}
...
void Materials_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->SetDefaultNamespace ("Materials");
	{
		engine->SetDefaultAccessMask (
		    ScriptAccessMask::SCRIPT_MASK_ALLOW_MATERIALS);
		engine->RegisterGlobalFunction (
		    "Material@ CreateMaterial(const string &in name, const "
		    "string &in vmt)",
		...
		engine->RegisterGlobalFunction (
		    "Material@ GetMaterial(const string &in name)",
		    asFUNCTION (GetMat), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "Texture@ CreateTextureNamedRenderTarget(const string &in "
