...
bool HasFocus () { return interfaces::Surface->HasFocus (); }
...
bool CreateFont (const std::string &name, const std::string &family,
		 int height, int weight, int flags)
{
	int font
	    = FontManager::CreateFont (name, family, height, weight, flags);
	return font != 0;
}

bool SetFont (const std::string &name) { return FontManager::SetFont (name); }
...
void RectFade (int x0, int y0, int x1, int y1, int alpha0, int alpha1,
	       bool bHorizontal)
{
	interfaces::Surface->DrawFilledRectFade (
	    x0, y0, x1, y1, (unsigned int)alpha0, (unsigned int)alpha1,
	    bHorizontal);
}

bool IsCursorVisible () { return interfaces::Surface->IsCursorVisible (); }
...
void Draw_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->SetDefaultNamespace ("Draw");
	{
		...
		engine->RegisterGlobalFunction (
		    "void Line(int x0, int y0, int x1, int y1)",
		    asFUNCTION (Line), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetColor(int r, int g, int b, int a)",
		    asFUNCTION (SetColor), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void GetTextSize(const string &in text, int &out w, int "
		    "&out h)",
		    asFUNCTION (GetTextSize), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void GetCursorPos(int &out x, int &out y)",
		    asFUNCTION (GetCursorPos), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetCursorPos(int x, int y)",
		...
		engine->RegisterGlobalFunction (
		    "bool HasFocus()", asFUNCTION (HasFocus), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void GetScreenSize(int &out w, int &out h)",
		    asFUNCTION (GetScreenSize), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool CreateFont(const string &in name, const string &in "
		    "family, int height, int weight, int flags)",
		    asFUNCTION (CreateFont), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool SetFont(const string &in name)",
		    asFUNCTION (SetFont), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void Rect(int x, int y, int w, int h)", asFUNCTION (Rect),
		    asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void RectFade(int x0, int y0, int x1, int y1, int "
		    "alpha0, int alpha1, bool bHorizontal)",
		    asFUNCTION (RectFade), asCALL_CDECL);
		engine->RegisterGlobalFunction ("bool IsCursorVisible()",
						asFUNCTION (IsCursorVisible),
						asCALL_CDECL);
	}
