...
float GetBlend () { return interfaces::RenderView->GetBlend (); }
...
void SetBlend (float blend) { interfaces::RenderView->SetBlend (blend); }

void ForcedMaterialOverride (IMaterial *mat)
{
	if (!mat)
		return interfaces::StudioRender->ForcedMaterialOverride (
		    nullptr);
...
void DrawScreenSpaceRectangle (IMaterial *mat, int destx, int desty, int width,
			       int height, float src_texture_x0,
			       float src_texture_y0, float src_texture_x1,
			       float src_texture_y1, int src_texture_width,
			       int src_texture_height)
{
	...
	ctx->DrawScreenSpaceRectangle (mat, destx, desty, width, height,
...
void ClearStencilBufferRectangle (int xmin, int ymin, int xmax, int ymax,
				  int value)
{
	auto ctx = interfaces::MaterialSystem->GetRenderContext ();
	if (!ctx)
...
void Render_RegisterLibrary (asIScriptEngine *engine)
{
	...
	engine->SetDefaultNamespace ("Render");
	{
		...
		engine->RegisterGlobalFunction (
		    "void GetColorModulation(float &out r, float &out g, "
		    "float &out b)",
		...
		engine->RegisterGlobalFunction (
		    "float GetBlend()", asFUNCTION (GetBlend), asCALL_CDECL);
		engine->RegisterGlobalFunction ("void SetBlend(float blend)",
						asFUNCTION (SetBlend),
						asCALL_CDECL);
		engine->RegisterGlobalFunction (
		...
		engine->RegisterGlobalFunction (
		    ...
		    asFUNCTION (GetMaterialOverride), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void DrawScreenSpaceRectangle(Material@, int, int, int, "
		    "int, float, float, float, float, int, int)",
		    asFUNCTION (DrawScreenSpaceRectangle), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void DrawScreenSpaceQuad(Material@)",
		    asFUNCTION (DrawScreenSpaceQuad), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void GetViewport(int &out, int &out, int &out, int &out)",
		    asFUNCTION (GetViewport), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void Viewport(int, int, int, int)", asFUNCTION (Viewport),
		    asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void DepthRange(float, float)", asFUNCTION (DepthRange),
		    asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "bool GetDepthRange(float &out, float &out)",
		    asFUNCTION (GetDepthRange), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetRenderTarget(Texture@)",
		    asFUNCTION (SetRenderTarget), asCALL_CDECL);
		engine->RegisterGlobalFunction ("Texture@ GetRenderTarget()",
						asFUNCTION (GetRenderTarget),
						asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void PushRenderTargetAndViewport()",
		...
		engine->RegisterGlobalFunction (
		    ...
		    asFUNCTION (ClearBuffers), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void ClearColor3ub(uint, uint, uint)",
		    asFUNCTION (ClearColor3ub), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void ClearColor4ub(uint, uint, uint, uint)",
		    asFUNCTION (ClearColor4ub), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void OverrideDepthEnable(bool, bool)",
		    asFUNCTION (OverrideDepthEnable), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void OverrideAlphaEnable(bool, bool)",
		    asFUNCTION (OverrideAlphaEnable), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void OverrideColorEnable(bool, bool)",
		    asFUNCTION (OverrideColorEnable), asCALL_CDECL);
		engine->RegisterGlobalFunction ("void SetStencilEnable(bool)",
						asFUNCTION (SetStencilEnable),
						asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetStencilFailOperation(int)",
		    asFUNCTION (SetStencilFailOperation), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetStencilZFailOperation(int)",
		    asFUNCTION (SetStencilZFailOperation), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetStencilPassOperation(int)",
		    asFUNCTION (SetStencilPassOperation), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		...
		engine->RegisterGlobalFunction (
		    ...
		    asFUNCTION (SetStencilTestMask), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void SetStencilWriteMask(uint)",
		    asFUNCTION (SetStencilWriteMask), asCALL_CDECL);
		engine->RegisterGlobalFunction (
		    "void ClearStencilBufferRectangle(int, int, int, int, "
		    "int)",
