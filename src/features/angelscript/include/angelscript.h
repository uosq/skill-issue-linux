...
enum asEObjTypeFlags : asQWORD
{
	...
	asOBJ_APP_CLASS_CDA =
	    (asOBJ_APP_CLASS + asOBJ_APP_CLASS_CONSTRUCTOR + asOBJ_APP_CLASS_DESTRUCTOR + asOBJ_APP_CLASS_ASSIGNMENT),
...
#if defined(__LP64__) || defined(__amd64__) || defined(__x86_64__) || defined(_M_X64) || defined(__aarch64__) ||       \
    defined(_M_ARM64)
#ifndef AS_64BIT_PTR
#define AS_64BIT_PTR
#endif
...
// Check if the compiler can use C++11 features
    ...
    (__GNUC__ == 4 && __GNUC_MINOR__ >= 7) // gnuc 4.7 or clang
#if !(defined(__GNUC__) && defined(__cplusplus) &&                                                                     \
      __cplusplus < 201103L) // gnuc and clang require compiler flag -std=c++11
...
#else
template <typename T> T _implicit_cast(T val)
{
	return val;
}
...
template <typename T> asUINT asGetTypeTraits()
{
...
class asIScriptEngine
{
	...
	virtual int GetGlobalPropertyByIndex(asUINT index, const char **name, const char **nameSpace = 0,
					     int *typeId = 0, bool *isConst = 0, const char **configGroup = 0,
					     void **pointer = 0, asDWORD *accessMask = 0) const = 0;
	...
	virtual int RegisterObjectMethod(const char *obj, const char *declaration, const asSFuncPtr &funcPointer,
					 asDWORD callConv, void *auxiliary = 0, int compositeOffset = 0,
					 bool isCompositeIndirect = false)			       = 0;
	virtual int RegisterObjectBehaviour(const char *obj, asEBehaviours behaviour, const char *declaration,
					    const asSFuncPtr &funcPointer, asDWORD callConv, void *auxiliary = 0,
					    int compositeOffset = 0, bool isCompositeIndirect = false) = 0;
	...
	virtual int RefCastObject(void *obj, asITypeInfo *fromType, asITypeInfo *toType, void **newPtr,
	...
	virtual int SetContextCallbacks(asREQUESTCONTEXTFUNC_t requestCtx, asRETURNCONTEXTFUNC_t returnCtx,
					void *param = 0)  = 0;

	// String interpretation
	virtual asETokenClass ParseToken(const char *string, size_t stringLength = 0,
					 asUINT *tokenLength = 0) const = 0;

	// Garbage collection
	virtual int GarbageCollect(asDWORD flags = asGC_FULL_CYCLE, asUINT numIterations = 1)		  = 0;
	virtual void GetGCStatistics(asUINT *currentSize, asUINT *totalDestroyed = 0, asUINT *totalDetected = 0,
				     asUINT *newObjects = 0, asUINT *totalNewDestroyed = 0) const	  = 0;
...
class asIScriptModule
{
	...
	virtual int Build()											    = 0;
	virtual int CompileFunction(const char *sectionName, const char *code, int lineOffset, asDWORD compileFlags,
...
class asIScriptContext
{
...
#endif
	virtual const char *GetVarDeclaration(asUINT varIndex, asUINT stackLevel = 0,
					      bool includeNamespace = false) = 0;
#ifdef AS_DEPRECATED
	// deprecated since 2022-05-04, 2.36.0
	...
	virtual int GetStateRegisters(asUINT stackLevel, asIScriptFunction **callingSystemFunction,
				      ...
				      asITypeInfo **objectTypeRegister)					     = 0;
	virtual int GetCallStateRegisters(asUINT stackLevel, asDWORD *stackFramePointer,
					  ...
					  asDWORD *stackPointer, asDWORD *stackIndex)			     = 0;
	virtual int SetStateRegisters(asUINT stackLevel, asIScriptFunction *callingSystemFunction,
				      asIScriptFunction *initialFunction, asDWORD origStackPointer,
				      asDWORD argumentsSize, asQWORD valueRegister, void *objectRegister,
				      asITypeInfo *objectTypeRegister)					     = 0;
	virtual int SetCallStateRegisters(asUINT stackLevel, asDWORD stackFramePointer,
					  asIScriptFunction *currentFunction, asDWORD programPointer,
					  asDWORD stackPointer, asDWORD stackIndex)			     = 0;
...
class asITypeInfo
{
	...
	// Properties
	virtual asUINT GetPropertyCount() const									  = 0;
	virtual int GetProperty(asUINT index, const char **name, int *typeId = 0, bool *isPrivate = 0,
				bool *isProtected = 0, int *offset = 0, bool *isReference = 0, asDWORD *accessMask = 0,
				int *compositeOffset = 0, bool *isCompositeIndirect = 0, bool *isConst = 0) const = 0;
...
class asIScriptFunction
{
	...
	virtual int GetParam(asUINT index, int *typeId, asDWORD *flags = 0, const char **name = 0,
...
{
	template <class M> static asSFuncPtr Convert(M Mthd)
	{
...
template <> struct asSMethodPtr<SINGLE_PTR_SIZE>
{
	template <class M> static asSFuncPtr Convert(M Mthd)
	{
...
template <> struct asSMethodPtr<SINGLE_PTR_SIZE + 1 * sizeof(int)>
{
	template <class M> static asSFuncPtr Convert(M Mthd)
	{
		// Mark this as a class method
...
template <> struct asSMethodPtr<SINGLE_PTR_SIZE + 2 * sizeof(int)>
{
	template <class M> static asSFuncPtr Convert(M Mthd)
	{
...
template <> struct asSMethodPtr<SINGLE_PTR_SIZE + 3 * sizeof(int)>
{
	template <class M> static asSFuncPtr Convert(M Mthd)
	{
		// Mark this as a class method
...
template <> struct asSMethodPtr<SINGLE_PTR_SIZE + 4 * sizeof(int)>
{
	template <class M> static asSFuncPtr Convert(M Mthd)
	{
...
const asSBCInfo asBCInfo[256] = {asBCINFO(PopPtr, NO_ARG, -AS_PTR_SIZE),
