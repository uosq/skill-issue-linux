...
void PrintTemplate(const char *base, const char *typeNameList, const char *retType, const char *objType,
		   const char *isConst, const char *newExpr, const char *objExpr, const char *argList1,
		   const char *argList2, const char *wrapName);
...
int main()
{
	...
	PrintTemplate("typename T, typename R", "", "R", "T::", "", new_exp.c_str(), obj_exp.c_str(), "void", "",
		      "Wrapper");
	PrintTemplate("typename T", "", "void", "T::", " const", "", obj_exp.c_str(), "void", "", "Wrapper");
	PrintTemplate("typename T, typename R", "", "R", "T::", " const", new_exp.c_str(), obj_exp.c_str(), "void", "",
		      "Wrapper");

	PrintTemplate("typename T", "", "void", "T::", "", "", obj_g_exp.c_str(), "void", "", "WrapperGlobal");
	PrintTemplate("typename T, typename R", "", "R", "T::", "", new_exp.c_str(), obj_g_exp.c_str(), "void", "",
		      "WrapperGlobal");
	PrintTemplate("typename T", "", "void", "T::", " const", "", obj_g_exp.c_str(), "void", "", "WrapperGlobal");
	PrintTemplate("typename T, typename R", "", "R", "T::", " const", new_exp.c_str(), obj_g_exp.c_str(), "void",
		      "", "WrapperGlobal");

	PrintTemplate("typename T", "", "void", "", "", "", "", "T", obj_arg_exp.c_str(), "ObjFirst");
	PrintTemplate("typename T, typename R", "", "R", "", "", new_exp.c_str(), "", "T", obj_arg_exp.c_str(),
		      "ObjFirst");
	...
	{
		PrintTemplate("", typename_list.c_str(), "void", "", "", "", "", type_list.c_str(), arg_list.c_str(),
			      "Wrapper");
		PrintTemplate("typename R, ", typename_list.c_str(), "R", "", "", new_exp.c_str(), "",
			      type_list.c_str(), arg_list.c_str(), "Wrapper");

		PrintTemplate("typename T, ", typename_list.c_str(), "void", "T::", "", "", obj_exp.c_str(),
			      type_list.c_str(), arg_list.c_str(), "Wrapper");
		PrintTemplate("typename T, typename R, ", typename_list.c_str(), "R", "T::", "", new_exp.c_str(),
			      obj_exp.c_str(), type_list.c_str(), arg_list.c_str(), "Wrapper");
		PrintTemplate("typename T, ", typename_list.c_str(), "void", "T::", " const", "", obj_exp.c_str(),
			      type_list.c_str(), arg_list.c_str(), "Wrapper");
		PrintTemplate("typename T, typename R, ", typename_list.c_str(), "R", "T::", " const", new_exp.c_str(),
			      obj_exp.c_str(), type_list.c_str(), arg_list.c_str(), "Wrapper");

		PrintTemplate("typename T, ", typename_list.c_str(), "void", "T::", "", "", obj_g_exp.c_str(),
			      type_list.c_str(), arg_list.c_str(), "WrapperGlobal");
		PrintTemplate("typename T, typename R, ", typename_list.c_str(), "R", "T::", "", new_exp.c_str(),
			      obj_g_exp.c_str(), type_list.c_str(), arg_list.c_str(), "WrapperGlobal");
		PrintTemplate("typename T, ", typename_list.c_str(), "void", "T::", " const", "", obj_g_exp.c_str(),
			      type_list.c_str(), arg_list.c_str(), "WrapperGlobal");
		PrintTemplate("typename T, typename R, ", typename_list.c_str(), "R", "T::", " const", new_exp.c_str(),
			      obj_g_exp.c_str(), type_list.c_str(), arg_list.c_str(), "WrapperGlobal");

		PrintTemplate("typename T, ", typename_list.c_str(), "void", "", "", "", "",
			      ("T, " + type_list).c_str(), (obj_arg_exp + "," + arg_list).c_str(), "ObjFirst");
		PrintTemplate("typename T, typename R, ", typename_list.c_str(), "R", "", "", new_exp.c_str(), "",
			      ("T, " + type_list).c_str(), (obj_arg_exp + "," + arg_list).c_str(), "ObjFirst");
		PrintTemplate("typename T, ", typename_list.c_str(), "void", "", "", "", "",
			      (type_list + ", T").c_str(), (arg_list + "," + obj_arg_exp).c_str(), "ObjLast");
		PrintTemplate("typename T, typename R, ", typename_list.c_str(), "R", "", "", new_exp.c_str(), "",
			      (type_list + ", T").c_str(), (arg_list + "," + obj_arg_exp).c_str(), "ObjLast");
		...
		arg_list += ",\n				static_cast<Proxy <A" + string(buf) +
			    "> *>(gen->GetAddressOfArg(" + string(buf) + "))->value";
	}

	printf("template <typename T>\n"
	       "struct Id {\n"
	       "	template <T fn_ptr> AS_NAMESPACE_QUALIFIER asSFuncPtr  f(void) { return AS_NAMESPACE_QUALIFIER "
	       "asFUNCTION(&Wrapper<T>::template f<fn_ptr>); }\n"
	       "	template <T fn_ptr> AS_NAMESPACE_QUALIFIER asSFuncPtr fg(void) { return AS_NAMESPACE_QUALIFIER "
	       "asFUNCTION(&WrapperGlobal<T>::template f<fn_ptr>); }\n"
	       "	template <T fn_ptr> AS_NAMESPACE_QUALIFIER asSFuncPtr of(void) { return AS_NAMESPACE_QUALIFIER "
	       "asFUNCTION(&ObjFirst<T>::template f<fn_ptr>); }\n"
	       "	template <T fn_ptr> AS_NAMESPACE_QUALIFIER asSFuncPtr ol(void) { return AS_NAMESPACE_QUALIFIER "
	       "asFUNCTION(&ObjLast<T>::template f<fn_ptr>); }\n"
	       "};\n"
	       "\n"
	       "template <typename T>\n"
	       ...
	       "\n"
	       "#define WRAP_FN_PR(name, Parameters, ReturnType)             asFUNCTION((::gw::Wrapper<ReturnType "
	       "(*)Parameters>::TMPL f< name >))\n"
	       "#if defined(__clang__)\n"
	       " // Clang doesn't like the use of AS_METHOD_AMBIGUITY_CAST in the inner template\n"
	       " #define WRAP_MFN_PR(ClassType, name, Parameters, ReturnType) asFUNCTION((::gw::Wrapper<ReturnType "
	       "(ClassType::*)Parameters>::TMPL f< &ClassType::name >))\n"
	       ...
	       "#else\n"
	       " #define WRAP_MFN_PR(ClassType, name, Parameters, ReturnType) asFUNCTION((::gw::Wrapper<ReturnType "
	       "(ClassType::*)Parameters>::TMPL f< AS_METHOD_AMBIGUITY_CAST(ReturnType "
	       "(ClassType::*)Parameters)(&ClassType::name) >))\n"
	       " #define WRAP_MFN_GLOBAL_PR(ClassType, name, Parameters, ReturnType) "
	       "asFUNCTION((::gw::WrapperGlobal<ReturnType (ClassType::*)Parameters>::TMPL f< "
	       "AS_METHOD_AMBIGUITY_CAST(ReturnType (ClassType::*)Parameters)(&ClassType::name) >))\n"
	       "#endif\n"
	       "#define WRAP_OBJ_FIRST_PR(name, Parameters, ReturnType)      asFUNCTION((::gw::ObjFirst<ReturnType "
	       "(*)Parameters>::TMPL f< name >))\n"
	       "#define WRAP_OBJ_LAST_PR(name, Parameters, ReturnType)       asFUNCTION((::gw::ObjLast<ReturnType "
	       "(*)Parameters>::TMPL f< name >))\n"
	       "\n"
