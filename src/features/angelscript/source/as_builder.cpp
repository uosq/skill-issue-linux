...
void asCBuilder::EvaluateTemplateInstances(asUINT startIdx, bool keepSilent)
{
	...
	if (keepSilent)
		engine->SetMessageCallback(asFUNCTION(asCOutputBuffer::CDeclCallback), &outBuffer,
	...
	for (asUINT n = startIdx; n < engine->templateInstanceTypes.GetLength(); n++)
	{
		    ...
		    !engine->CallGlobalFunctionRetBool(tmpl, &dontGarbageCollect, callback->sysFuncIntf, callback))
		{
			...
			WriteError(tmpl->scriptSectionIdx >= 0
				       ? engine->scriptSectionNames[tmpl->scriptSectionIdx]->AddressOf()
				       : "",
...
int asCBuilder::CompileGlobalVar(const char *sectionName, const char *code, int lineOffset)
{
	...
	for (asUINT n = 0; n < functions.GetLength(); n++)
	{
		...
		int r = compiler.CompileFunction(this, functions[n]->script, func->parameterNames, functions[n]->node,
						 func, 0);
		if (r < 0)
			break;
	}
				...
				asCScriptFunction **outFunc)
{
	...
	{
		r = CheckNameConflict(func->name.AddressOf(), node, scripts[0], module->m_defaultNamespace, false,
				      false, false);
		if (r < 0)
		{
			func->ReleaseInternal();
	...
	if (numErrors == 0)
	{
		...
		for (n = 0; n < interfaceDeclarations.GetLength(); n++)
		{
			...
			while (node)
			{
				...
				{
					node->DisconnectParent();
					RegisterScriptFunctionFromNode(node, decl->script,
								       CastToObjectType(decl->typeInfo), true, false, 0,
								       decl->isExistingShared);
				}
				else if (node->nodeType == snVirtualProperty)
				{
					node->DisconnectParent();
					RegisterVirtualProperty(node, decl->script, CastToObjectType(decl->typeInfo),
								true, false, 0, decl->isExistingShared);
				}
		...
		for (n = 0; n < classDeclarations.GetLength(); n++)
		{
			...
			while (node)
			{
				...
				if (node->nodeType == snFunction)
				{
					node->DisconnectParent();
					RegisterScriptFunctionFromNode(node, decl->script,
								       CastToObjectType(decl->typeInfo), false, false,
								       0, decl->isExistingShared, false, decl);
				}
				else if (node->nodeType == snVirtualProperty)
				{
					node->DisconnectParent();
					RegisterVirtualProperty(node, decl->script, CastToObjectType(decl->typeInfo),
								false, false, 0, decl->isExistingShared);
				}
			...
			if (!decl->isExistingShared)
			{
				...
				if (engine->ep.alwaysImplDefaultCopy == 2 || decl->isDefaultCopyDeleted ||
				    (copyOperatorExists && ot->beh.copy == engine->scriptTypeBehaviours.beh.copy &&
				     engine->ep.alwaysImplDefaultCopy == 0))
				{
				...
				if (!decl->isDefaultConstructorDeleted &&
				    ((engine->ep.alwaysImplDefaultConstruct == 0 &&
				...
				{
					if (engine->scriptFunctions[ot->beh.constructors[i]]
						->parameterTypes.GetLength() == 1)
					{
				...
				if (!decl->isDefaultCopyConstructorDeleted &&
				    ((engine->ep.alwaysImplDefaultCopyConstruct == 0 && !copyConstructExists) ||
				     engine->ep.alwaysImplDefaultCopyConstruct == 1))
...
void asCBuilder::AddVisibleNamespaces(asSNameSpace *ns, const asCArray<asSNameSpace *> &visited,
				      asCArray<asSNameSpace *> &pending)
{
...
asSNameSpace *asCBuilder::FindNextVisibleNamespace(const asCArray<asSNameSpace *> &visited,
						   asCArray<asSNameSpace *> &pending, asSNameSpace *parentNs,
						   bool *checkAmbiguous)
{
...
void asCBuilder::CompileFunctions()
{
	...
	for (asUINT n = 0; n < functions.GetLength(); n++)
	{
		...
		if (current->node && !(current->node->nodeType == snStatementBlock ||
		...
		else if (current->objType && current->name == current->objType->name)
		{
			...
			else
			{
				asASSERT(func->parameterTypes.GetLength() == 1 &&
					 func->parameterTypes[0].GetTypeInfo() == current->objType);
				...
				// automatically if not implemented by the user.
				r = compiler.CompileDefaultCopyConstructor(this, current->script, node, func,
									   classDecl);
			}
					 ...
					 asCObjectType **outParentClass)
{
	...
	func->returnType = CreateDataTypeFromNode(node->firstChild, &source, objType ? objType->nameSpace : ns, true,
						  parentClass ? parentClass : objType, true, 0,
						  isTemplate ? &func->templateSubTypes : 0);
	...
	if (isSystemFunction &&
	    (func->returnType.GetTypeInfo() && (func->returnType.GetTypeInfo()->flags & asOBJ_REF)) &&
	...
	while (n)
	{
		...
		asCDataType type;
		type = CreateDataTypeFromNode(n, &source, objType ? objType->nameSpace : ns, false,
					      parentClass ? parentClass : objType, true, 0,
					      isTemplate ? &func->templateSubTypes : 0);
...
int asCBuilder::CheckNameConflict(const char *name, asCScriptNode *node, asCScriptCode *code, asSNameSpace *ns,
				  bool isProperty, bool isVirtualProperty, bool isSharedIntf)
{
...
int asCBuilder::ValidateVirtualProperty(asCScriptFunction *func)
{
	...
	if (prefix == "set_" && (func->returnType != asCDataType::CreatePrimitive(ttVoid, false) ||
				 func->parameterTypes.GetLength() < 1 || func->parameterTypes.GetLength() > 2))
		return -3;
...
void asCBuilder::CompleteFuncDef(sFuncDef *funcDef)
{
	...
	if (func->returnType.GetTypeInfo() && !func->returnType.GetTypeInfo()->IsShared())
	{
		if (declaredShared)
		{
			...
			s.Format(TXT_SHARED_CANNOT_USE_NON_SHARED_TYPE_s,
				 func->returnType.GetTypeInfo()->name.AddressOf());
	...
	if (func->IsShared())
	{
		for (asUINT n = 0; n < engine->funcDefs.GetLength(); n++)
		{
			...
			if (fdt2->name == fdt->name && fdt2->nameSpace == fdt->nameSpace &&
			    fdt2->parentClass == fdt->parentClass && fdt2->funcdef->IsSignatureExceptNameEqual(func))
			{
	...
	if (funcTraits.GetTrait(asTRAIT_EXTERNAL) && found)
		module->m_externalTypes.PushLast(
		    engine->scriptFunctions[module->m_funcDefs[funcDef->idx]->GetFuncdefSignature()->GetId()]
			->funcdefType);
}
...
int asCBuilder::RegisterMixinClass(asCScriptNode *node, asCScriptCode *file, asSNameSpace *ns)
{
	...
	while (n->tokenType == ttIdentifier && (file->TokenEquals(n->tokenPos, n->tokenLength, FINAL_TOKEN) ||
		...
		for (; it; it++)
		{
			...
			if (gvar->isEnumValue)
			{
				...
				if (gvar->initializationNode)
				{
					...
					r = comp.CompileGlobalVariable(this, gvar->script, gvar->initializationNode,
								       gvar, &func);
				...
				else
				{
					...
					if (prev_it)
					{
						...
						if (gvar2->datatype == gvar->datatype)
						{
							...
							{
								int row, col;
								gvar->script->ConvertPosToRowCol(
								    gvar->declaredAtNode->tokenPos, &row, &col);
								...
								str.Format(TXT_UNINITIALIZED_GLOBAL_VAR_s,
									   gvar2->name.AddressOf());
			...
			else
			{
				...
				asCCompiler comp(engine);
				int r = comp.CompileGlobalVariable(this, gvar->script, gvar->initializationNode, gvar,
								   initFunc);
				if (r >= 0)
				{
			...
			if (gvar->isCompiled)
			{
				...
				if (initFunc && initFunc->scriptData->byteCode.GetLength() > 2)
				{
					...
					{
						int row, col;
						gvar->script->ConvertPosToRowCol(gvar->declaredAtNode->tokenPos, &row,
										 &col);
						initFunc->scriptData->declaredAt =
						    (row & 0xFFFFF) | ((col & 0xFFF) << 20);
					}
...
int asCBuilder::GetNamespaceAndNameFromNode(asCScriptNode *n, asCScriptCode *script, asSNameSpace *implicitNs,
					    asSNameSpace *&outNs, asCString &outName, bool *isExplicitNs)
{
...
void asCBuilder::DetermineTypeRelations()
{
	...
	for (asUINT n = 0; n < interfaceDeclarations.GetLength(); n++)
	{
		...
		while (node && node->nodeType == snIdentifier)
		{
			...
			if (GetNamespaceAndNameFromNode(node, intfDecl->script, intfType->nameSpace, ns, name,
							&isExplicitNs) < 0)
			{
				node = node->next;
				continue;
			...
			bool ok = FindObjectTypeOrMixinInNsHierarchy(name, ns, isExplicitNs, node, intfDecl->script,
								     &objType, 0);
			...
			if (ok)
			{
				if (objType && objType->IsInterface())
				{
					...
					{
						asCString str;
						str.Format(TXT_SHARED_CANNOT_IMPLEMENT_NON_SHARED_s,
							   objType->GetName());
	...
	for (asUINT n = 0; n < classDeclarations.GetLength(); n++)
	{
		...
		while (node && node->nodeType == snIdentifier)
		{
			...
			asCString name;
			bool isExplicitNs = false;
			if (GetNamespaceAndNameFromNode(node, file, decl->typeInfo->nameSpace, ns, name,
							&isExplicitNs) < 0)
			{
				node = node->next;
				continue;
			...
			bool ok =
			    FindObjectTypeOrMixinInNsHierarchy(name, ns, isExplicitNs, node, file, &objType, &mixin);
			...
			if (ok)
			{
				if (objType == 0 && mixin == 0)
				{
					asCString str;
					if (origNs->name == "")
						str.Format(TXT_IDENTIFIER_s_NOT_DATA_TYPE_IN_GLOBAL_NS,
							   name.AddressOf());
					else
						str.Format(TXT_IDENTIFIER_s_NOT_DATA_TYPE_IN_NS_s, name.AddressOf(),
							   origNs->name.AddressOf());
					WriteError(str, file, node);
				}
				else if (mixin)
				{
				...
				else if (objType->size != 0)
				{
					...
					if (!decl->isExistingShared &&
					    CastToObjectType(decl->typeInfo)->derivedFrom != 0)
					{
						if (!multipleInheritance)
						{
							WriteError(TXT_CANNOT_INHERIT_FROM_MULTIPLE_CLASSES, file,
								   node);
					...
					else
					{
						...
						if (!error)
						{
							...
							{
								asCString msg;
								msg.Format(TXT_SHARED_CANNOT_INHERIT_FROM_NON_SHARED_s,
									   objType->name.AddressOf());
						...
						if (!error)
						{
							if (decl->isExistingShared)
							{
								...
								if (CastToObjectType(decl->typeInfo)->derivedFrom !=
								    objType)
								{
									asCString str;
									str.Format(TXT_SHARED_s_DOESNT_MATCH_ORIGINAL,
										   decl->typeInfo->GetName());
									WriteError(str, file, node);
								}
							}
							else
...
void asCBuilder::CompileClasses(asUINT numTempl)
{
		...
		if (!decl->isExistingShared && ot->derivedFrom)
		{
			...
			for (asUINT m = 0; m < baseType->methods.GetLength(); m++)
			{
				...
				for (asUINT d = 0; d < ot->methods.GetLength(); d++)
				{
					    ...
					    baseFunc->name == "opCast" || baseFunc->name == "opImplCast")
					{
						    ...
						    derivedFunc->IsSignatureExceptNameEqual(baseFunc))
						{
							...
							{
								asCString msg;
								msg.Format(TXT_METHOD_CANNOT_OVERRIDE_s,
					...
					else
					{
						    ...
						    derivedFunc->IsSignatureExceptNameAndReturnTypeEqual(baseFunc))
						{
							...
							{
								asCString msg;
								msg.Format(TXT_DERIVED_METHOD_MUST_HAVE_SAME_RETTYPE_s,
									   baseFunc->GetDeclaration());
								WriteError(msg, decl->script, decl->node);
							}
				...
				if (!found)
				{
					...
					CheckForConflictsDueToDefaultArgs(decl->script, decl->node,
									  baseType->virtualFunctionTable[m], ot);
				}
				...
				else
				{
					...
					for (asUINT p = 0; p < ot->properties.GetLength(); p++)
					{
						...
						if (prop->isPrivate == isPrivate && prop->isProtected == isProtected &&
						    prop->name == name && prop->type.IsEqualExceptRef(dt))
						{
							found = true;
							break;
						}
	...
	for (n = 0; n < classDeclarations.GetLength(); n++)
	{
		...
		{
			if (!overrideValidations[j] &&
			    (!hasBaseClass || !DoesMethodExist(ot->derivedFrom, ot->methods[j])))
			{
				asCString msg;
				msg.Format(TXT_METHOD_s_DOES_NOT_OVERRIDE,
					   ot->GetMethodByIndex(j, false)->GetDeclaration());
				WriteError(msg, decl->script, decl->node);
			}
		}
	}
		...
		if (funcDesc != asUINT(-1) && functions[funcDesc]->node == 0)
		{
			...
			if (objType->derivedFrom)
			{
				...
				else
				{
					...
					asCScriptFunction *baseFunc =
					    engine->scriptFunctions[objType->derivedFrom->beh.copyconstruct];
			...
			for (asUINT m = 0; canBeCopied && m < objType->properties.GetLength(); m++)
			{
				...
				if (propObjType && !prop->type.IsObjectHandle() &&
				    propObjType->beh.copyconstruct == 0 &&
				...
				else if (propObjType && !prop->type.IsObjectHandle())
				{
					...
					// TODO: Should allow it anyway, but the copy constructor should be generated as taking a non-const reference
					asCScriptFunction *baseFunc =
					    engine->scriptFunctions[propObjType->beh.copyconstruct];
					if (baseFunc && !baseFunc->parameterTypes[0].IsObjectConst())
						canBeCopied = false;
					else if (!baseFunc)
					{
			...
			for (asUINT p = 0; p < type->properties.GetLength(); p++)
			{
				...
				else
				{
					if (dt.IsTemplate())
					{
						...
						{
							asCObjectType *t = reinterpret_cast<asCObjectType *>(
							    dt.GetTypeInfo()->GetSubType(s));
					...
					if (dt.IsObjectHandle())
					{
						...
						if (prop->flags & asOBJ_SCRIPT_OBJECT)
						{
							...
							{
								for (asUINT sp = 0; sp < prop->properties.GetLength();
								     sp++)
								{
		...
		if (mixin)
		{
			...
			while (n)
			{
				if (n->nodeType == snFunction)
				{
					...
					RegisterScriptFunctionFromNode(copy, mixin->script,
								       CastToObjectType(decl->typeInfo), false, false,
								       mixin->ns, false, true);
				}
		...
		if (mixin)
		{
			...
			while (n)
			{
				if (n->nodeType == snDeclaration)
				{
					...
					if (decl->typeInfo->IsShared() && dt.GetTypeInfo() &&
					    !dt.GetTypeInfo()->IsShared())
					{
						asCString msg;
						msg.Format(TXT_SHARED_CANNOT_USE_NON_SHARED_TYPE_s,
							   dt.GetTypeInfo()->name.AddressOf());
						WriteError(msg, file, n);
					...
					while (n2)
					{
						...
						if (!exists)
						{
							if (!decl->isExistingShared)
							{
								...
								int r = CheckNameConflictMember(ot, name.AddressOf(),
												n2, file, true, false);
								if (r < 0)
									WriteInfo(TXT_WHILE_INCLUDING_MIXIN,
										  decl->script, node);

								AddPropertyToClass(decl, name, dt, isPrivate,
										   isProtected, false, file, n2);
							}
							else
							{
								// Verify that the property exists in the original declaration
...
asCObjectProperty *asCBuilder::AddPropertyToClass(sClassDeclaration *decl, const asCString &name, const asCDataType &dt,
						  bool isPrivate, bool isProtected, bool isInherited,
						  asCScriptCode *file, asCScriptNode *node)
{
	if (node)
	{
		...
		if (!dt.CanBeInstantiated())
		{
			if (file && node)
			{
				...
				if (dt.IsAbstractClass())
					...
					// TODO: Improve error message to explain why
					str.Format(TXT_DATA_TYPE_CANT_BE_s,
						   dt.Format(decl->typeInfo->nameSpace).AddressOf());
				WriteError(str, file, node);
			}
			return 0;
					  ...
					  asSNameSpace *implicitNamespace)
{
	...
	if (!funcTraits.GetTrait(asTRAIT_CONSTRUCTOR) && !funcTraits.GetTrait(asTRAIT_DESTRUCTOR))
	{
		...
		if (engine->ep.disallowValueAssignForRefType && returnType.GetTypeInfo() &&
		    ...
		    !(returnType.GetTypeInfo()->flags & asOBJ_SCOPED) && !returnType.IsReference() &&
		...
		while (decorator && decorator->tokenType == ttIdentifier)
		{
			if (objType && file->TokenEquals(decorator->tokenPos, decorator->tokenLength, FINAL_TOKEN))
				funcTraits.SetTrait(asTRAIT_FINAL, true);
				 ...
				 file->TokenEquals(decorator->tokenPos, decorator->tokenLength, OVERRIDE_TOKEN))
				funcTraits.SetTrait(asTRAIT_OVERRIDE, true);
			else if (objType &&
				 file->TokenEquals(decorator->tokenPos, decorator->tokenLength, EXPLICIT_TOKEN))
	...
	while (n)
	{
		...
		if (engine->ep.disallowValueAssignForRefType && type.GetTypeInfo() &&
		    (type.GetTypeInfo()->flags & asOBJ_REF) && !(type.GetTypeInfo()->flags & asOBJ_SCOPED) &&
		    !type.IsReference() && !type.IsObjectHandle())
		{
...
int asCBuilder::RegisterScriptFunctionFromNode(asCScriptNode *node, asCScriptCode *file, asCObjectType *objType,
					       ...
					       bool isExistingShared, bool isMixin, sClassDeclaration *decl)
{
	asCString name;
					      ...
					      const asCString &name, asSNameSpace *ns, bool isShared)
{
	...
	int r = RegisterScriptFunction(args, file, 0, 0, true, ns, false, false, funcName, funcDef->returnType,
				       parameterNames, funcDef->parameterTypes, funcDef->inOutFlags, defaultArgs,
				       traits, 0);
	if (r < 0)
		return 0;
...
int asCBuilder::RegisterScriptFunction(asCScriptNode *node, asCScriptCode *file, asCObjectType *objType,
				       bool isInterface, bool isGlobalFunction, asSNameSpace *ns, bool isExistingShared,
				       bool isMixin, asCString &name, asCDataType &returnType,
				       ...
				       asSFunctionTraits funcTraits, sClassDeclaration *classDecl)
{
	...
	else
	{
		...
		if (name != objType->name)
		{
			...
			if (funcTraits.GetTrait(asTRAIT_DESTRUCTOR))
				...
				str.Format(TXT_METHOD_s_s_HAS_NO_RETURN_TYPE, objType->name.AddressOf(),
					   name.AddressOf());
			WriteError(str, file, node);
		}
	...
	if (funcTraits.GetTrait(asTRAIT_DELETED))
	{
		...
		if (classDecl && funcTraits.GetTrait(asTRAIT_CONSTRUCTOR) && parameterTypes.GetLength() == 0)
		{
			...
			if (objType->beh.construct &&
			    objType->beh.construct != engine->scriptTypeBehaviours.beh.construct)
				WriteError(TXT_CANNOT_DEFINE_FUNC_THAT_IS_DELETED, file, node);
		}
		else if (classDecl && funcTraits.GetTrait(asTRAIT_CONSTRUCTOR) && parameterTypes.GetLength() == 1 &&
		...
		else if (classDecl && name == "opAssign" && parameterTypes.GetLength() == 1 &&
			 parameterTypes[0].GetTypeInfo() == objType)
		{
	...
	if (!isInterface)
	{
		...
		if (funcTraits.GetTrait(asTRAIT_SHARED))
		{
			...
			for (asUINT n = 0; n < engine->scriptFunctions.GetLength(); n++)
			{
				...
				if (f && f->IsShared() && f->name == name && f->nameSpace == ns &&
		...
		if (funcTraits.GetTrait(asTRAIT_EXTERNAL) &&
		    !(node == 0 || node->tokenType == ttEndStatement ||
		      (node->lastChild && node->lastChild->tokenType == ttEndStatement)))
		{
			asCString str;
			str.Format(TXT_EXTERNAL_SHARED_s_CANNOT_REDEF, name.AddressOf());
			WriteError(str, file, node);
			 ...
			 (node == 0 || !(node->nodeType == snStatementBlock ||
					 (node->lastChild && node->lastChild->nodeType == snStatementBlock))))
		{
			asCString str;
			str.Format(TXT_MISSING_DEFINITION_OF_s, name.AddressOf());
			WriteError(str, file, node);
	...
	if (objType)
	{
		...
		if (funcTraits.GetTrait(asTRAIT_CONSTRUCTOR))
		{
			...
			module->AddScriptFunction(file->idx, engine->scriptFunctions[funcId]->scriptData->declaredAt,
						  factoryId, name, dt, parameterTypes, parameterNames, inOutFlags,
						  defaultArgs, false, 0, false, funcTraits, objType->nameSpace);
					...
					bool isExistingShared)
{
	...
	while (node)
	{
		...
		if (name != "")
		{
			...
			while (funcNode &&
			       (funcNode->nodeType != snStatementBlock && funcNode->tokenType != ttEndStatement))
			{
			...
			if ((funcNode == 0 || funcNode->nodeType != snStatementBlock) &&
			    (objType == 0 || !objType->IsInterface()))
			{
		...
		{
			if (!isExistingShared)
				RegisterScriptFunction(funcNode ? funcNode : node, file, objType, isInterface,
						       isGlobalFunction, ns, false, false, name, returnType, paramNames,
						       paramTypes, paramModifiers, defaultArgs, funcTraits, 0);
			else
			{
				...
				{
					asCScriptFunction *func = engine->scriptFunctions[objType->methods[n]];
					if (func->name == name && func->IsSignatureExceptNameEqual(
								      returnType, paramTypes, paramModifiers, objType,
								      funcTraits.GetTrait(asTRAIT_CONST)))
					{
						found = true;
						break;
...
int asCBuilder::RegisterImportedFunction(int importID, asCScriptNode *node, asCScriptCode *file, asSNameSpace *ns)
{
	...
	GetParsedFunctionDetails(node->firstChild, file, 0, name, returnType, parameterNames, parameterTypes,
				 inOutFlags, defaultArgs, funcTraits, ns);
	CheckNameConflict(name.AddressOf(), node, file, ns, false, false, false);
...
bool asCBuilder::FindObjectTypeOrMixinInNsHierarchy(const asCString &name, asSNameSpace *startNs, bool isExplicitNs,
						    asCScriptNode *errNode, asCScriptCode *script,
						    asCObjectType **outObjType, sMixinClass **outMixin)
{
	if (outObjType)
		*outObjType = 0;
	if (outMixin)
...
asSNameSpace *asCBuilder::GetNameSpaceFromNode(asCScriptNode *node, asCScriptCode *script, asSNameSpace *implicitNs,
					       asCScriptNode **next, asCObjectType **objType, bool *isExplicitNs)
{
	if (objType)
		*objType = 0;
	...
	while (sn)
	{
		...
		else
		{
			...
			bool ok = FindObjectTypeOrMixinInNsHierarchy(templateName, ns, scope != "", sn->next, script,
								     &templateType, 0);
	...
	asCTypeInfo *ti	 = 0;
	asSNameSpace *ns = GetNameSpaceByString(scope, implicitNs ? implicitNs : engine->nameSpaces[0], node, script,
						&ti, scope == originalScope);
	...
	if (scope != originalScope && ns == 0)
		ns = GetNameSpaceByString(originalScope, implicitNs ? implicitNs : engine->nameSpaces[0], node, script,
					  &ti, true);
...
asCDataType asCBuilder::CreateDataTypeFromNode(asCScriptNode *node, asCScriptCode *file,
					       ...
					       asCArray<asCDataType> *templSubTypes,
					       asCArray<asSNameSpace *> *scopeVisibleNamespaces)
{
		...
		while ((ns || parentType) && (!found || checkAmbiguousSymbols))
		{
			if (!visitedNamespaces.Exists(ns) || parentType)
			{
				...
				if (templSubTypes)
				{
					...
					for (asUINT subtypeIndex = 0; subtypeIndex < templSubTypes->GetLength();
					     subtypeIndex++)
					{
				...
				if (ti == 0 && currentType)
				{
					...
					if (currentType->flags & asOBJ_TEMPLATE)
					{
						for (asUINT subtypeIndex = 0;
						     subtypeIndex < currentType->templateSubTypes.GetLength();
						     subtypeIndex++)
						{
							asCTypeInfo *type =
							    currentType->templateSubTypes[subtypeIndex].GetTypeInfo();
							if (type && str == type->name)
							{
								ti = type;
								break;
					...
					if (ti == 0)
					{
						...
						if (ti)
						{
							...
							{
								asCString msg;
								msg.Format(TXT_AMBIGUOUS_SYMBOL_NAME_s,
									   str.AddressOf());
								WriteError(msg, file, node);
				...
				if (ti && (!found || checkAmbiguousSymbols) && canContinue)
				{
					...
					if (!module || (module->m_accessMask & ti->accessMask))
					{
						...
						else
						{
							...
							{
								ti = GetTemplateInstanceFromNode(
								    n, file, CastToObjectType(ti), implicitNamespace,
								    currentType, &n);
								if (ti == 0)
								{
									if (isValid)
									...
									// Return a dummy
									return asCDataType::CreatePrimitive(ttInt,
													    false);
								}
							}
							else if (n && n->next && n->next->nodeType == snDataType)
							{
								...
								{
									asCString msg;
									msg.Format(TXT_TYPE_s_NOT_TEMPLATE,
										   ti->name.AddressOf());
									WriteError(msg, file, n);
								}
								if (isValid)
					...
					else
					{
						...
						{
							asCString msg;
							msg.Format(TXT_TYPE_s_NOT_AVAILABLE_FOR_MODULE,
								   (const char *)str.AddressOf());
							WriteError(msg, file, n);
						}
			...
			if (!found || checkAmbiguousSymbols)
			{
				...
				ns = FindNextVisibleNamespace(visitedNamespaces, pendingNamespaces, parentNs,
