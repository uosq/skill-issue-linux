...
template <class T> inline T *asCArray<T>::AddressOf()
{
	return array;
}

template <class T> inline const T *asCArray<T>::AddressOf() const
{
	return array;
}

template <class T> asCArray<T>::asCArray(void) : buf()
{
	array	  = 0;
	length	  = 0;
	maxLength = 0;
...
template <class T> asCArray<T>::asCArray(const asCArray<T> &copy) : buf()
{
	array	  = 0;
	length	  = 0;
	maxLength = 0;
...
template <class T> asCArray<T>::asCArray(asUINT reserve) : buf()
{
	array	  = 0;
	length	  = 0;
	maxLength = 0;
...
template <class T> inline asUINT asCArray<T>::GetLength() const
{
	return length;
}

template <class T> inline const T &asCArray<T>::operator[](asUINT index) const
{
...
template <class T> inline T &asCArray<T>::operator[](asUINT index)
{
	asASSERT(index < length);
...
template <class T> inline T asCArray<T>::PopLast()
{
	asASSERT(length > 0);
...
template <class T> void asCArray<T>::Allocate(asUINT numElements, bool keepData)
{
...
template <class T> void asCArray<T>::AllocateNoConstruct(asUINT numElements, bool keepData)
{
	// We have 4 situations
	// 1. The previous array is 8 bytes or smaller and the new array is also 8 bytes or smaller
	// 2. The previous array is 8 bytes or smaller and the new array is larger than 8 bytes
...
template <class T> inline asUINT asCArray<T>::GetCapacity() const
{
	return maxLength;
}

template <class T> inline void asCArray<T>::SetLengthNoAllocate(asUINT numElements)
{
	asASSERT(numElements <= maxLength);
	length = numElements;
}

template <class T> bool asCArray<T>::SetLength(asUINT numElements)
{
...
template <class T> bool asCArray<T>::SetLengthNoConstruct(asUINT numElements)
{
	if (numElements > maxLength)
	{
...
template <class T> void asCArray<T>::Copy(const T *data, asUINT count)
{
	if (maxLength < count)
	{
...
template <class T> void asCArray<T>::SwapWith(asCArray<T> &other)
{
	T *tmpArray	    = array;
...
template <class T> bool asCArray<T>::Concatenate(const asCArray<T> &other)
{
...
template <class T> void asCArray<T>::Concatenate(T *other, unsigned int count)
{
	for (unsigned int c = 0; c < count; c++)
		PushLast(other[c]);
}

template <class T> bool asCArray<T>::Exists(const T &e) const
{
	return IndexOf(e) == -1 ? false : true;
}

template <class T> int asCArray<T>::IndexOf(const T &e) const
{
	for (asUINT n = 0; n < length; n++)
		if (array[n] == e)
...
template <class T> void asCArray<T>::RemoveIndex(asUINT index)
{
	if (index < length)
	{
...
template <class T> void asCArray<T>::RemoveIndexUnordered(asUINT index)
{
	if (index == length - 1)
		PopLast();
