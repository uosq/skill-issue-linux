...
int asCCompiler::CompileDefaultCopyConstructor(asCBuilder *in_builder, asCScriptCode *in_script, asCScriptNode *in_node,
					       asCScriptFunction *in_outFunc, sClassDeclaration *in_classDecl)
{
	...
	if (outFunc->objectType->derivedFrom)
	{
		if (outFunc->objectType->derivedFrom->beh.copyconstruct)
		{
			...
			asCScriptFunction *copyfunc =
			    engine->scriptFunctions[outFunc->objectType->derivedFrom->beh.copyconstruct];
			if (copyfunc->parameterTypes[0].IsObjectHandle())
			{
				...
				ctx.bc.Call(asBC_CALL, outFunc->objectType->derivedFrom->beh.copyconstruct,
					    2 * AS_PTR_SIZE);
		...
		else
		{
			if (outFunc->objectType->derivedFrom->beh.construct == 0 ||
			    outFunc->objectType->derivedFrom->beh.copy == 0)
	...
	byteCode.DebugOutput(("__" + outFunc->objectType->name + "_" + outFunc->name + "__copyconstr.txt").AddressOf(),
			     in_outFunc);
#endif
	...
	for (asUINT n = 0; n < outFunc->objectType->properties.GetLength(); n++)
	{
		...
		if (declNode)
		{
			...
			ctx.bc.InstrSHORT_DW(
			    asBC_ADDSi, (short)prop->byteOffset,
		...
		if (declNode)
		{
			      ...
			      !(prop->type.GetTypeInfo()->flags & asOBJ_POD))))
			{
				...
				if (prop->type.GetFuncDef())
					str.Format(TXT_NO_DEFAULT_CONSTRUCTOR_FOR_s,
						   prop->type.GetFuncDef()->GetName());
				else
					str.Format(TXT_NO_DEFAULT_CONSTRUCTOR_FOR_s,
						   prop->type.GetTypeInfo()->GetName());
				Error(str, declNode);
			}
#else
			...
			CompileInitialization(initNode, &bcInit, prop->type, declNode, prop->byteOffset, &constantValue,
					      asVGM_MEMBER);
		...
		if (engine->ep.memberInitMode == 1)
		{
			...
			if (!m_isConstructorCalled && outFunc->objectType->derivedFrom)
			{
				if (outFunc->objectType->derivedFrom->beh.construct)
				{
					...
					tmpBC.Call(asBC_CALL, outFunc->objectType->derivedFrom->beh.construct,
						   AS_PTR_SIZE);
...
int asCCompiler::CallCopyConstructor(asCDataType &type, int offset, bool isObjectOnHeap, asCExprContext *ctx,
				     asCExprContext *arg, asCScriptNode *node, EVarGlobOrMem isVarGlobOrMem,
				     bool derefDest)
{
	if (!type.IsObject())
		return 0;
	...
	if (type.GetTypeInfo()->flags & asOBJ_REF)
	{
		...
		if (func > 0)
		{
			if (isVarGlobOrMem == asVGM_VARIABLE)
			{
				...
				PerformFunctionCall(func, ctx, false, &args, CastToObjectType(type.GetTypeInfo()), true,
						    offset);
			...
			else
			{
				...
				if (isVarGlobOrMem == asVGM_GLOBAL)
				{
					...
					ctx->bc.InstrPTR(asBC_PGA,
							 engine->globalProperties[offset]->GetAddressOfValue());
				}
				else
				{
...
int asCCompiler::CallDefaultConstructor(const asCDataType &type, int offset, bool isObjectOnHeap, asCByteCode *bc,
					asCScriptNode *node, EVarGlobOrMem isVarGlobOrMem, bool derefDest)
{
			...
			if (!isObjectOnHeap)
			{
				if (isVarGlobOrMem == asVGM_VARIABLE)
				{
					...
					if (func)
					{
						...
						PerformFunctionCall(func, &ctxCall, false, 0,
				...
				else if (isVarGlobOrMem == asVGM_MEMBER)
				{
					...
					if (func)
					{
						...
						bc->Instr(asBC_RDSPtr);
						bc->InstrSHORT_DW(asBC_ADDSi, (short)offset,
								  engine->GetTypeIdFromDataType(asCDataType::CreateType(
								      outFunc->objectType, false)));
				       ...
				       sGlobalVariableDescription *in_gvar, asCScriptFunction *in_outFunc)
{
	...
	if (CompileInitialization(in_node, &ctx.bc, in_gvar->datatype, in_gvar->declaredAtNode, in_gvar->index,
				  &constantValue, asVGM_GLOBAL, preCompiled ? &compiledCtx : 0))
	{
...
void asCCompiler::CompileInitAsCopy(asCDataType &dt, int offset, asCExprContext *ctx, asCExprContext *arg,
				    asCScriptNode *node, bool derefDestination, EVarGlobOrMem isVarGlobOrMem)
{
	...
	if (!dt.IsObjectHandle() && ot && (ot->beh.copyconstruct || ot->beh.copyfactory))
	{
		...
		asCScriptFunction *func = ot->beh.copyconstruct ? engine->scriptFunctions[ot->beh.copyconstruct]
								: engine->scriptFunctions[ot->beh.copyfactory];
		...
		int r =
		    CallCopyConstructor(dt, offset, isObjectOnHeap, ctx, arg, node, isVarGlobOrMem, derefDestination);
	...
	else
	{
		...
		asCByteCode tmpBC(engine);
		int r =
		    CallDefaultConstructor(dt, offset, isObjectOnHeap, &tmpBC, node, isVarGlobOrMem, derefDestination);
		if (r < 0)
		{
				 ...
				 int refType, bool isMakingCopy)
{
	...
	if (paramType->GetTokenType() == ttQuestion)
	{
		...
		param.MakeHandle(ctx->type.isExplicitHandle || ctx->type.IsNullConstant() ||
				 CastToFuncdefType(ctx->type.dataType.GetTypeInfo()));
	...
	if (isFunction && dt.IsReference())
	{
			...
			else
			{
				...
				if (!isMakingCopy)
				{
					...
					if (dt.IsObjectHandle() && !ctx->type.dataType.IsObjectHandle() &&
					    !ctx->IsAnonymousInitList() && ctx->type.dataType.SupportHandles())
					{
					    ...
					    !(param.IsReadOnly() && (ctx->type.isVariable || ctx->type.isRefSafe)))
					{
						     ...
						     !(ctx->type.dataType.GetTypeInfo()->flags & asOBJ_SCOPED)))
						{
							...
							ctx->bc.InstrWORD(asBC_PSF, (asWORD)offset);
							if (ctx->type.dataType.IsFuncdef())
								ctx->bc.InstrPTR(asBC_REFCPY,
										 &engine->functionBehaviours);
							else
								ctx->bc.InstrPTR(asBC_REFCPY,
										 ctx->type.dataType.GetTypeInfo());
							ctx->bc.Instr(asBC_PopPtr);
							ctx->bc.InstrWORD(asBC_PSF, (asWORD)offset);
					...
					if (paramType->GetTokenType() == ttQuestion && !param.IsObjectHandle() &&
					    ctx->type.isVariable)
					{
		...
		else if (refType == asTM_INOUTREF)
		{
			...
			if ((!ctx->type.isVariable && ctx->type.isConstant))
			{
				...
				if (param.IsReadOnly() &&
				    (ctx->type.dataType.SupportHandles() || engine->ep.allowUnsafeReferences))
				{
			...
			if ((ctx->type.dataType.IsObject() || ctx->type.dataType.IsFuncdef()) &&
			    ctx->type.dataType.IsReference() && !param.IsObjectHandle())
				Dereference(ctx, true);
	...
	else
	{
		...
		else
		{
			...
			if ((dt.IsObject() || dt.IsFuncdef()) && !dt.IsNullHandle() && !dt.IsReference())
			{
				...
				sVariable *var =
				    ctx->type.isVariable ? variables->GetVariableByOffset(ctx->type.stackOffset) : 0;
				if (!(!isFunction && isMakingCopy && ctx->type.dataType.IsObjectHandle() &&
				      ctx->type.isVariable) ||
...
int asCCompiler::PrepareFunctionCall(int funcId, asCByteCode *bc, asCArray<asCExprContext *> &args)
{
	...
	asASSERT((descr->IsVariadic() && args.GetLength() >= descr->parameterTypes.GetLength() - 1) ||
		 descr->parameterTypes.GetLength() == args.GetLength());
	...
	for (int n = (int)args.GetLength() - 1; n >= 0; n--)
	{
		...
		int r = PrepareArgument2(&e, args[n], &descr->parameterTypes[realParamIdx], true,
					 descr->inOutFlags[realParamIdx], makingCopy);
...
void asCCompiler::MoveArgsToStack(int funcId, asCByteCode *bc, asCArray<asCExprContext *> &args, bool addOneToOffset)
{
	...
	for (asUINT n = 0; n < args.GetLength(); n++)
	{
		...
		{
			if ((descr->parameterTypes[realParamIdx].IsObject() ||
			     descr->parameterTypes[realParamIdx].IsFuncdef()) &&
			    !descr->parameterTypes[realParamIdx].IsObjectHandle())
			{
			...
			else if (descr->inOutFlags[realParamIdx] != asTM_INOUTREF)
			{
				...
				if (!args[n]->type.isRefSafe)
				{
					...
					else
					{
						...
						sVariable *var =
						    variables->GetVariableByOffset(args[n]->type.stackOffset);
		...
		else if (descr->parameterTypes[realParamIdx].IsObject() ||
			 descr->parameterTypes[realParamIdx].IsFuncdef())
		{
...
asUINT asCCompiler::MatchFunctions(asCArray<int> &funcs, asCArray<asCExprContext *> &args, asCScriptNode *node,
				   ...
				   bool isConstMethod, bool silent, bool allowObjectConstruct, const asCString &scope)
{
			...
			for (asUINT i = 0; i < matchingFuncs.GetLength(); ++i)
			{
				...
				if (matchedAll)
				{
					for (n = 0; n < namedArgs->GetLength(); ++n)
					{
						...
						// Add to the cost
						cost =
						    MatchArgument(desc, named.ctx, named.match, allowObjectConstruct);
						if (cost == asUINT(-1))
						{
							matchedAll = false;
							break;
...
{
	// Get the data type
	asCDataType type = builder->CreateDataTypeFromNode(decl->firstChild, script, outFunc->nameSpace, false,
							   outFunc->objectType, true, 0, 0, &m_namespaceVisibility);
	...
	while (node)
	{
		...
		else
		{
			...
			asQWORD constantValue = 0;
			if (CompileInitialization(node, bc, type, varNode, offset, &constantValue, asVGM_VARIABLE,
...
bool asCCompiler::CompileInitialization(asCScriptNode *node, asCByteCode *bc, const asCDataType &type,
					asCScriptNode *errNode, int offset, asQWORD *constantValue,
					EVarGlobOrMem isVarGlobOrMem, asCExprContext *preCompiled)
{
	...
	if (node && node->nodeType == snArgList)
	{
		...
		else
		{
			...
			if (CompileArgumentList(node, args, namedArgs) >= 0)
			{
				...
				{
					// Add the default values for arguments not explicitly supplied
					int r = CompileDefaultAndNamedArgs(
					    node, args, funcs[0], CastToObjectType(type.GetTypeInfo()), &namedArgs);
					...
					if (r == asSUCCESS)
					{
						...
						{
							if (isVarGlobOrMem == asVGM_VARIABLE)
								MakeFunctionCall(&ctx, funcs[0], 0, args, node, true,
										 offset);
							else
							{
								...
								if (isVarGlobOrMem == asVGM_GLOBAL)
								{
									// Store the returned handle in the global variable
									ctx.bc.InstrPTR(asBC_PGA,
											engine->globalProperties[offset]
											    ->GetAddressOfValue());
								}
								else
								{
								...
								if (type.GetTypeInfo()->flags & asOBJ_SCOPED)
								{
									...
									// variable rather than copy it as there is no AddRef behaviour
									ctx.bc.InstrSHORT_DW(
									    asBC_COPY, AS_PTR_SIZE,
									    asTYPEID_OBJHANDLE |
										engine->GetTypeIdFromDataType(type));

									// Clear the local variable so the reference isn't released
									ctx.bc.InstrSHORT(asBC_ClrVPtr,
											  (short)ctx.type.stackOffset);
								}
						...
						else
						{
							...
							else
							{
								...
								onHeap = !(type.IsObject() || type.IsFuncdef()) ||
									 type.IsReference() ||
	...
	else if (node && node->nodeType == snAssignment)
	{
		...
		return CompileInitializationWithAssignment(bc, type, errNode, offset, constantValue, isVarGlobOrMem,
							   node, expr);
	}
	else
	{
...
bool asCCompiler::CompileInitializationWithAssignment(asCByteCode *bc, const asCDataType &type, asCScriptNode *errNode,
						      int offset, asQWORD *constantValue, EVarGlobOrMem isVarGlobOrMem,
						      asCScriptNode *rnode, asCExprContext *rexpr)
{
	bool isConstantExpression = false;
	...
	if ((isVarGlobOrMem == asVGM_MEMBER || isVarGlobOrMem == asVGM_GLOBAL) && rexpr->type.isTemporary &&
	    type.GetTypeInfo() == rexpr->type.dataType.GetTypeInfo() && type.IsObject() &&
	    (type.IsObjectHandle() || type.SupportHandles()))
	{
		...
		if (type.GetTypeInfo()->flags & asOBJ_SCOPED)
		{
			// For scoped typed we must move the reference from the local
			// variable rather than copy it as there is no AddRef behaviour
			bc->InstrSHORT_DW(asBC_COPY, AS_PTR_SIZE,
					  asTYPEID_OBJHANDLE | engine->GetTypeIdFromDataType(type));
			      ...
			      (lexpr.type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE))))
			{
				...
				if (lexpr.type.dataType.IsHandleToAsHandleType())
				{
					...
					if (!rexpr->type.isExplicitHandle && !rexpr->type.IsNullConstant())
					{
						...
						if (!rexpr->type.dataType.IsObjectHandle() &&
						    rexpr->type.dataType.SupportHandles() &&
						    rexpr->type.dataType.GetTypeInfo() &&
						    !(rexpr->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE))
						{
							asCDataType to = rexpr->type.dataType;
							to.MakeHandle(true);
							to.MakeReference(true);
							to.MakeHandleToConst(rexpr->type.dataType.IsReadOnly());
							ImplicitConversion(rexpr, to, rnode, asIC_IMPLICIT_CONV, true,
									   false);

							asASSERT(rexpr->type.dataType.IsObjectHandle());
						}
						else if (rexpr->type.dataType.GetTypeInfo() &&
							 rexpr->type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE)
						{
				...
				assigned =
				    CompileOverloadedDualOperator(rnode, &lexpr, rexpr, false, &ctx, useHndlAssign);
				if (assigned)
				{
...
void asCCompiler::CompileInitList(asCExprValue *var, asCScriptNode *node, asCByteCode *bc, int isVarGlobOrMem)
{
	...
	if (var->dataType.GetTypeInfo() == 0 || var->dataType.GetBehaviour() == 0 ||
	...
	{
		asCString msg;
		msg.Format(TXT_PREV_ERROR_WHILE_COMP_LIST_FOR_TYPE_s,
			   var->dataType.Format(outFunc->nameSpace).AddressOf());
		Error(msg, node);
	}
	...
	else
	{
		if (var->dataType.GetTypeInfo()->GetFlags() & asOBJ_REF)
		{
			...
			if (isVarGlobOrMem == 1)
			{
				// Store the returned handle in the global variable
				ctx.bc.InstrPTR(asBC_PGA,
						engine->globalProperties[var->stackOffset]->GetAddressOfValue());
			}
			else
			{
				// Store the returned handle in the member
		...
		else
		{
			...
			{
				onHeap = !(var->dataType.IsObject() || var->dataType.IsFuncdef()) ||
					 var->dataType.IsReference() ||
					 (var->dataType.GetTypeInfo()->flags & asOBJ_REF);
				if (onHeap)
				{
					ctx.bc.InstrSHORT(asBC_PSF, 0);
					ctx.bc.Instr(asBC_RDSPtr);
					ctx.bc.InstrSHORT_DW(asBC_ADDSi, (short)var->stackOffset,
							     engine->GetTypeIdFromDataType(
								 asCDataType::CreateType(outFunc->objectType, false)));
				}
			}
					...
					int &elementsInSubList)
{
	...
	else if (patternNode->type == asLPT_REPEAT || patternNode->type == asLPT_REPEAT_SAME)
	{
		...
		while (valueNode)
		{
			...
			int r = CompileInitListElement(patternNode, valueNode, bufferTypeId, bufferVar, bufferSize,
						       ctx.bc, elementsInSubSubList);
			if (r < 0)
				return r;
	...
	else if (patternNode->type == asLPT_TYPE)
	{
		...
		if (valueNode->nodeType == snAssignment || valueNode->nodeType == snInitList)
		{
			...
			if (lctx.type.dataType.IsNullHandle())
			{
				...
				asASSERT(reinterpret_cast<asSListPatternDataTypeNode *>(patternNode)
					     ->dataType.GetTokenType() == ttQuestion);
			}
			else
			{
				asCExprContext ctx(engine);
			...
			else
			{
				...
				else if (dt.GetTypeInfo() && dt.GetTypeInfo()->flags & asOBJ_VALUE)
				{
					...
					else if (func)
					{
						...
						asCExprContext ctx(engine);
						PerformFunctionCall(func, &ctx, false, 0,
								    CastToObjectType(dt.GetTypeInfo()));
						bcInit.AddCode(&ctx.bc);
					}
				}
				else if (!dt.IsObjectHandle() && dt.GetTypeInfo() &&
					 dt.GetTypeInfo()->flags & asOBJ_REF)
				{
					...
					{
						asCExprContext rctx(engine);
						PerformFunctionCall(func, &rctx, false, 0,
								    CastToObjectType(dt.GetTypeInfo()));
						...
						asCExprContext ctx(engine);
						DoAssignment(&ctx, &lctx, &rctx, valueNode, valueNode, ttAssignment,
							     valueNode);
...
void asCCompiler::CompileIfStatement(asCScriptNode *inode, bool *hasReturn, asCByteCode *bc)
{
	...
	if (r == 0)
	{
		...
		if (engine->ep.boolConversionMode == 1)
			ImplicitConversion(&expr, asCDataType::CreatePrimitive(ttBool, false), inode,
		...
		// else, allow value types to be converted to bool using 'bool opImplConv()'
			 ...
			 (expr.type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE))
			ImplicitConversion(&expr, asCDataType::CreatePrimitive(ttBool, false), inode,
					   asIC_IMPLICIT_CONV);
	...
	if (m_isConstructor && initializedProperties1 != initializedProperties2)
	{
		...
		for (asUINT n = origInitializedProperties.GetLength(); n < initializedProperties1.GetLength(); n++)
		{
			...
			{
				asCString str;
				str.Format(TXT_BOTH_CONDITIONS_MUST_INIT_MEMBER_s,
					   initializedProperties1[n]->name.AddressOf());
				Error(str, inode);
			}
		}
		for (asUINT n = origInitializedProperties.GetLength(); n < initializedProperties2.GetLength(); n++)
		{
			if (!initializedProperties1.Exists(initializedProperties2[n]))
			{
				asCString str;
				str.Format(TXT_BOTH_CONDITIONS_MUST_INIT_MEMBER_s,
					   initializedProperties2[n]->name.AddressOf());
				Error(str, inode);
			}
		}
...
void asCCompiler::CompileForStatement(asCScriptNode *fnode, asCByteCode *bc)
{
	...
	if (second->firstChild)
	{
		...
		if (r >= 0)
		{
			...
			// If turned on, allow the compiler to use either 'bool opImplConv()' or 'bool opConv()' on the type
			if (engine->ep.boolConversionMode == 1)
				ImplicitConversion(&expr, asCDataType::CreatePrimitive(ttBool, false),
						   second->firstChild, asIC_EXPLICIT_VAL_CAST);
			// else, allow value types to be converted to bool using 'bool opImplConv()'
			else if (expr.type.dataType.GetTypeInfo() &&
				 (expr.type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE))
				ImplicitConversion(&expr, asCDataType::CreatePrimitive(ttBool, false),
						   second->firstChild, asIC_IMPLICIT_CONV);

			if (!expr.type.dataType.IsEqualExceptRefAndConst(asCDataType::CreatePrimitive(ttBool, true)))
			{
				asCString str;
				str.Format(TXT_EXPR_MUST_BE_BOOL_s,
...
int asCCompiler::DeclareVariable(const asCString &name, const asCDataType &type, int offset, asCByteCode *bc,
				 asCScriptNode *node)
{
...
void asCCompiler::CompileForEachStatement(asCScriptNode *node, asCByteCode *bc)
{
	...
	while (rangeNode->nodeType != snAssignment)
	{
		...
		asCDataType itemDt =
		    builder->CreateDataTypeFromNode(rangeNode, script, outFunc->nameSpace, false, outFunc->objectType,
						    true, 0, 0, &m_namespaceVisibility);
	...
	if (rangeDt.SupportHandles())
		rangeDt.MakeHandle(true);
	int rangeOffset = AllocateVariable(rangeDt, false);
	if (DeclareVariable("", rangeDt, rangeOffset, &rangeExpr.bc, node) < 0)
		return;

	CompileInitializationWithAssignment(&rangeExpr.bc, rangeDt, rangeNode, rangeOffset, 0, asVGM_VARIABLE,
					    rangeNode, &rangeExpr);
	...
	if (iterTid)
	{
		...
		for (asUINT i = 0; i < funcs.GetLength(); i++)
		{
			...
			asDWORD flags;
			int paramTid;
			if (f->GetParamCount() == 1 && f->GetReturnTypeId(&flags) == asTYPEID_BOOL &&
			    !(flags & asTM_INOUTREF) && f->GetParam(0, &paramTid) >= 0 && paramTid == iterTid)
			{
	...
	if (iterTid)
	{
		...
		for (asUINT i = 0; i < funcs.GetLength(); i++)
		{
			asIScriptFunction *f = engine->scriptFunctions[funcs[i]];
			int paramTid;
			if (f->GetParamCount() == 1 && f->GetReturnTypeId() == iterTid &&
			    f->GetParam(0, &paramTid) >= 0 && paramTid == iterTid)
			{
	...
	if (iterTid)
	{
		...
		if (itemCount > 1 || opForValueNIds.GetLength() == 0)
		{
			for (asUINT i = 0; i < itemCount; ++i)
			{
				...
				if (itemCount == 1)
					builder->GetObjectMethodDescriptions(
					    VALUE_NAME, CastToObjectType(rangeTypeInfo), funcs, isConstRange);
				if (funcs.GetLength() == 0)
				{
					methodName.Format("%s%d", VALUE_NAME, (int)i);
					builder->GetObjectMethodDescriptions(methodName.AddressOf(),
									     CastToObjectType(rangeTypeInfo), funcs,
									     isConstRange);
				}
				...
				for (asUINT j = 0; j < funcs.GetLength(); j++)
				{
					...
					int paramTid;
					if (f->GetParamCount() == 1 && f->GetParam(0, &paramTid) >= 0 &&
					    paramTid == iterTid)
					{
	...
	if (!(opForBeginId && opForEndId && opForNextId && opForValueNIds.GetLength() == itemCount))
	{
		...
		else
		{
			...
			{
				str.Format(TXT_MISSING_OR_INVALID_DEFINITON_OF_s,
					   (asCString(VALUE_NAME) + "#").AddressOf());
				Information(str, node->lastChild);
			}
		}
	...
	asCExprContext opForBeginExpr(engine);
	{
		...
		asCArray<asCExprContext *> args;
		int r =
		    MakeFunctionCall(&opForBeginExpr, opForBeginId,
				     CastToObjectType(rangeExpr.type.dataType.GetTypeInfo()), args, node->firstChild);
		UNUSED_VAR(r);
		asASSERT(r >= 0);
	}
	CompileInitializationWithAssignment(&initBC, iterDt, node->firstChild, iterOffset, NULL, asVGM_VARIABLE,
					    rangeNode, &opForBeginExpr);
	...
	asCExprContext opForEndExpr(engine);
	{
		...
		if (!iterDt.IsPrimitive())
		{
			...
			if (iterDt.IsObjectHandle())
				arg.type.dataType.MakeReference(true);
		}
		arg.type.isLValue = true;
		arg.exprNode	  = node;
		args.PushLast(&arg);

		int r = MakeFunctionCall(&opForEndExpr, opForEndId,
					 CastToObjectType(rangeExpr.type.dataType.GetTypeInfo()), args, node);
		UNUSED_VAR(r);
		asASSERT(r >= 0);
	...
	asCExprContext next(engine);
	{
		...
		int r = MakeFunctionCall(&opForNextExpr, opForNextId,
					 CastToObjectType(rangeExpr.type.dataType.GetTypeInfo()), args, node);
		UNUSED_VAR(r);
		asASSERT(r >= 0);
		...
		if (!iterDt.IsPrimitive())
		{
			...
			if (iterDt.IsObjectHandle())
				lhs.type.dataType.MakeReference(true);
		}
		if (iterDt.IsObjectHandle())
		...
		DoAssignment(&next, &lhs, &opForNextExpr, node, node, ttAssignment, node);
	...
	for (asUINT i = 0; i < itemCount; ++i)
	{
		...
		args.PushLast(&arg);
		int r =
		    MakeFunctionCall(&opForValueNExpr, opForValueNIds[i],
				     CastToObjectType(rangeExpr.type.dataType.GetTypeInfo()), args, node->firstChild);
		UNUSED_VAR(r);
		asASSERT(r >= 0);

		CompileInitializationWithAssignment(&itemNBC, itemDt, itemNode->prev, itemOffset, NULL, asVGM_VARIABLE,
						    rangeNode, &opForValueNExpr);
...
void asCCompiler::CompileBreakStatement(asCScriptNode *node, asCByteCode *bc)
{
	...
	while (!vs->isBreakScope)
	{
		for (int n = (int)vs->variables.GetLength() - 1; n >= 0; n--)
			CallDestructor(vs->variables[n]->type, vs->variables[n]->stackOffset, vs->variables[n]->onHeap,
				       bc);
...
void asCCompiler::PrepareTemporaryVariable(asCScriptNode *node, asCExprContext *ctx, bool forceOnHeap)
{
	...
	if (ctx->type.isTemporary && ctx->type.isVariable && !(forceOnHeap && !IsVariableOnHeap(ctx->type.stackOffset)))
	{
	...
	if (rnode->firstChild)
	{
		...
		else // if( !v->type.IsReference() )
		{
			...
			if (v->type.IsPrimitive())
			{
				...
				{
					asCString str;
					str.Format(TXT_NO_CONVERSION_s_TO_s,
						   expr.type.dataType.Format(outFunc->nameSpace).AddressOf(),
						   v->type.Format(outFunc->nameSpace).AddressOf());
					Error(str, rnode);
					return;
			...
			else if (v->type.IsObject() || v->type.IsFuncdef())
			{
				...
				if (outFunc->DoesReturnOnStack())
				{
					...
					{
						ImplicitConversion(&expr, v->type, rnode->firstChild,
								   asIC_IMPLICIT_CONV);
						if (!v->type.IsEqualExceptRefAndConst(expr.type.dataType))
						{
							asCString str;
	...
	while (vs)
	{
		for (int n = (int)vs->variables.GetLength() - 1; n >= 0; n--)
			if (vs->variables[n]->stackOffset > 0)
				CallDestructor(vs->variables[n]->type, vs->variables[n]->stackOffset,
					       vs->variables[n]->onHeap, bc);
	...
	for (unsigned int n = 0; n < funcs.GetLength(); n++)
	{
		...
		if (func->objectType && (func->objectType->flags & asOBJ_TEMPLATE))
		{
			...
			for (unsigned int p = 0; p < func->GetParamCount(); p++)
			{
				...
				{
					asCString msg;
					msg.Format(TXT_WHERE_s_IS_s, ti->GetName(),
						   ti->GetFuncdefSignature()->GetDeclaration());
...
void asCCompiler::PrepareForAssignment(asCDataType *lvalue, asCExprContext *rctx, asCScriptNode *node, bool toTemporary,
				       asCExprContext *lvalueExpr)
{
	...
	else if (!lvalue->isExplicitHandle)
	{
		...
		*lvalue = ctx.type;
		...
		if (beh && beh->copy && beh->copy != engine->scriptTypeBehaviours.beh.copy)
		{
			...
			PerformFunctionCall(beh->copy, &res, false, 0,
					    CastToObjectType(lvalue->dataType.GetTypeInfo()));
		...
		else
		{
			...
			{
				asCString msg;
				msg.Format(TXT_NO_DEFAULT_COPY_OP_FOR_s,
					   lvalue->dataType.GetTypeInfo()->name.AddressOf());
				Error(msg, node);
				return -1;
			}
				 ...
				 bool generateCode)
{
	...
	if (ops.GetLength() == 1)
	{
		...
		if (generateCode)
		{
			    ...
			    !(ctx->type.dataType.GetTypeInfo()->flags & asOBJ_NOHANDLE))
			{
				...
				asCArray<asCExprContext *> args;
				MakeFunctionCall(ctx, ops[0], CastToObjectType(ctx->type.dataType.GetTypeInfo()), args,
						 node);
				ctx->bc.Instr(asBC_PopPtr);
	...
	if (!conversionDone && (ctx->type.dataType.GetTypeInfo()->flags & asOBJ_SCRIPT_OBJECT))
	{
		...
		if (!ctx->type.dataType.IsReference())
		{
			...
			toRef.MakeReference(true);
			ImplicitConversion(ctx, toRef, 0, isExplicit ? asIC_EXPLICIT_REF_CAST : asIC_IMPLICIT_CONV,
					   generateCode);
		}
						     ...
						     asCScriptNode *node, EImplicitConv convType, bool generateCode)
{
	...
	if (ctx->enumValue != "")
	{
		if (to.IsEnumType())
		{
			...
			asCDataType out;
			asDWORD value;
			if (builder->GetEnumValueFromType(CastToEnumType(to.GetTypeInfo()), ctx->enumValue.AddressOf(),
							  out, value))
			{
					     ...
					     EImplicitConv /*convType*/, bool generateCode)
{
	...
	while (argNode->nodeType != snStatementBlock)
	{
		...
		if (argNode->nodeType == snUndefined)
		{
			...
			{
				asCDataType dt = builder->CreateDataTypeFromNode(typeNode, script, outFunc->nameSpace,
										 false, outFunc->objectType, true, 0, 0,
										 &m_namespaceVisibility);
				...
				if (count >= funcDef->parameterTypes.GetLength() ||
				    funcDef->parameterTypes[count] != dt || funcDef->inOutFlags[count] != inOutFlag)
					return asCC_NO_CONV;
			}
	...
	if (generateCode)
	{
		...
		{
			name.Format(iterations == 0 ? "$%s$%d" : "$%s$%d(%d)",
				    m_globalVar ? m_globalVar->name.AddressOf() : outFunc->GetDeclaration(), numLambdas,
				    iterations);
		...
		asCScriptFunction *func = builder->RegisterLambda(ctx->exprNode, script, funcDef, name,
								  outFunc->nameSpace, outFunc->IsShared());
				       ...
				       EImplicitConv convType, bool generateCode, bool allowObjectConstruct)
{
	...
	{
		if (ctx->type.dataType.IsPrimitive())
			return ImplicitConvPrimitiveToObject(ctx, to, node, convType, generateCode,
						  ...
						  EImplicitConv convType, bool generateCode)
{
	...
	if (convType == asIC_EXPLICIT_VAL_CAST)
	{
		for (unsigned int n = 0; n < ot->methods.GetLength(); n++)
		{
			...
			if ((mthd->name == "opConv" || mthd->name == "opImplConv") &&
			    mthd->parameterTypes.GetLength() == 0 && mthd->returnType.IsPrimitive())
	...
	else
	{
		for (unsigned int n = 0; n < ot->methods.GetLength(); n++)
		{
			...
			asCScriptFunction *mthd = engine->scriptFunctions[ot->methods[n]];
			if (mthd->name == "opImplConv" && mthd->parameterTypes.GetLength() == 0 &&
		...
		else
		{
			...
			if (funcs.GetLength() == 1)
			{
				...
				if (generateCode)
				{
					// Allocate a temporary variable of the requested type
					int stackOffset = AllocateVariableNotIn(to, true, false, ctx);
					CallDefaultConstructor(to, stackOffset, IsVariableOnHeap(stackOffset), &ctx->bc,
							       node);
					...
					asCArray<asCExprContext *> args;
					args.PushLast(&arg);
					MakeFunctionCall(ctx, funcs[0],
							 CastToObjectType(ctx->type.dataType.GetTypeInfo()), args,
							 node);
			...
			else if (CastToObjectType(to.GetTypeInfo()))
			{
				...
				cost = asCC_TO_OBJECT_CONV +
				       MatchFunctions(funcs, args, node, 0, 0, 0, false, true, false);
				...
				if (funcs.GetLength() == 1)
				{
					if (generateCode)
					{
						...
						if (to.GetTypeInfo()->flags & asOBJ_VALUE)
						{
							...
							tempObj.stackOffset =
							    (short)AllocateVariable(tempObj.dataType, true);
						...
						if (to.GetTypeInfo()->flags & asOBJ_VALUE)
						{
							...
							{
								int offset = 0;
								asCScriptFunction *descr =
								    builder->GetFunctionDescription(funcs[0]);
								offset =
								    descr->parameterTypes[0].GetSizeOnStackDWords();
			...
			if (generateCode)
			{
				...
				if (func->parameterTypes[0].GetTokenType() == ttQuestion)
				{
					if (!ctx->type.isExplicitHandle)
					{
						...
						ImplicitConversion(ctx, toHandle, node, asIC_IMPLICIT_CONV, true,
								   false);
					     ...
					     EImplicitConv convType)
{
		    ...
		    from->type.dataType.IsUnsignedType() || from->type.dataType.IsIntegerType())
		{
				 ...
				 from->type.dataType.GetSizeInMemoryDWords() == 1)
			{
				...
				if (from->type.dataType.GetSizeInMemoryBytes() == 1)
				 ...
				 from->type.dataType.GetSizeInMemoryDWords() == 2)
			{
			...
			else
			{
				...
				asASSERT(
				    (from->type.dataType.GetTokenType() == ttInt || from->type.dataType.IsEnumType()) &&
		...
		else if (from->type.dataType.IsUnsignedType())
		{
			...
			if (from->type.dataType.GetSizeInMemoryBytes() == 1)
				from->type.SetConstantQW(asCDataType::CreatePrimitive(ttInt64, true),
							 from->type.GetConstantB());
			else if (from->type.dataType.GetSizeInMemoryBytes() == 2)
				from->type.SetConstantQW(asCDataType::CreatePrimitive(ttInt64, true),
							 from->type.GetConstantW());
			else if (from->type.dataType.GetSizeInMemoryBytes() == 4)
				from->type.SetConstantQW(asCDataType::CreatePrimitive(ttInt64, true),
							 from->type.GetConstantDW());
	...
	else if (to.IsUnsignedType() && to.GetSizeInMemoryDWords() == 1)
	{
		...
		else if (from->type.dataType.IsIntegerType())
		{
			...
			if ((from->type.dataType.GetSizeInMemoryBytes() > 4 &&
			     asINT64(from->type.GetConstantQW()) < 0) ||
			    (from->type.dataType.GetSizeInMemoryBytes() == 4 && int(from->type.GetConstantDW()) < 0) ||
			    (from->type.dataType.GetSizeInMemoryBytes() == 2 &&
			     asINT16(from->type.GetConstantW()) < 0) ||
			      ...
			      asCScriptNode *rexpr, eTokenType op, asCScriptNode *opNode)
{
	...
	if (engine->ep.memberInitMode == 1 && m_isConstructor)
	{
		...
		if (instr && instr->op == asBC_PSF && instr->wArg[0] == 0 && instr->next &&
		    instr->next->op == asBC_RDSPtr && instr->next->next && instr->next->next->op == asBC_ADDSi &&
		    ((instr->next->next->next && instr->next->next->next->op == asBC_RDSPtr &&
		      instr->next->next->next->next == 0) ||
		     (instr->next->next->next && instr->next->next->next->op == asBC_RDSPtr &&
		      instr->next->next->next->next && instr->next->next->next->next->op == asBC_PopRPtr &&
		      instr->next->next->next->next->next == 0) ||
		     (instr->next->next->next && instr->next->next->next->op == asBC_PopRPtr &&
		      instr->next->next->next->next == 0)))
		{
			// Which property?
			...
			if (!(prop && prop->isInherited))
			{
				...
				if (prop && m_initializedProperties.IndexOf(prop) < 0)
				{
					...
					asQWORD constantValue;
					CompileInitializationWithAssignment(&ctx->bc, prop->type, opNode, offset,
									    &constantValue, asVGM_MEMBER, rexpr, rctx);
	...
	else if (lctx->type.isExplicitHandle)
	{
		...
		{
			asCString str;
			str.Format(TXT_ILLEGAL_OPERATION_ON_s,
	...
	else // if( lctx->type.dataType.IsObject() )
	{
		    ...
		    !lctx->type.IsNullConstant())
		{
			...
			lctx->type.isLValue =
			    true; // Handle may not have been an lvalue, but the dereferenced object is
		}
		...
		bool simpleExpr = (lctx->type.dataType.GetTypeInfo() &&
				   lctx->type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE) &&
				  lctx->bc.IsSimpleExpression();
		...
		else
		{
			...
			if (rctx->type.dataType.IsReference() && (!(rctx->type.isVariable || rctx->type.isTemporary) ||
...
int asCCompiler::CompileAssignment(asCScriptNode *expr, asCExprContext *ctx)
{
	...
	if (lexpr->next)
	{
		...
		if (lr >= 0 && rr >= 0)
			return DoAssignment(ctx, &lctx, &rctx, lexpr, lexpr->next->next, lexpr->next->tokenType,
					    lexpr->next);
...
int asCCompiler::CompileCondition(asCScriptNode *expr, asCExprContext *ctx)
{
	...
	if (cexpr->next)
	{
		...
		// If turned on, allow the compiler to use either 'bool opImplConv()' or 'bool opConv()' on the type
		if (engine->ep.boolConversionMode == 1)
			ImplicitConversion(&e, asCDataType::CreatePrimitive(ttBool, false), cexpr,
					   asIC_EXPLICIT_VAL_CAST);
		// else, allow value types to be converted to bool using 'bool opImplConv()'
		...
		if (lr >= 0 && rr >= 0)
		{
			...
			if (le.type.dataType != re.type.dataType)
			{
				...
				asUINT costAtoB =
				    ImplicitConversion(&tmp, re.type.dataType, cexpr->next, asIC_IMPLICIT_CONV, false);
				if (!tmp.type.dataType.IsEqualExceptRef(re.type.dataType))
					costAtoB = 0xFFFFFFFF;
				tmp.type = re.type;
				tmp.type.dataType.MakeReference(false);
				asUINT costBtoA = ImplicitConversion(&tmp, le.type.dataType, cexpr->next->next,
								     asIC_IMPLICIT_CONV, false);
				...
				if (costAtoB < costBtoA && costAtoB != 0xFFFFFFFF)
				{
					...
					ImplicitConversion(&le, re.type.dataType, cexpr->next, asIC_IMPLICIT_CONV,
							   true);
				}
				else if (costAtoB > costBtoA && costBtoA != 0xFFFFFFFF)
				{
					...
					ImplicitConversion(&re, le.type.dataType, cexpr->next->next, asIC_IMPLICIT_CONV,
							   true);
				}
			...
			else if (re.type.isConstant && re.type.GetConstantData() == 0 &&
				 re.type.dataType.IsIntegerType())
			{
				asCDataType to = le.type.dataType;
				to.MakeReference(false);
				to.MakeReadOnly(true);
			...
			else
			{
				...
				if (le.type.isLValue && re.type.isLValue && le.deferredParams.GetLength() == 0 &&
				    re.deferredParams.GetLength() == 0 && !le.type.isTemporary &&
				    !re.type.isTemporary && le.type.dataType == re.type.dataType)
				{
					// Put the code for the condition expression on the output
					MergeExprBytecode(ctx, &e);
					...
					{
						asASSERT(re.type.dataType.IsReference() &&
							 re.type.dataType.IsObject() &&
					...
					ctx->type.isRefToLocal = le.type.isVariable || le.type.isRefToLocal ||
								 re.type.isVariable || re.type.isRefToLocal;
				}
				else
				{
					...
					CallDefaultConstructor(temp.dataType, offset, IsVariableOnHeap(offset),
							       &ctx->bc, expr);
	...
	if (node->firstChild)
	{
		if (node->firstChild->nodeType == snDataType)
		{
			...
			asCDataType dt = builder->CreateDataTypeFromNode(node->firstChild, script, outFunc->nameSpace,
									 false, 0, true, 0, 0, &m_namespaceVisibility);
...
bool asCCompiler::isAmbiguousSymbol(const asCString &name, asCScriptNode *errNode, SYMBOLTYPE &currentType,
				    SYMBOLTYPE nextType)
{
...
//  SL_ERROR            = error
asCCompiler::SYMBOLTYPE asCCompiler::SymbolLookup(const asCString &name, const asCString &scope, asCObjectType *objType,
						  asCExprContext *outResult, asCScriptNode *errNode)
{
	asASSERT(outResult);
	...
	while (!objType && currNamespace)
	{
		if (!visitedNamespaces.Exists(currNamespace))
		{
			...
			if (ns)
			{
				...
				if (scopeType)
				{
					...
					{
						SYMBOLTYPE r =
						    SymbolLookupMember(name, CastToObjectType(scopeType), outResult);
						if (r != 0)
						{
							if (!checkAmbiguousSymbols)
								return r;
					...
					if (CastToEnumType(scopeType))
					{
						asDWORD value = 0;
						asCDataType dt;
						if (builder->GetEnumValueFromType(CastToEnumType(scopeType),
										  name.AddressOf(), dt, value))
						{
			...
			if (ns)
			{
				...
				if (resultSymbolType != SL_GLOBALPROPACCESS)
				{
					...
					asQWORD constantValue = 0;
					asCGlobalProperty *prop =
					    builder->GetGlobalProperty(name.AddressOf(), ns, &isCompiled,
								       &isPureConstant, &constantValue, &isAppProp);
					if (prop)
					{
			...
			if (ns && !engine->ep.requireEnumScope && resultSymbolType == SL_NOMATCH)
			{
				...
				if (e)
				{
					...
					if (e == 2)
					{
						...
						outResult->type.SetConstantDW(
						    asCDataType::CreatePrimitive(ttIdentifier, true), 0);
						outResult->symbolNamespace = ns;
					}
					else
					{
		...
		currNamespace = builder->FindNextVisibleNamespace(visitedNamespaces, pendingNamespaces, parentNamespace,
								  &checkAmbiguousSymbols);
				       ...
				       asCScriptNode *errNode, bool isOptional, asCObjectType *objType)
{
	    ...
	    symbolType == SL_GLOBALFUNC || symbolType == SL_ENUMVAL)
	{
		...
		if (symbolType == SL_GLOBALCONST || symbolType == SL_GLOBALVAR)
		{
			...
			bool isAppProp		= false;
			asQWORD constantValue	= 0;
			asCGlobalProperty *prop = builder->GetGlobalProperty(
			    name.AddressOf(), ns, &isCompiled, &isPureConstant, &constantValue, &isAppProp);
			asASSERT(prop);
			...
			else
			{
				...
				if (outFunc->IsShared())
				{
					...
					{
						asCString str;
						str.Format(TXT_SHARED_CANNOT_ACCESS_NON_SHARED_VAR_s,
							   prop->name.AddressOf());
						Error(str, errNode);
				...
				else
				{
					...
					if (!isAppProp && ((ctx->type.dataType.GetTypeInfo()->flags & asOBJ_VALUE) ||
							   !ctx->type.dataType.IsObjectHandle()))
					{
						ctx->bc.Instr(asBC_ChkRefS);
					}
		...
		if (symbolType == SL_ENUMVAL)
		{
			...
			asDWORD value = 0;
			builder->GetEnumValueFromType(CastToEnumType(lookupResult.type.dataType.GetTypeInfo()),
						      name.AddressOf(), dt, value);
	...
	else if (vnode->nodeType == snConstant)
	{
			 ...
			 vnode->tokenType == ttHeredocStringConstant)
		{
			...
			else
			{
				...
				{
					void *strPtr = const_cast<void *>(engine->stringFactory->GetStringConstant(
					    str.AddressOf(), (asUINT)str.GetLength()));
					if (strPtr == 0)
					{
...
int asCCompiler::CompileConversion(asCScriptNode *node, asCExprContext *ctx)
{
	...
	if (node->nodeType == snConstructCall || node->nodeType == snFunctionCall)
	{
		...
		to = builder->CreateDataTypeFromNode(node->firstChild, script, outFunc->nameSpace, false, 0, true, 0, 0,
						     &m_namespaceVisibility);
...
void asCCompiler::AfterFunctionCall(int funcID, asCArray<asCExprContext *> &args, asCExprContext *ctx, bool deferAll)
{
	...
	for (; n >= 0; n--)
	{
		...
		if ((descr->parameterTypes[realParamIdx].IsReference() &&
		     (descr->inOutFlags[realParamIdx] & asTM_OUTREF) && !args[realParamIdx]->isCleanArg) ||
		    (descr->parameterTypes[realParamIdx].IsObject() && deferAll &&
		     (descr->parameterTypes[realParamIdx].IsReference() ||
		      descr->parameterTypes[realParamIdx].IsObjectHandle())))
		{
			asASSERT(!(descr->parameterTypes[realParamIdx].IsReference() &&
				   (descr->inOutFlags[realParamIdx] == asTM_OUTREF) &&
				   !args[realParamIdx]->isCleanArg) ||
				 args[realParamIdx]->origExpr);

			// For &inout, only store the argument if it is for a temporary variable
			if (engine->ep.allowUnsafeReferences || descr->inOutFlags[realParamIdx] != asTM_INOUTREF ||
			    args[realParamIdx]->type.isTemporary)
			{
		...
		else if (outParam.argInOutFlags == asTM_OUTREF)
		{
			...
			if (IsLValue(expr->type) || expr->property_get || expr->property_set)
			{
				...
				else
				{
					...
					rctx.type.dataType.MakeReference(
					    IsVariableOnHeap(outParam.argType.stackOffset));
					if (expr->type.isExplicitHandle)
						rctx.type.isExplicitHandle = true;
				}

				asCExprContext o(engine);
				DoAssignment(&o, expr, &rctx, outParam.argNode, outParam.argNode, ttAssignment,
					     outParam.argNode);
			...
			else
			{
				...
				MergeExprBytecode(ctx, expr);
				if (!expr->IsVoidExpression() &&
				    (!expr->type.isConstant || expr->type.IsNullConstant()))
					ctx->bc.Instr(asBC_PopPtr);
	...
	if (CompileArgumentList(node->lastChild, args, namedArgs) >= 0)
	{
		...
		if (args.GetLength() == 0 && namedArgs.GetLength() == 0)
		{
			...
			if (beh && beh->construct == 0 && !(dt.GetTypeInfo()->flags & asOBJ_REF))
			{
				...
				CallDefaultConstructor(tempObj.dataType, tempObj.stackOffset,
						       IsVariableOnHeap(tempObj.stackOffset), &ctx->bc, node);
		...
		if (dt.IsFuncdef() && args.GetLength() == 1 && args[0]->methodName != "" && namedArgs.GetLength() == 0)
		{
			...
			else
			{
				...
				if (bestMethod)
				{
					...
					builder->GetFunctionDescriptions(DELEGATE_FACTORY, delegateFuncs,
									 engine->nameSpaces[0]);
		...
		else
		{
			...
			// Add the default values for arguments not explicitly supplied
			int r = CompileDefaultAndNamedArgs(node, args, funcs[0], CastToObjectType(dt.GetTypeInfo()),
							   &namedArgs);
...
int asCCompiler::InstantiateTemplateFunctions(asCArray<int> &funcs, asCScriptNode *types)
{
	...
	for (asUINT i = 0; i < funcs.GetLength(); i++)
	{
		...
		for (asUINT j = 0; j < numTypes; j++)
		{
			...
			dataTypes.PushLast(builder->CreateDataTypeFromNode(
			    types, script, func->nameSpace, func->objectType, 0, true, 0, 0, &m_namespaceVisibility));
			types = types->next;
		}
				     ...
				     bool objIsConst, const asCString &scope)
{
	    ...
	    symbolType == SL_GLOBALCONST || symbolType == SL_GLOBALVAR || symbolType == SL_ENUMVAL)
	{
		...
		else if (funcExpr.type.dataType.IsObject())
		{
			...
			builder->GetObjectMethodDescriptions(
			    "opCall", CastToObjectType(funcExpr.type.dataType.GetTypeInfo()), funcs, objIsConst);
		}
		else
		{
	...
	if (symbolType == SL_CLASSMETHOD)
	{
		...
		else
		{
			...
			builder->GetObjectMethodDescriptions(name.AddressOf(),
							     CastToObjectType(lookupResult.type.dataType.GetTypeInfo()),
							     funcs, objIsConst, scope, node, script);
	...
	if (symbolType == SL_GLOBALFUNC)
	{
		...
		if (datatypeNode->nodeType == snIdentifier)
			datatypeNode = datatypeNode->next;
	...
	if (symbolType == SL_CLASSTYPE || symbolType == SL_GLOBALTYPE)
	{
		bool isValid = false;
		asCDataType dt =
		    builder->CreateDataTypeFromNode(node->firstChild, script, outFunc->nameSpace, false,
						    outFunc->objectType, false, &isValid, 0, &m_namespaceVisibility);
		if (isValid)
	...
	if (CompileArgumentList(node->lastChild, args, namedArgs) >= 0)
	{
		...
		MatchFunctions(funcs, args, node, name.AddressOf(), &namedArgs, objectType, objIsConst, false, true,
			       scope);
		...
		else
		{
			...
			if (r == asSUCCESS)
			{
				...
				r = MakeFunctionCall(ctx, funcs[0], objectType, args, node, false, 0,
						     funcExpr.type.stackOffset);
				if (r < 0)
				{
					ctx->type.SetDummy();
					isOK = false;
...
int asCCompiler::CompileExpressionPreOp(asCScriptNode *node, asCExprContext *ctx)
{
		 ...
		 ctx->type.dataType.IsObject())
	{
		...
		if (opName)
		{
			...
			{
				asCArray<asCExprContext *> args;
				return MakeFunctionCall(ctx, funcs[0],
							CastToObjectType(ctx->type.dataType.GetTypeInfo()), args, node);
			}
	...
	else if (op == ttPlus || op == ttMinus)
	{
		...
		if (!ctx->type.isConstant)
		{
			...
			{
				if (ctx->type.dataType.IsIntegerType() &&
				    ctx->type.dataType.GetSizeInMemoryDWords() == 1)
					ctx->bc.InstrSHORT(asBC_NEGi, (short)ctx->type.stackOffset);
				else if (ctx->type.dataType.IsIntegerType() &&
					 ctx->type.dataType.GetSizeInMemoryDWords() == 2)
	...
	else if (op == ttNot)
	{
		// If turned on, allow the compiler to use either 'bool opImplConv()' or 'bool opConv()' on the type
		if (engine->ep.boolConversionMode == 1)
			ImplicitConversion(ctx, asCDataType::CreatePrimitive(ttBool, false), node,
					   asIC_EXPLICIT_VAL_CAST);
		// else, allow value types to be converted to bool using 'bool opImplConv()'
		else if (ctx->type.dataType.GetTypeInfo() &&
			 (ctx->type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE))
...
int asCCompiler::FindPropertyAccessor(const asCString &name, asCExprContext *ctx, asCScriptNode *node, asSNameSpace *ns,
				      bool isThisAccess)
{
				      ...
				      asCScriptNode *node, asSNameSpace *ns, bool isThisAccess)
{
	...
	if (outFunc->objectType && isThisAccess)
	{
		...
		if (getFunc && getFunc->funcType == asFUNC_VIRTUAL &&
		...
		if (setFunc && setFunc->funcType == asFUNC_VIRTUAL &&
...
int asCCompiler::CompileExpressionPostOp(asCScriptNode *node, asCExprContext *ctx)
{
	...
	else if (op == ttDot)
	{
		if (node->firstChild->nodeType == snIdentifier)
		{
			...
			if (ctx->type.dataType.IsObject())
			{
				...
				asCObjectProperty *prop =
				    builder->GetObjectProperty(ctx->type.dataType, name.AddressOf());
				if (prop)
				{
					// Is the property access allowed?
					...
					if (prop->compositeOffset || prop->isCompositeIndirect)
						ctx->bc.InstrSHORT_DW(
						    asBC_ADDSi, (short)prop->compositeOffset,
						    engine->GetTypeIdFromDataType(asCDataType::CreateType(
					...
					if (ctx->type.isTemporary)
					{
						...
						deferred.argInOutFlags = asTM_INREF;
						deferred.argNode       = 0;
						deferred.argType.SetVariable(ctx->type.dataType, ctx->type.stackOffset,
									     true);
		...
		else
		{
			...
			int r = CompileFunctionCall(node->firstChild, ctx,
						    CastToObjectType(ctx->type.dataType.GetTypeInfo()),
		...
		if (CompileArgumentList(node->firstChild, args, namedArgs) >= 0)
		{
			...
			if (propertyName == "")
			{
				...
				if (funcs.GetLength() > 0)
				{
					...
					else
					{
						...
						if (r < 0)
							isOK = false;
						else if (MakeFunctionCall(ctx, funcs[0], objectType, args, node, false,
									  0, ctx->type.stackOffset) < 0)
							isOK = false;
					}
				}
			}
			if (lookForProperty && isOK)
			{
				...
				else
				{
					...
					int r = FindPropertyAccessor(propertyName == "" ? "opIndex"
											: propertyName.AddressOf(),
								     &lctx, args[0], node, ns);
					if (r == 0)
					{
						asCString str;
	...
	else if (op == ttOpenParenthesis)
	{
		...
		if (CompileArgumentList(node->lastChild, args, namedArgs) >= 0)
		{
			...
			if (ctx->type.dataType.IsFuncdef())
			{
				...
				MatchFunctions(funcs, args, node, ctx->type.dataType.GetTypeInfo()->name.AddressOf(),
					       &namedArgs);
			}
			else
			{
				...
				builder->GetObjectMethodDescriptions(
				    "opCall", CastToObjectType(ctx->type.dataType.GetTypeInfo()), funcs, isConst);
			...
			else
			{
				...
				if (r == asSUCCESS)
				{
					...
					r = MakeFunctionCall(ctx, funcs[0],
							     ctx->type.dataType.IsFuncdef()
								 ? 0
								 : CastToObjectType(ctx->type.dataType.GetTypeInfo()),
							     args, node, false, 0, ctx->type.stackOffset);
					if (r < 0)
						isOK = false;
				}
				else
			       ...
			       bool allowObjectConstruct)
{
	...
	if (desc->parameterTypes[paramNum].IsReference() && desc->inOutFlags[paramNum] == asTM_OUTREF &&
	    desc->parameterTypes[paramNum].GetTokenType() != ttQuestion && !argExpr->type.IsNullConstant() &&
	    !argExpr->IsVoidExpression())
	{
...
bool asCCompiler::CompileOverloadedDualOperator(asCScriptNode *node, asCExprContext *lctx, asCExprContext *rctx,
						bool leftToRight, asCExprContext *ctx, bool isHandle, eTokenType token)
{
...
// Compiler error, don't continue
	...
	if (isHandle)
	{
		...
		asASSERT(lctx->type.dataType.GetTypeInfo() &&
						...
						bool specificReturn, const asCDataType &returnType)
{
	    ...
	    !lctx->type.IsNullConstant())
	{
		...
		if (ops.GetLength() == 1)
		{
			...
			else
			{
				...
				for (n = 0; n < rctx->deferredParams.GetLength(); n++)
				{
					    ...
					    usedVars.Exists(rctx->deferredParams[n].argType.stackOffset))
					{
						...
						int offset = AllocateVariableNotIn(
						    rctx->deferredParams[n].argType.dataType, true, false, rctx);
			...
			MergeExprBytecode(ctx, lctx);
			ctx->type = lctx->type;
			if (MakeFunctionCall(ctx, ops[0], CastToObjectType(ctx->type.dataType.GetTypeInfo()), args,
					     node) < 0)
				return -1;
				 ...
				 eTokenType op, bool leftToRight)
{
	...
	else
	{
		...
		// == != < > <= >=
		if (op == ttEqual || op == ttNotEqual || op == ttLessThan || op == ttLessThanOrEqual ||
...
void asCCompiler::ConvertToVariable(asCExprContext *ctx)
{
	...
	int offset;
	if (!ctx->type.isVariable && (ctx->type.dataType.IsObjectHandle() ||
...
void asCCompiler::ImplicitConvObjectToBestMathType(asCExprContext *ctx, asCScriptNode *node)
{
	...
	if (ot)
	{
		for (unsigned int n = 0; n < ot->methods.GetLength(); n++)
		{
			...
			asCScriptFunction *func = engine->scriptFunctions[ot->methods[n]];
			if (func->name == "opImplConv" && func->returnType.IsPrimitive() &&
			    func->parameterTypes.GetLength() == 0)
				funcs.PushLast(ot->methods[n]);
		}

		// Use the one with the highest precision
		const eTokenType match[10] = {ttDouble, ttFloat, ttInt64,  ttUInt64, ttInt,
					      ttUInt,	ttInt16, ttUInt16, ttInt8,   ttUInt8};
				      ...
				      asCExprContext *ctx, eTokenType op)
{
	...
	if (engine->ep.disableIntegerDivision && (op == ttSlash || op == ttDivAssign) &&
	...
	{
		asCString str;
		str.Format(TXT_NO_CONVERSION_s_TO_MATH_TYPE,
			   lctx->type.dataType.Format(outFunc->nameSpace).AddressOf());
		Error(str, node);
	...
	if (!isConstant)
	{
		...
		if (op == ttAddAssign || op == ttSubAssign || op == ttMulAssign || op == ttDivAssign ||
			...
			if (lctx->type.dataType.GetSizeInMemoryDWords() == 1)
			{
				...
				else if (op == ttSlash)
				{
					...
					if (rctx->type.GetConstantDW() == 0 ||
					    (int(rctx->type.GetConstantDW()) == -1 &&
					     lctx->type.GetConstantDW() == 0x80000000))
						v = 0;
				...
				else if (op == ttStarStar)
				{
					bool isOverflow;
					if (lctx->type.dataType.IsIntegerType())
						v = as_powi(int(lctx->type.GetConstantDW()),
							    int(rctx->type.GetConstantDW()), isOverflow);
					else
						v = as_powu(lctx->type.GetConstantDW(), rctx->type.GetConstantDW(),
							    isOverflow);
			...
			else
			{
				...
				else if (op == ttSlash)
				{
					...
					if (rctx->type.GetConstantQW() == 0 ||
						...
						v = asINT64(lctx->type.GetConstantQW()) /
						    asINT64(rctx->type.GetConstantQW());
					else
				...
				else if (op == ttStarStar)
				{
					bool isOverflow;
					if (lctx->type.dataType.IsIntegerType())
						v = as_powi64(asINT64(lctx->type.GetConstantQW()),
							      asINT64(rctx->type.GetConstantQW()), isOverflow);
					else
						v = as_powu64(lctx->type.GetConstantQW(), rctx->type.GetConstantQW(),
							      isOverflow);
					 ...
					 asCExprContext *ctx, eTokenType op)
{
	...
	else if (op == ttBitShiftLeft || op == ttShiftLeftAssign || op == ttBitShiftRight ||
		 op == ttShiftRightLAssign || op == ttBitShiftRightArith || op == ttShiftRightAAssign)
	{
			...
			if (opCtx->type.isConstant)
			{
				    ...
				    opCtx->type.dataType.GetTokenType() == ttInt64)
				{
					if (!(opCtx->type.GetConstantQW() & (asQWORD(1) << 63)))
					 ...
					 opCtx->type.dataType.GetTokenType() == ttInt ||
					 opCtx->type.dataType.IsEnumType())
				{
		...
		else
		{
			...
			if (lctx->type.dataType.IsIntegerType() && lctx->type.dataType.GetSizeInMemoryDWords() == 1)
				iCmp = asBC_CMPi;
				 ...
				 lctx->type.dataType.GetSizeInMemoryDWords() == 1)
				iCmp = asBC_CMPu;
			else if (lctx->type.dataType.IsIntegerType() &&
				 lctx->type.dataType.GetSizeInMemoryDWords() == 2)
				iCmp = asBC_CMPi64;
			else if (lctx->type.dataType.IsUnsignedType() &&
				 lctx->type.dataType.GetSizeInMemoryDWords() == 2)
				iCmp = asBC_CMPu64;
	...
	if ((descr->returnType.IsObject() || descr->returnType.IsFuncdef()) && !descr->returnType.IsReference())
	{
		...
		else
		{
			...
			else
			{
				...
				returnOffset =
				    AllocateVariable(descr->returnType, true, !descr->returnType.IsObjectHandle());
				ctx->type.SetVariable(descr->returnType, returnOffset, true);
			}
