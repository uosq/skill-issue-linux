...
{
	if (ch >= '0' && ch <= '9')
		return ((ch -= '0') < radix ? ch : -1);
	if (ch >= 'A' && ch <= 'Z')
		return ((ch -= 'A' - 10) < radix ? ch : -1);
	if (ch >= 'a' && ch <= 'z')
		return ((ch -= 'a' - 10) < radix ? ch : -1);
	return -1;
}
...
asQWORD asStringScanUInt64(const char *string, int base, size_t *numScanned, bool *overflow)
{
	...
	if (base == 10)
	{
		while (*end >= '0' && *end <= '9')
		{
			if (overflow &&
			    ((res > QWORD_MAX / 10) ||
			     ((asUINT(*end - '0') > (QWORD_MAX - (QWORD_MAX / 10) * 10)) && res == QWORD_MAX / 10)))
				*overflow = true;
			res *= 10;
			res += *end++ - '0';
		}
	...
	else
	{
		...
		if (base)
		{
			...
			{
				if (overflow && ((res > QWORD_MAX / base) ||
						 ((asUINT(nbr) > (QWORD_MAX - (QWORD_MAX / base) * base)) &&
						  res == QWORD_MAX / base)))
					*overflow = true;
