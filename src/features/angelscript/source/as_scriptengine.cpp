...
asCScriptEngine::asCScriptEngine()
{
	...
	// Engine properties
	{
		...
		ep.alwaysImplDefaultConstruct =
		    0;			 // 0 = as per language spec, 1 = always implement it, 2, never implement
		...
		ep.heredocTrimMode =
		    1; // 0 = never trim, 1 = don't trim on single line, 2 = trim initial and final empty line
		...
		ep.jitInterfaceVersion =
		    1; // 1 = JIT compiler uses asJITCompiler, 2 = JIT compiler uses asJITCompilerV2
		ep.alwaysImplDefaultCopy = 0; // 0 = as per language spec, 1 = always implement it, 2, never implement
		ep.alwaysImplDefaultCopyConstruct =
		    0; // 0 = as per language spec, 1 = always implement it, 2, never implement
		ep.memberInitMode =
		    1; // 0 = pre 2.38.0, members with init expr in declaration are initialized after super(), 1 = all members initialized in beginning, except if explicitly initialized in body
		ep.boolConversionMode =
		    0; // 0 = only do use opImplConv for registered value type, 1 = use also opConv in contextual conversion even for reference types
	...
	defaultAccessMask =
	    0xFFFFFFFF; // All bits set so that built-in functions/types will be available to all modules
...
int asCScriptEngine::RegisterObjectType(const char *name, int byteSize, asQWORD flags)
{
	...
	isPrepared =
	    false; // TODO: Only set this after the validations have been completed, to avoid unnecessary Prepare in case no change was made
	...
	else if (flags & asOBJ_VALUE)
	{
		...
		else
		{
			...
			if (flags & (asOBJ_APP_CLASS_CONSTRUCTOR | asOBJ_APP_CLASS_DESTRUCTOR |
				     asOBJ_APP_CLASS_ASSIGNMENT | asOBJ_APP_CLASS_COPY_CONSTRUCTOR |
				     asOBJ_APP_CLASS_ALLINTS | asOBJ_APP_CLASS_ALLFLOATS | asOBJ_APP_CLASS_UNION))
			{
				return ConfigError(asINVALID_ARG, "RegisterObjectType", name, 0);
			}
		}
	...
	else
	{
		...
		if (originalSizeOfGeneratedTemplateTypes)
			mostRecentTemplateInstanceType =
			    generatedTemplateTypes[originalSizeOfGeneratedTemplateTypes - 1];
		...
		else
		{
			...
			if (dt.GetSubType().IsTemplate())
				return ConfigError(asLOWER_ARRAY_DIMENSION_NOT_REGISTERED, "RegisterObjectType", name,
						   0);
			...
			if (generatedTemplateTypes.Exists(CastToObjectType(dt.GetTypeInfo())) &&
			    generatedTemplateTypes[generatedTemplateTypes.GetLength() - 1] ==
				mostRecentTemplateInstanceType)
			{
				asCString str;
...
// internal
int asCScriptEngine::RegisterBehaviourToObjectType(asCObjectType *objectType, asEBehaviours behaviour, const char *decl,
						   const asSFuncPtr &funcPointer, asDWORD callConv, void *auxiliary,
						   int compositeOffset, bool isCompositeIndirect)
{
#ifdef AS_MAX_PORTABILITY
	...
	if (objectType->flags & asOBJ_TEMPLATE)
	{
		for (asUINT n = 0; n < generatedTemplateTypes.GetLength(); n++)
		{
			      ...
			      (tmpl->templateSubTypes[0].GetTypeInfo()->flags & asOBJ_TEMPLATE_SUBTYPE)))
			{
				...
				return ConfigError(asERROR, "RegisterObjectBehaviour", objectType->name.AddressOf(),
						   decl);
			}
		}
	}
	...
	if (behaviour == asBEHAVE_CONSTRUCT)
	{
		...
		if (func.returnType != asCDataType::CreatePrimitive(ttVoid, false))
			return ConfigError(asINVALID_DECLARATION, "RegisterObjectBehaviour",
		...
		else
		{
			...
			{
				WriteMessage("", 0, 0, asMSGTYPE_ERROR,
					     TXT_FIRST_PARAM_MUST_BE_REF_FOR_TEMPLATE_FACTORY);
				return ConfigError(asINVALID_DECLARATION, "RegisterObjectBehaviour",
						   objectType->name.AddressOf(), decl);
			}
	...
	else if (behaviour == asBEHAVE_DESTRUCT)
	{
		...
		if (beh->destruct)
			return ConfigError(asALREADY_REGISTERED, "RegisterObjectBehaviour",
					   objectType->name.AddressOf(), decl);
	...
	else if (behaviour == asBEHAVE_FACTORY || behaviour == asBEHAVE_LIST_FACTORY)
	{
		...
		if (behaviour == asBEHAVE_LIST_FACTORY)
		{
			...
			if (objectType->flags & asOBJ_TEMPLATE)
			{
				if (func.parameterTypes.GetLength() != 2 || !func.parameterTypes[1].IsReference())
				{
					...
					WriteMessage("", 0, 0, asMSGTYPE_ERROR,
						     TXT_TEMPLATE_LIST_FACTORY_EXPECTS_2_REF_PARAMS);
					return ConfigError(asINVALID_DECLARATION, "RegisterObjectBehaviour",
							   objectType->name.AddressOf(), decl);
				}
			}
			 ...
			 (func.parameterTypes.GetLength() == 2 && (objectType->flags & asOBJ_TEMPLATE)))
		{
			if (behaviour == asBEHAVE_LIST_FACTORY)
			{
				...
				if (r < 0)
					return ConfigError(r, "RegisterObjectBehaviour", objectType->name.AddressOf(),
							   decl);
			}
			else
			{
	...
	else if (behaviour >= asBEHAVE_FIRST_GC && behaviour <= asBEHAVE_LAST_GC)
	{
		...
		if ((behaviour == asBEHAVE_GETREFCOUNT || behaviour == asBEHAVE_SETGCFLAG ||
		...
		if ((behaviour == asBEHAVE_SETGCFLAG || behaviour == asBEHAVE_ENUMREFS ||
	...
	for (asUINT subTypeIdx = 0; subTypeIdx < templateType->templateSubTypes.GetLength(); subTypeIdx++)
	{
		...
		for (asUINT n = 0; n < func->parameterTypes.GetLength(); n++)
		{
			if (func->parameterTypes[n].GetTypeInfo() ==
			    templateType->templateSubTypes[subTypeIdx].GetTypeInfo())
			{
				...
				if (!func->parameterTypes[n].IsObjectHandle() && !func->parameterTypes[n].IsReference())
					return ConfigError(asNOT_SUPPORTED, caller, templateType->name.AddressOf(),
							   decl);
			}
		}
	}
						...
						int compositeOffset, bool isCompositeIndirect)
{
		...
		for (asUINT n = 0; n < generatedTemplateTypes.GetLength(); n++)
		{
			      ...
			      (tmpl->templateSubTypes[0].GetTypeInfo()->flags & asOBJ_TEMPLATE_SUBTYPE)))
			{
				...
				msg.Format(TXT_TEMPLATE_s_ALREADY_GENERATED_CANT_REGISTER,
					   asCDataType::CreateType(tmpl, false).Format(tmpl->nameSpace).AddressOf());
				WriteMessage("", 0, 0, asMSGTYPE_ERROR, msg.AddressOf());
				return ConfigError(asERROR, "RegisterObjectMethod", objectType->name.AddressOf(),
						   declaration);
			}
		}
	}
	...
	if (func->IsProperty() && (r = bld.ValidateVirtualProperty(func)) < 0)
	{
		...
		asDELETE(func, asCScriptFunction);
		if (r == -5)
			return ConfigError(asNAME_TAKEN, "RegisterObjectMethod", objectType->name.AddressOf(),
					   declaration);
		else
			return ConfigError(asINVALID_DECLARATION, "RegisterObjectMethod", objectType->name.AddressOf(),
					   declaration);
	    ...
	    func->name == "opImplCast")
	{
		...
		for (asUINT n = 0; n < func->objectType->methods.GetLength(); n++)
		{
			...
			if (f->name == func->name && f->IsSignatureExceptNameEqual(func))
			{
				func->funcType = asFUNC_DUMMY;
				asDELETE(func, asCScriptFunction);
				return ConfigError(asALREADY_REGISTERED, "RegisterObjectMethod",
						   objectType->name.AddressOf(), declaration);
			}
		}
	}
	else
	...
	if (func->name == "opAssign" && func->parameterTypes.GetLength() == 1 && !func->IsReadOnly() &&
	    ((objectType->flags & asOBJ_SCRIPT_OBJECT) ||
	     func->parameterTypes[0].IsEqualExceptRefAndConst(asCDataType::CreateType(func->objectType, false))))
	{
					    ...
					    void *auxiliary)
{
	...
	if (func->templateSubTypes.GetLength())
		registeredTemplateGlobalFuncs.PushLast(func);
...
int asCScriptEngine::GetTemplateFunctionInstance(asCScriptFunction *baseFunc, const asCArray<asCDataType> &types)
{
	...
	for (asUINT n = 0; n < generatedTemplateFunctionInstances.GetLength(); n++)
	{
		...
		if (func->name == baseFunc->name && func->nameSpace == baseFunc->nameSpace &&
		    func->objectType == baseFunc->objectType && func->templateSubTypes == types)
			return func->id;
	}
	...
	if (newFunc->objectType)
		newFunc->objectType->AddRefInternal();
	...
	newFunc->returnType =
	    DetermineTypeForTemplate(baseFunc->returnType, baseFunc->templateSubTypes, types, 0, 0, 0);
	newFunc->parameterTypes.SetLength(baseFunc->parameterTypes.GetLength());
	for (asUINT p = 0; p < baseFunc->parameterTypes.GetLength(); p++)
		newFunc->parameterTypes[p] =
		    DetermineTypeForTemplate(baseFunc->parameterTypes[p], baseFunc->templateSubTypes, types, 0, 0, 0);
	...
	generatedTemplateFunctionInstances.PushLast(
	    newFunc); // This holds a reference (constructor already set it to 1)

	// Adjust the clean up instructions
	if (newFunc->sysFuncIntf->callConv == ICC_GENERIC_FUNC || newFunc->sysFuncIntf->callConv == ICC_GENERIC_METHOD)
...
void asCScriptEngine::PrepareEngine()
{
	...
	for (n = 0; n < registeredObjTypes.GetLength(); n++)
	{
		...
		if (type && !(type->flags & asOBJ_SCRIPT_OBJECT))
		{
			...
			if (type->flags & asOBJ_GC)
			{
				if (type->flags & asOBJ_REF)
				{
					if (type->beh.addref == 0 || type->beh.release == 0 ||
					    type->beh.gcGetRefCount == 0 || type->beh.gcSetFlag == 0 ||
					    type->beh.gcGetFlag == 0 || type->beh.gcEnumReferences == 0 ||
			...
			if ((type->flags & asOBJ_REF) && !(type->flags & asOBJ_SCOPED) &&
			    !(type->flags & asOBJ_NOHANDLE) && !(type->flags & asOBJ_NOCOUNT))
			{
				if (type->beh.addref == 0 || type->beh.release == 0)
				{
...
int asCScriptEngine::ConfigError(int err, const char *funcName, const char *arg1, const char *arg2)
{
	...
	if (funcName)
	{
		...
		{
			if (arg2)
				str.Format(TXT_FAILED_IN_FUNC_s_WITH_s_AND_s_s_d, funcName, arg1, arg2,
					   errorNames[-err], err);
			else
							...
							asCModule *requestingModule)
{
	...
	{
		asCFuncdefType *funcdef =
		    GenerateFuncdefForTemplateObjectInstance(templateType, ot, templateType->childFuncDefs[n]);
	...
	if (templateType->flags & asOBJ_REF)
	{
		...
		for (n = 0; n < ot->beh.constructors.GetLength(); n++)
		{
			asCScriptFunction *func =
			    GenerateFactoryStubForTemplateObjectInstance(templateType, ot, ot->beh.constructors[n]);
	...
	{
		asCScriptFunction *func =
		    GenerateFactoryStubForTemplateObjectInstance(templateType, ot, templateType->beh.listFactory);
...
asILockableSharedBool *asCScriptEngine::GetWeakRefFlagOfScriptObject(void *obj, const asITypeInfo *type) const
{
	...
	if (objType->beh.getWeakRefFlag)
	{
		...
		dest =
		    reinterpret_cast<asILockableSharedBool *>(CallObjectMethodRetPtr(obj, objType->beh.getWeakRefFlag));
	}
	return dest;
}
...
asCDataType asCScriptEngine::DetermineTypeForTemplate(const asCDataType &orig,
						      const asCArray<asCDataType> &templateSubTypes,
						      const asCArray<asCDataType> &targetSubTypes, asCModule *mod,
						      asCObjectType *tmpl, asCObjectType *ot)
{
...
// internal
asCScriptFunction *asCScriptEngine::GenerateFactoryStubForTemplateObjectInstance(asCObjectType *templateType,
										 asCObjectType *ot, int factoryId)
{
	...
	*(((asWORD *)bc) + 1) = (asWORD)func->GetSpaceNeededForArguments() + (func->objectType ? AS_PTR_SIZE : 0);
	...
	if (factory->listPattern)
	{
		...
		while (n)
		{
			...
			{
				asSListPatternDataTypeNode *typeNode =
				    reinterpret_cast<asSListPatternDataTypeNode *>(newNode);
				typeNode->dataType =
				    DetermineTypeForTemplate(typeNode->dataType, templateType->templateSubTypes,
							     ot->templateSubTypes, ot->module, templateType, ot);
			}
...
bool asCScriptEngine::GenerateFunctionForTemplateObjectInstance(asCObjectType *templateType, asCObjectType *ot,
...
*/
	...
	func2->returnType = DetermineTypeForTemplate(func->returnType, templateType->templateSubTypes,
						     ot->templateSubTypes, ot->module, templateType, ot);
	func2->parameterTypes.SetLength(func->parameterTypes.GetLength());
	for (asUINT p = 0; p < func->parameterTypes.GetLength(); p++)
		func2->parameterTypes[p] =
		    DetermineTypeForTemplate(func->parameterTypes[p], templateType->templateSubTypes,
					     ot->templateSubTypes, ot->module, templateType, ot);
...
asCFuncdefType *asCScriptEngine::GenerateFuncdefForTemplateObjectInstance(asCObjectType *templateType,
									  asCObjectType *ot, asCFuncdefType *func)
{
	...
	func2->returnType = DetermineTypeForTemplate(func->funcdef->returnType, templateType->templateSubTypes,
						     ot->templateSubTypes, ot->module, templateType, ot);
	...
	for (asUINT p = 0; p < func->funcdef->parameterTypes.GetLength(); p++)
		func2->parameterTypes[p] =
		    DetermineTypeForTemplate(func->funcdef->parameterTypes[p], templateType->templateSubTypes,
					     ot->templateSubTypes, ot->module, templateType, ot);
...
int asCScriptEngine::AssignScriptObject(void *dstObj, void *srcObj, const asITypeInfo *type)
{
	...
	if (objType->beh.copy)
	{
		...
		else
		{
			...
			reinterpret_cast<asCScriptObject *>(dstObj)->CopyFrom(
			    reinterpret_cast<asCScriptObject *>(srcObj));
		}
	}
...
void asCScriptEngine::AddScriptFunction(asCScriptFunction *func)
{
	...
	if (freeScriptFunctionIds.GetLength() &&
	    freeScriptFunctionIds[freeScriptFunctionIds.GetLength() - 1] == func->id)
...
asCFuncdefType *asCScriptEngine::FindMatchingFuncdef(asCScriptFunction *func, asCModule *module)
{
	...
	if (funcDef == 0)
	{
		...
		fd->name =
		    "$func_" + func->name; // Add a prefix to avoid name conflicts with explicitly declared funcdefs
