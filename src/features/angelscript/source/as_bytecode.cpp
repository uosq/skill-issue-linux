...
bool asCByteCode::RemoveUnusedValue(asCByteInstruction *curr, asCByteInstruction **next)
{
	...
	if (curr->op == asBC_SetV4 && curr->next)
	{
		...
		if (curr->next->op == asBC_CpyVtoV4 && curr->wArg[0] == curr->next->wArg[1] &&
		...
		if (curr->next->op == asBC_PshV4 && curr->wArg[0] == curr->next->wArg[0] &&
		    IsTemporary(curr->wArg[0]) && !IsTempVarRead(curr->next, curr->wArg[0]))
		{
			curr->op       = asBC_PshC4;
		...
		if (curr->next->op == asBC_CpyVtoG4 && curr->wArg[0] == curr->next->wArg[0] &&
		    IsTemporary(curr->wArg[0]) && !IsTempVarRead(curr->next, curr->wArg[0]))
		{
			curr->op				 = asBC_SetG4;
...
void asCByteCode::OptimizeLocally(const asCArray<int> &tempVariableOffsets)
{
	...
	while (instr)
	{
		...
		{
			// T??, ClrHi -> T??
			if (instr && (instr->op == asBC_TZ || instr->op == asBC_TNZ || instr->op == asBC_TS ||
				      instr->op == asBC_TNS || instr->op == asBC_TP || instr->op == asBC_TNP))
			{
		...
		else if (currOp == asBC_VAR)
		{
			...
			if (curr->next && curr->next->op == asBC_PSF && curr->next->next &&
			    curr->next->next->op == asBC_GETOBJREF && curr->next->next->wArg[0] == AS_PTR_SIZE)
			{
			...
			if (curr->next && curr->next->op == asBC_PSF && curr->next->next &&
			    curr->next->next->op == asBC_GETREF && curr->next->next->wArg[0] == AS_PTR_SIZE)
			{
				curr->op = asBC_PSF;
				DeleteInstruction(curr->next->next);
				instr = GoForward(curr);
...
bool asCByteCode::IsTempVarRead(asCByteInstruction *curr, int offset)
{
	...
	while (openPaths.GetLength())
	{
		...
		while (curr)
		{
			...
			else if (curr->op == asBC_JZ || curr->op == asBC_JNZ || curr->op == asBC_JS ||
...
bool asCByteCode::IsTempRegUsed(asCByteInstruction *curr)
{
	...
	while (curr->next)
	{
		...
		if (curr->op == asBC_INCi || curr->op == asBC_INCi16 || curr->op == asBC_INCi8 ||
		    curr->op == asBC_INCf || curr->op == asBC_INCd || curr->op == asBC_DECi ||
		    curr->op == asBC_DECi16 || curr->op == asBC_DECi8 || curr->op == asBC_DECf ||
		    curr->op == asBC_DECd || curr->op == asBC_WRTV1 || curr->op == asBC_WRTV2 ||
		    curr->op == asBC_WRTV4 || curr->op == asBC_WRTV8 || curr->op == asBC_RDR1 ||
		    curr->op == asBC_RDR2 || curr->op == asBC_RDR4 || curr->op == asBC_RDR8 ||
		    curr->op == asBC_PshRPtr || curr->op == asBC_CpyRtoV4 || curr->op == asBC_CpyRtoV8 ||
		    curr->op == asBC_TZ || curr->op == asBC_TNZ || curr->op == asBC_TS || curr->op == asBC_TNS ||
		    curr->op == asBC_TP || curr->op == asBC_TNP || curr->op == asBC_JZ || curr->op == asBC_JNZ ||
		    curr->op == asBC_JLowZ || curr->op == asBC_JLowNZ || curr->op == asBC_JS || curr->op == asBC_JNS ||
		    curr->op == asBC_JP || curr->op == asBC_JNP ||
		    curr->op ==
			asBC_JMPP || // TODO: JMP and JMPP cannot be said to read the temporary register. Need to follow the branch to determine what happens next
		    curr->op == asBC_JMP)
			return true;

		// Which instructions overwrite the register or discard the value?
		if (curr->op == asBC_CALL || curr->op == asBC_PopRPtr || curr->op == asBC_CALLSYS ||
		    curr->op == asBC_CALLBND || curr->op == asBC_Thiscall1 || curr->op == asBC_SUSPEND ||
		    curr->op == asBC_ALLOC || curr->op == asBC_CpyVtoR4 || curr->op == asBC_LdGRdR4 ||
		    curr->op == asBC_LDG || curr->op == asBC_LDV || curr->op == asBC_TZ || curr->op == asBC_TNZ ||
		    curr->op == asBC_TS || curr->op == asBC_TNS || curr->op == asBC_TP || curr->op == asBC_TNP ||
		    curr->op == asBC_CMPi || curr->op == asBC_CMPu || curr->op == asBC_CMPf || curr->op == asBC_CMPd ||
		    curr->op == asBC_CMPIi || curr->op == asBC_CMPIu || curr->op == asBC_CMPIf ||
...
bool asCByteCode::IsSimpleExpression()
{
	...
	while (instr)
	{
		if (instr->op == asBC_ALLOC || instr->op == asBC_CALL || instr->op == asBC_CALLSYS ||
		    instr->op == asBC_SUSPEND || instr->op == asBC_LINE || instr->op == asBC_FREE ||
		    instr->op == asBC_CallPtr || instr->op == asBC_CALLINTF || instr->op == asBC_CALLBND ||
	...
	while (instr)
	{
		if (instr->op == asBC_Block)
		{
			...
			else
			{
				...
				if (outFunc->scriptData
					    ->objVariableInfo[outFunc->scriptData->objVariableInfo.GetLength() - 1]
...
void asCByteCode::DebugOutput(const char *name, asCScriptFunction *func)
{
	...
	for (n = 0; n < func->scriptData->variables.GetLength(); n++)
	{
		...
		fprintf(file, " %.3d: %s%s %s\n", func->scriptData->variables[n]->stackOffset,
			isOnHeap ? "(heap) " : "",
			func->scriptData->variables[n]->type.Format(func->nameSpace, true).AddressOf(),
			func->scriptData->variables[n]->name.AddressOf());
	}
	if (func->objectType)
	...
	while (instr)
	{
		...
		if (instr->GetSize() > 0)
		{
			...
			fprintf(file, "%3d %c ", int(instr->stackSize + func->scriptData->variableSpace),
				instr->marked ? '*' : ' ');
		...
		case asBCTYPE_rW_W_DW_ARG:
			...
			case asBC_MULIf:
				fprintf(file, "   %-8s v%d, v%d, %f\n", asBCInfo[instr->op].name, instr->wArg[0],
					instr->wArg[1], *((float *)ARG_DW(instr->arg)));
				break;
			default:
				fprintf(file, "   %-8s v%d, v%d, %d\n", asBCInfo[instr->op].name, instr->wArg[0],
					instr->wArg[1], *((int *)ARG_DW(instr->arg)));
				break;
			}
			break;
		...
		case asBCTYPE_DW_ARG:
			...
			case asBC_OBJTYPE:
			{
				...
				fprintf(file, "   %-8s 0x%x           (type:%s)\n", asBCInfo[instr->op].name,
					(asUINT)*ARG_DW(instr->arg), ot->GetName());
			}
			break;
			...
			case asBC_FuncPtr:
			{
				...
				fprintf(file, "   %-8s 0x%x          (func:%s)\n", asBCInfo[instr->op].name,
					(asUINT)*ARG_DW(instr->arg), f->GetDeclaration());
			}
			break;
			...
			case asBC_Cast:
				fprintf(file, "   %-8s 0x%x          (i:%d, f:%g)\n", asBCInfo[instr->op].name,
					(asUINT)*ARG_DW(instr->arg), *((int *)ARG_DW(instr->arg)),
					*((float *)ARG_DW(instr->arg)));
				break;

			case asBC_TYPEID:
				fprintf(file, "   %-8s 0x%x          '%s'\n", asBCInfo[instr->op].name,
					(asUINT)*ARG_DW(instr->arg),
			...
			case asBC_Thiscall1:
			{
				...
				fprintf(file, "   %-8s %d           (%s)\n", asBCInfo[instr->op].name,
					*((int *)ARG_DW(instr->arg)), decl.AddressOf());
			}
			break;
			...
			case asBC_JNP:
				fprintf(file, "   %-8s %+d              (d:%d)\n", asBCInfo[instr->op].name,
					*((int *)ARG_DW(instr->arg)), pos + *((int *)ARG_DW(instr->arg)));
				break;
		...
		case asBCTYPE_QW_ARG:
			...
			case asBC_OBJTYPE:
			{
				...
				fprintf(file, "   %-8s 0x%x          (type:%s)\n", asBCInfo[instr->op].name,
					(asUINT)*ARG_QW(instr->arg), ot->GetName());
			}
			break;
			...
			case asBC_FuncPtr:
			{
				...
				fprintf(file, "   %-8s 0x%x          (func:%s)\n", asBCInfo[instr->op].name,
					(asUINT)*ARG_QW(instr->arg), f->GetDeclaration());
			}
			break;
			...
			case asBC_PGA:
			{
				...
				if (engine->varAddressMap.MoveTo(&cursor, ptr))
				{
					fprintf(file, "   %-8s 0x%x          (var:%s)\n", asBCInfo[instr->op].name,
						(asUINT)*ARG_QW(instr->arg), cursor->value->name.AddressOf());
				}
				else
				{
					asUINT length;
					...
					fprintf(file, "   %-8s 0x%x          (str:%s)\n", asBCInfo[instr->op].name,
						(asUINT)*ARG_QW(instr->arg), str.AddressOf());
				}
			}
			break;
			...
			default:
				...
				fprintf(file, "   %-8s 0x%lx           (i:%ld, f:%g)\n", asBCInfo[instr->op].name,
					*ARG_QW(instr->arg), *((asINT64 *)ARG_QW(instr->arg)),
					*((double *)ARG_QW(instr->arg)));
#else
		...
		case asBCTYPE_rW_QW_ARG:
			...
			case asBC_FREE:
			{
				...
				fprintf(file, "   %-8s v%d, 0x%x          (type:%s)\n", asBCInfo[instr->op].name,
					instr->wArg[0], (asUINT)*ARG_QW(instr->arg), ot->GetName());
			}
			break;
			...
			default:
				...
				fprintf(file, "   %-8s v%d, 0x%lx           (i:%ld, f:%g)\n", asBCInfo[instr->op].name,
					instr->wArg[0], *ARG_QW(instr->arg), *((asINT64 *)ARG_QW(instr->arg)),
					*((double *)ARG_QW(instr->arg)));
#else
				fprintf(file, "   %-8s v%d, 0x%llx           (i:%lld, f:%g)\n",
					asBCInfo[instr->op].name, instr->wArg[0], *ARG_QW(instr->arg),
		...
		case asBCTYPE_DW_DW_ARG:
			if (instr->op == asBC_ALLOC)
			{
				...
				fprintf(file, "   %-8s 0x%x, %d             (type:%s, %s)\n", asBCInfo[instr->op].name,
					*(int *)ARG_DW(instr->arg), *(int *)(ARG_DW(instr->arg) + 1), ot->GetName(),
					f ? f->GetDeclaration() : "{no func}");
			}
			else
				fprintf(file, "   %-8s %u, %d\n", asBCInfo[instr->op].name, *(int *)ARG_DW(instr->arg),
		...
		case asBCTYPE_rW_DW_DW_ARG:
			fprintf(file, "   %-8s v%d, %u, %u\n", asBCInfo[instr->op].name, instr->wArg[0],
				*(int *)ARG_DW(instr->arg), *(int *)(ARG_DW(instr->arg) + 1));
			break;
		...
		case asBCTYPE_QW_DW_ARG:
			if (instr->op == asBC_ALLOC)
			{
				...
				fprintf(file, "   %-8s 0x%llx, %d             (type:%s, %s)\n",
					...
					*(int *)(ARG_DW(instr->arg) + 2), ot->GetName(),
					f ? f->GetDeclaration() : "{no func}");
#endif
#else
				...
				fprintf(file, "   %-8s %lu, %d\n", asBCInfo[instr->op].name,
					*(asINT64 *)ARG_QW(instr->arg), *(int *)(ARG_DW(instr->arg) + 2));
#else
		...
		case asBCTYPE_W_DW_ARG:
			if (instr->op == asBC_SetV1)
				fprintf(file, "   %-8s v%d, 0x%x\n", asBCInfo[instr->op].name, instr->wArg[0],
					*(asBYTE *)ARG_DW(instr->arg));
			else if (instr->op == asBC_SetV2)
				fprintf(file, "   %-8s v%d, 0x%x\n", asBCInfo[instr->op].name, instr->wArg[0],
					*(asWORD *)ARG_DW(instr->arg));
			else if (instr->op == asBC_SetV4)
				fprintf(file, "   %-8s v%d, 0x%x          (i:%d, f:%g)\n", asBCInfo[instr->op].name,
					instr->wArg[0], (asUINT)*ARG_DW(instr->arg), *((int *)ARG_DW(instr->arg)),
					*((float *)ARG_DW(instr->arg)));
