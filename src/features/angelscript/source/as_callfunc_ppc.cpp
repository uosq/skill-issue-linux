...
#define PPC_STACK_SIZE(numParams)                                                                                      \
	(-((((((numParams) < 8) ? 8 : (numParams)) << 2) + EXTRA_STACK_SIZE + 15) &                                    \
		...
		for (asUINT n = 0; n < descr->parameterTypes.GetLength(); n++)
		{
			...
			else
			{
				...
				if (descr->parameterTypes[n].IsFloatType() && !descr->parameterTypes[n].IsReference())
					argsType[a++] = ppcFLOATARG;
				else if (descr->parameterTypes[n].IsDoubleType() &&
					 !descr->parameterTypes[n].IsReference())
					argsType[a++] = ppcDOUBLEARG;
				else
					argsType[a++] = ppcINTARG;
				...
				{
					paramBuffer[dpos++] = args[spos++];
					if (!descr->parameterTypes[n]
						 .IsDoubleType()) // Double already knows it is 2 dwords
						argsType[a++] = ppcINTARG;
				}
