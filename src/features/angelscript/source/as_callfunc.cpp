			    ...
			    asSSystemFunctionInterface *internal)
{
	...
	{
		internal->callConv = base == asCALL_CDECL_OBJFIRST ? ICC_CDECL_OBJFIRST : ICC_CDECL_OBJLAST;
		if (!isMethod)
		{
			if (auxiliary == 0)
...
int PrepareSystemFunction(asCScriptFunction *func, asSSystemFunctionInterface *internal, asCScriptEngine *engine)
{
	...
	else if (func->returnType.IsObject())
	{
		...
		else if (objType & asOBJ_APP_CLASS)
		{
			...
			else
			{
				...
				else
				{
					...
					if (func->returnType.GetTypeInfo()->flags & asOBJ_APP_CLASS_ALLFLOATS)
					{
						...
						if (!(func->returnType.GetTypeInfo()->flags & asOBJ_APP_CLASS_ALIGN8) &&
						    func->returnType.GetSizeInMemoryDWords() > 2)
						{
...
#endif
				    func->returnType.GetSizeInMemoryDWords() >=
				...
				if (internal->callConv == ICC_STDCALL && func->returnType.GetSizeInMemoryDWords() >=
			...
			if (!internal->hostReturnInMemory && !(func->returnType.GetTypeInfo()->flags &
							       (asOBJ_APP_CLASS_ALLINTS | asOBJ_APP_CLASS_ALLFLOATS)))
			{
				engine->WriteMessage("", 0, 0, asMSGTYPE_INFORMATION,
						     func->GetDeclarationStr().AddressOf());
	...
	for (n = 0; n < func->parameterTypes.GetLength(); n++)
	{
		    ...
		    !func->parameterTypes[n].IsReference())
		{
			      ...
			      (asOBJ_APP_CLASS | asOBJ_APP_PRIMITIVE | asOBJ_APP_FLOAT | asOBJ_APP_ARRAY)))
			{
				...
				asCString str;
				str.Format(TXT_CANNOT_PASS_TYPE_s_BY_VAL,
			       ...
			       asOBJ_APP_CLASS_ALLFLOATS)))
			{
				...
				asCString str;
				str.Format(TXT_DONT_SUPPORT_TYPE_s_BY_VAL,
					   func->parameterTypes[n].GetTypeInfo()->name.AddressOf());
				engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
				engine->ConfigError(asINVALID_CONFIGURATION, 0, 0, 0);
			}
...
// secondObj  - This is the object pointer that the proxy method should invoke its method on when the call convention is THISCALL_OBJFIRST/LAST
void CallSystemFunctionNative(asCContext *context, asCScriptFunction *descr, void *obj, asDWORD *args, void *retPointer,
			      asQWORD *retQW, void *secondObj);
...
int CallSystemFunction(int id, asCContext *context)
{
...
#ifndef RETURN_VALUE_MAX_SIZE
	retQW[0] = CallSystemFunctionNative(context, descr, obj, args, sysFunc->hostReturnInMemory ? retPointer : 0,
					    retQW[1], secondObj);
#else
	CallSystemFunctionNative(context, descr, obj, args, sysFunc->hostReturnInMemory ? retPointer : 0, retQW,
				 secondObj);
#endif
#else
	...
	{
#ifndef RETURN_VALUE_MAX_SIZE
		retQW[0] = CallSystemFunctionNative(context, descr, obj, args,
						    sysFunc->hostReturnInMemory ? retPointer : 0, retQW[1], secondObj);
#else
		CallSystemFunctionNative(context, descr, obj, args, sysFunc->hostReturnInMemory ? retPointer : 0, retQW,
					 secondObj);
#endif
