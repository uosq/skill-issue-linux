...
// forward declaration
template <class T> class asCSymbolTable;

// Iterator that allows iterating in index order
template <class T, class T2 = T> class asCSymbolTableIterator
{
      public:
	T2 *operator*() const;
	T2 *operator->() const;
...
template <class T> void asCSymbolTable<T>::SwapWith(asCSymbolTable<T> &other)
{
...
template <class T> asCSymbolTable<T>::asCSymbolTable(asUINT initialCapacity) : m_entries(initialCapacity)
{
	m_size = 0;
}

template <class T>
int asCSymbolTable<T>::GetFirstIndex(const asSNameSpace *ns, const asCString &name, const asIFilter &filter) const
{
...
template <class T> int asCSymbolTable<T>::GetFirstIndex(const asSNameSpace *ns, const asCString &name) const
{
	asSNameSpaceNamePair key(ns, name);
...
template <class T> int asCSymbolTable<T>::GetIndex(const T *entry) const
{
...
template <class T> T *asCSymbolTable<T>::Get(asUINT idx)
{
	if (!CheckIdx(idx))
		return 0;
...
template <class T> const T *asCSymbolTable<T>::Get(asUINT idx) const
{
	return const_cast<asCSymbolTable<T> *>(this)->Get(idx);
}

template <class T> T *asCSymbolTable<T>::GetFirst(const asSNameSpace *ns, const asCString &name)
{
	int idx = GetFirstIndex(ns, name);
	return Get(idx);
}

template <class T> const T *asCSymbolTable<T>::GetFirst(const asSNameSpace *ns, const asCString &name) const
{
	return const_cast<asCSymbolTable<T> *>(this)->GetFirst(ns, name);
}

template <class T> T *asCSymbolTable<T>::GetLast()
{
	return Get(GetLastIndex());
}

template <class T> const T *asCSymbolTable<T>::GetLast() const
{
...
template <class T> void asCSymbolTable<T>::Clear()
{
...
template <class T> void asCSymbolTable<T>::Allocate(asUINT elemCnt, bool keepData)
{
...
template <class T> bool asCSymbolTable<T>::Erase(asUINT idx)
{
	if (!CheckIdx(idx))
	{
		asASSERT(false);
...
template <class T> asUINT asCSymbolTable<T>::Put(T *entry)
{
...
template <class T> void asCSymbolTable<T>::GetKey(const T *entry, asSNameSpaceNamePair &key) const
{
	key = asSNameSpaceNamePair(entry->nameSpace, entry->name);
}

template <class T> asUINT asCSymbolTable<T>::GetSize() const
{
	return m_size;
}

template <class T> bool asCSymbolTable<T>::CheckIdx(asUINT idx) const
{
	return idx < m_entries.GetLength();
}

template <class T> int asCSymbolTable<T>::GetLastIndex() const
{
...
template <class T> asCSymbolTableIterator<T, T> asCSymbolTable<T>::List()
{
	return asCSymbolTableIterator<T, T>(this);
}

template <class T> typename asCSymbolTable<T>::const_iterator asCSymbolTable<T>::List() const
{
...
template <class T, class T2> T2 *asCSymbolTableIterator<T, T2>::operator*() const
{
...
template <class T, class T2> asCSymbolTableIterator<T, T2> &asCSymbolTableIterator<T, T2>::operator++(int)
{
	Next();
	return *this;
}
...
template <class T, class T2> asCSymbolTableIterator<T, T2>::operator bool() const
{
	return m_idx < m_table->m_entries.GetLength() && m_table->m_entries[m_idx] != 0;
}

template <class T, class T2> void asCSymbolTableIterator<T, T2>::Next()
{
	asUINT sz = m_table->m_entries.GetLength();
	m_idx++;
	while (m_idx < sz && m_table->m_entries[m_idx] == 0)
...
template <class T, class T2> void asCSymbolTableIterator<T, T2>::Previous()
{
