		...
		for (asUINT n = 0; n < descr->parameterTypes.GetLength(); n++)
		{
			...
			else
			{
				...
				if (!descr->parameterTypes[n].IsObjectHandle() &&
				    !descr->parameterTypes[n].IsReference() &&
				    descr->parameterTypes[n].GetSizeOnStackDWords() == 2 && ((dpos & 1) == mask))
				{
					// 64 bit value align
					dpos++;
					paramSize++;
...
#ifdef __GNUC__
		// On GNUC the address where the return value will be placed should be put in R0
		retQW = armFuncR0R1(args, (paramSize + 1) << 2, vftable[FuncPtrToUInt(func) >> 2], (asDWORD)retPointer,
				    (asDWORD)obj);
#else
		// On Windows the R0 should always hold the object pointer, and the address for the return value comes after
		retQW = armFuncR0R1(args, (paramSize + 1) << 2, vftable[FuncPtrToUInt(func) >> 2], (asDWORD)obj,
				    (asDWORD)retPointer);
#endif
		break;
				 ...
				 void *retPointer, asQWORD &retQW2, void *secondObject)
{
		...
		for (asUINT n = 0; n < descr->parameterTypes.GetLength(); n++)
		{
			    ...
			    !(descr->parameterTypes[n].GetTypeInfo()->flags & asOBJ_APP_ARRAY))
			{
...
#endif
				{
					...
					{
						int target =
						    (freeFloatSlot > freeDoubleSlot) ? freeFloatSlot : freeDoubleSlot;

						if (descr->parameterTypes[n].GetSizeInMemoryDWords() <=
						    ((VFP_OFFSET + 16) - target))
						{
							memcpy(&paramBuffer[target], *(void **)(args + spos),
							       descr->parameterTypes[n].GetSizeInMemoryBytes());
							memset(&paramBuffer[target + 18], (asDWORD)1,
						...
						{
							memcpy(&paramBuffer[stackPos], *(void **)(args + spos),
							       descr->parameterTypes[n].GetSizeInMemoryBytes());
			...
			else if (descr->parameterTypes[n].IsFloatType() && !descr->parameterTypes[n].IsReference())
			{
				...
				if (freeFloatSlot < (VFP_OFFSET + 16))
				{
					...
					while (freeFloatSlot < (VFP_OFFSET + 16) &&
					       paramBuffer[freeFloatSlot + 18] != 0)
						freeFloatSlot++;
				}
			...
			else if (descr->parameterTypes[n].IsDoubleType() && !descr->parameterTypes[n].IsReference())
			{
				...
				if (freeDoubleSlot < (VFP_OFFSET + 15))
				{
					...
					while (freeDoubleSlot < (VFP_OFFSET + 15) &&
					       paramBuffer[freeDoubleSlot + 18] != 0)
						freeDoubleSlot += 2;
				}
	...
	else if (descr->returnType.IsObject())
	{
		...
		if (!descr->returnType.IsObjectHandle() && !descr->returnType.IsReference() &&
