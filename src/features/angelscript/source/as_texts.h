...
#define TXT_BASE_DOESNT_HAVE_DEF_CONSTR                                                                                \
	"Base class doesn't have default constructor. Make explicit call to base constructor"
#define TXT_BASE_DOESNT_HAVE_CPY_CONSTR_OR_DEF_CONSTR                                                                  \
	"Base class doesn't have copy constructor or default constructor and assign operator. Make explicit call to "  \
	"base constructor"
...
#define TXT_COMPOUND_ASGN_REQUIRE_GET_SET                                                                              \
...
#define TXT_DEF_ARG_MISSING_IN_FUNC_s                                                                                  \
	"All subsequent parameters after the first default value must have default values in function '%s'"
#define TXT_DEF_ARG_TYPE_DOESNT_MATCH                                                                                  \
...
#define TXT_DERIVED_METHOD_MUST_HAVE_SAME_RETTYPE_s                                                                    \
	"The method in the derived class must have the same return type as in the base class: '%s'"
...
#define TXT_DISALLOW_ASSIGN_ON_REF_TYPE                                                                                \
...
#define TXT_INVALID_REF_PROP_ACCESS                                                                                    \
...
#define TXT_METHOD_s_DOES_NOT_OVERRIDE                                                                                 \
	"Method '%s' marked as override but does not replace any base class or interface method"
#define TXT_METHOD_s_s_HAS_NO_RETURN_TYPE                                                                              \
	"Method '%s::%s' is missing the return type, nor is it the same name as object to be a constructor"
...
#define TXT_NOT_A_FUNC_s_IS_TYPE_s                                                                                     \
	"Expression doesn't form a function call. '%s' evaluates to the non-function type '%s'"
...
#define TXT_ONLY_OBJECTS_MAY_USE_REF_INOUT                                                                             \
...
#define TXT_OVERLOAD_CONFLICTS_DUE_TO_DEFAULT_ARGS                                                                     \
...
#define TXT_POTENTIAL_MATCHING_NON_CONST_METHOD_HIDDEN                                                                 \
	"Potentially matching non-const method is hidden on read-only object reference"
#define TXT_PREV_ERROR_WHILE_COMP_LIST_FOR_TYPE_s                                                                      \
	"Previous error occurred while attempting to compile initialization list for type '%s'"
...
#define TXT_TOO_MANY_JUMP_LABELS                                                                                       \
...
#define TXT_AUTOHANDLE_CANNOT_BE_USED_FOR_NOCOUNT                                                                      \
	"Autohandles cannot be used with types that have been registered with NOCOUNT"
#define TXT_FIRST_PARAM_MUST_BE_REF_FOR_TEMPLATE_FACTORY                                                               \
	"First parameter to template factory must be a reference. This will be used to pass the object type of the "   \
	"template"
...
#define TXT_GC_REQUIRE_ADD_REL_GC_BEHAVIOUR                                                                            \
...
#define TXT_NON_POD_REQUIRE_CONSTR_DESTR_BEHAVIOUR                                                                     \
	"A non-pod value type must have at least one constructor and the destructor behaviours"
#define TXT_CANNOT_PASS_TYPE_s_BY_VAL                                                                                  \
	"Can't pass type '%s' by value unless the application type is informed in the registration"
#define TXT_CANNOT_RET_TYPE_s_BY_VAL                                                                                   \
	"Can't return type '%s' by value unless the application type is informed in the registration"
...
#define TXT_DONT_SUPPORT_TYPE_s_BY_VAL                                                                                 \
	"Don't support passing type '%s' by value to application in native calling convention on this platform"
...
#define TXT_DONT_SUPPORT_RET_TYPE_s_BY_VAL                                                                             \
	"Don't support returning type '%s' by value from application in native calling convention on this platform"
#define TXT_d_GC_CANNOT_FREE_OBJ_OF_TYPE_s                                                                             \
	"Object {%d}. GC cannot destroy an object of type '%s' as it doesn't know how many references to there are."
#define TXT_d_GC_CANNOT_FREE_OBJ_OF_TYPE_s_REF_COUNT_d                                                                 \
	"Object {%d}. GC cannot destroy an object of type '%s' as it can't see all references. Current ref count is "  \
	"%d."
#define TXT_OBJECT_TYPE_s_DOESNT_EXIST "Object type '%s' doesn't exist"
#define TXT_TEMPLATE_s_ALREADY_GENERATED_CANT_REGISTER                                                                 \
	"Cannot register. The template type instance '%s' has already been generated."
...
#define TXT_TEMPLATE_LIST_FACTORY_EXPECTS_2_REF_PARAMS                                                                 \
	"Template list factory expects two reference parameters. The last is the pointer to the initialization buffer"
#define TXT_LIST_FACTORY_EXPECTS_1_REF_PARAM                                                                           \
	"List factory expects only one reference parameter. The pointer to the initialization buffer will be passed "  \
	"in this parameter"
...
#define TXT_RESURRECTING_SCRIPTOBJECT_s                                                                                \
	"The script object of type '%s' is being resurrected illegally during destruction"
...
#define TXT_ENGINE_REF_COUNT_ERROR_DURING_SHUTDOWN                                                                     \
	"Uh oh! The engine's reference count is increasing while it is being destroyed. Make sure references needed "  \
	"for clean-up are immediately released"
#define TXT_MODULE_IS_IN_USE "The module is still in use and cannot be rebuilt. Discard it and request another module"
#define TXT_EXTRNL_REF_TO_MODULE_s                                                                                     \
	"There is an external reference to an object in module '%s', preventing it from being deleted"
#define TXT_FUNC_s_RELEASED_BEFORE_CLEANUP                                                                             \
	"The engine was shutdown before the context released. Function '%s' cannot be cleaned up"
...
static const char *const errorNames[] = {ERROR_NAME(asSUCCESS),
					 ERROR_NAME(asERROR),
