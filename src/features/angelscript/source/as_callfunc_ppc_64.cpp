...
#define EXTRA_STACK_SIZE                                                                                               \
	(PPC_LINKAGE_SIZE + PPC_REGSTORE_SIZE) // memory required, not including parameters, for the stack frame
#define PPC_STACK_SIZE(numParams)                                                                                      \
	(-((((((numParams) < 8) ? 8 : (numParams)) << 3) + EXTRA_STACK_SIZE + 15) &                                    \
	   ~15)) // calculates the total stack size needed for ppcFunc64, must pad to 16bytes
...
static void stackArgs(const asDWORD *args, const asBYTE *argsType, int &numIntArgs, int &numFloatArgs,
		      int &numDoubleArgs, int &numLongArgs)
{
	...
	for (i = 0, typeIndex = 0;; i++, typeIndex++)
	{
		...
		{
			// stow double
			memcpy(&ppcArgs[argWordPos], &args[i],
			       sizeof(double)); // we have to do this because of alignment
			numDoubleArgs++;
...
static asQWORD CallThisCallFunction(const void *obj, const asDWORD *pArgs, const asBYTE *pArgsType, int argSize,
				    asDWORD func, void *retInMemory)
{
	int baseArgCount = 0;
	if (retInMemory)
	{
...
static asQWORD CallThisCallFunction_objLast(const void *obj, const asDWORD *pArgs, const asBYTE *pArgsType, int argSize,
					    asDWORD func, void *retInMemory)
{
	UNUSED_VAR(argSize);
	int baseArgCount = 0;
				 ...
				 void *retPointer, asQWORD & /*retQW2*/, void * /*secondObject*/)
{
	...
	for (a = 0; a < (int)descr->parameterTypes.GetLength(); ++a)
	{
		...
		if (numBytes >= 4 || descr->parameterTypes[a].IsReference() ||
	...
	for (a = 0; a < (int)descr->parameterTypes.GetLength(); ++a, ++argIndex)
	{
		...
		if (descr->parameterTypes[a].GetSizeOnStackDWords() == 2 && !descr->parameterTypes[a].IsDoubleType() &&
