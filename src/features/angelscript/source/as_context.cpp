					...
					for (int m = 0; m < 256; m++)
					{
						if (instrCount2[n][m])
							fprintf(f, "%-10.10s, %-10.10s : %.0f\n", asBCInfo[n].name,
								asBCInfo[m].name, instrCount2[n][m]);
					}
				}
			}
			fclose(f);
...
int asCContext::PushFunction(asIScriptFunction *func, void *object)
{
	...
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_s_d, "PushFunction", errorNames[-asCONTEXT_NOT_PREPARED],
...
// interface
int asCContext::GetStateRegisters(asUINT stackLevel, asIScriptFunction **_callingSystemFunction,
				  asIScriptFunction **_initialFunction, asDWORD *_originalStackPointer,
				  asDWORD *_argumentSize, asQWORD *_valueRegister, void **_objectRegister,
				  asITypeInfo **_objectRegisterType)
{
	...
	if (_callingSystemFunction)
		*_callingSystemFunction = callingSystemFunction;
	if (_initialFunction)
		*_initialFunction = initialFunction;
	asDWORD sp = SerializeStackPointer(originalStackPointer);
	if (_originalStackPointer)
		*_originalStackPointer = sp;
	if (_argumentSize)
		*_argumentSize = argumentsSize;
	if (_valueRegister)
		*_valueRegister = valueRegister;
	if (_objectRegister)
		*_objectRegister = objectRegister;
	if (_objectRegisterType)
		*_objectRegisterType = objectType;
...
// interface
int asCContext::GetCallStateRegisters(asUINT stackLevel, asDWORD *_stackFramePointer,
				      asIScriptFunction **_currentFunction, asDWORD *_programPointer,
				      asDWORD *_stackPointer, asDWORD *_stackIndex)
{
	...
	if (_stackFramePointer)
		*_stackFramePointer =
		    sfp; // TODO: Calculate stack frame pointer as delta from previous stack frame pointer (Or perhaps it will always be the same as the stack pointer in previous function?)
	if (_currentFunction)
		*_currentFunction = currentFunction;
	if (_programPointer)
		*_programPointer = programPointer != 0
				       ? asUINT(programPointer - currentFunction->scriptData->byteCode.AddressOf())
				       : -1;
	asDWORD sp = SerializeStackPointer(stackPointer);
	if (_stackPointer)
		*_stackPointer = sp; // TODO: Calculate the stack pointer as offset from the stack frame pointer
	if (_stackIndex)
		*_stackIndex =
		    stackIndex; // TODO: This shouldn't be returned, as it should be calculated during deserialization
...
// interface
int asCContext::SetStateRegisters(asUINT stackLevel, asIScriptFunction *callingSystemFunction,
				  asIScriptFunction *initialFunction, asDWORD originalStackPointer,
				  ...
				  asITypeInfo *objectType)
{
	...
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_s_d, "SetStateRegisters", errorNames[-asCONTEXT_ACTIVE],
			   asCONTEXT_ACTIVE);
		m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
	...
	if (stackLevel == 0)
	{
		...
		m_argumentsSize =
		    argumentsSize; // TODO: Calculate this from the initialFunction so it doesn't need to be serialized
...
// interface
int asCContext::SetCallStateRegisters(asUINT stackLevel, asDWORD stackFramePointer, asIScriptFunction *_currentFunction,
				      asDWORD _programPointer, asDWORD stackPointer, asDWORD stackIndex)
{
	if (m_status != asEXECUTION_DESERIALIZATION)
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_s_d, "SetCallStateRegisters", errorNames[-asCONTEXT_ACTIVE],
			   asCONTEXT_ACTIVE);
		m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
		return asCONTEXT_ACTIVE;
	}
...
int asCContext::Prepare(asIScriptFunction *func)
{
	...
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_WITH_s_s_d, "Prepare", "null", errorNames[-asNO_FUNCTION],
			   asNO_FUNCTION);
		m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
		return asNO_FUNCTION;
	}
	...
	else
	{
		...
		if (m_engine != func->GetEngine())
		{
			asCString str;
			str.Format(TXT_FAILED_IN_FUNC_s_WITH_s_s_d, "Prepare", func->GetDeclaration(true, true),
				   errorNames[-asINVALID_ARG], asINVALID_ARG);
			m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
			return asINVALID_ARG;
		}
...
int asCContext::Execute()
{
	...
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_s_d, "Execute", errorNames[-asCONTEXT_NOT_PREPARED],
			   asCONTEXT_NOT_PREPARED);
		m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
		return asCONTEXT_NOT_PREPARED;
	}
...
{
	if (currentFunction->funcType == asFUNC_VIRTUAL || currentFunction->funcType == asFUNC_INTERFACE)
	{
...
int asCContext::PushCallState()
{
	...
	{
		if (m_engine->ep.maxCallStackSize > 0 &&
		    oldLength >= m_engine->ep.maxCallStackSize * CALLSTACK_FRAME_SIZE)
		{
			// The call stack is too big to grow further
...
#else
			asDWORD *stack =
			    asNEWARRAYALIGNED(asDWORD, (m_stackBlockSize << m_stackIndex), MAX_TYPE_ALIGNMENT);
#endif
			if (stack == 0)
			{
				// Out of memory
...
{
#if AS_USE_COMPUTED_GOTOS
	static const void *const dispatch_table[256] = {&&INSTRUCTION(asBC_PopPtr),
	...
	for (;;)
	{
		...
		*(l_fp - asBC_SWORDARG0(l_bc)) = (*(l_fp - asBC_SWORDARG0(l_bc)) == 0 ? VALUE_OF_BOOLEAN_TRUE : 0);
			...
			*(l_fp - asBC_SWORDARG0(l_bc)) = **(asDWORD **)&m_regs.valueRegister;
			...
			INSTRUCTION(asBC_JZ) : if (*(int *)&m_regs.valueRegister == 0) l_bc += asBC_INTARG(l_bc) + 2;
			else l_bc += 2;
			NEXT_INSTRUCTION();

			// Jump to a relative position if the value in the register is not 0
			INSTRUCTION(asBC_JNZ) : if (*(int *)&m_regs.valueRegister != 0) l_bc += asBC_INTARG(l_bc) + 2;
			else l_bc += 2;
			NEXT_INSTRUCTION();

			// Jump to a relative position if the value in the register is negative
			INSTRUCTION(asBC_JS) : if (*(int *)&m_regs.valueRegister < 0) l_bc += asBC_INTARG(l_bc) + 2;
			else l_bc += 2;
			NEXT_INSTRUCTION();

			// Jump to a relative position if the value in the register it not negative
			INSTRUCTION(asBC_JNS) : if (*(int *)&m_regs.valueRegister >= 0) l_bc += asBC_INTARG(l_bc) + 2;
			else l_bc += 2;
			NEXT_INSTRUCTION();

			// Jump to a relative position if the value in the register is greater than 0
			INSTRUCTION(asBC_JP) : if (*(int *)&m_regs.valueRegister > 0) l_bc += asBC_INTARG(l_bc) + 2;
			else l_bc += 2;
			NEXT_INSTRUCTION();

			// Jump to a relative position if the value in the register is not greater than 0
			INSTRUCTION(asBC_JNP) : if (*(int *)&m_regs.valueRegister <= 0) l_bc += asBC_INTARG(l_bc) + 2;
			else l_bc += 2;
			NEXT_INSTRUCTION();
			//--------------------
			// test instructions
			...
			*(asPWORD *)l_sp = *(asPWORD *)&m_regs.valueRegister;
			...
			INSTRUCTION(asBC_ALLOC) :
			{
				...
				if (objType->flags & asOBJ_SCRIPT_OBJECT)
				{
					...
					asDWORD **a	     = (asDWORD **)*(asPWORD *)(m_regs.stackPointer +
										f->GetSpaceNeededForArguments());
					if (a)
						*a = mem;
			...
			*(asPWORD *)l_sp = (asPWORD)asBC_SWORDARG0(l_bc);
			...
			INSTRUCTION(asBC_uTOd)
			    : *(double *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  double(*(asUINT *)(l_fp - asBC_SWORDARG1(l_bc)));
			l_bc += 2;
			NEXT_INSTRUCTION();

			INSTRUCTION(asBC_fTOd)
			    : *(double *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  double(*(float *)(l_fp - asBC_SWORDARG1(l_bc)));
			l_bc += 2;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_DIVf) :
			{
				...
				*(float *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    *(float *)(l_fp - asBC_SWORDARG1(l_bc)) / divider;
			}
			l_bc += 2;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_MODf) :
			{
				...
				*(float *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    fmodf(*(float *)(l_fp - asBC_SWORDARG1(l_bc)), divider);
			}
			l_bc += 2;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_DIVd) :
			{
				...
				*(double *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    *(double *)(l_fp - asBC_SWORDARG1(l_bc)) / divider;
				l_bc += 2;
			}
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_uTOi64)
			    : *(asINT64 *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  asINT64(*(asUINT *)(l_fp - asBC_SWORDARG1(l_bc)));
			l_bc += 2;
			NEXT_INSTRUCTION();

			INSTRUCTION(asBC_iTOi64)
			    : *(asINT64 *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  asINT64(*(int *)(l_fp - asBC_SWORDARG1(l_bc)));
			l_bc += 2;
			NEXT_INSTRUCTION();

			INSTRUCTION(asBC_fTOi64)
			    : *(asINT64 *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  asINT64(*(float *)(l_fp - asBC_SWORDARG1(l_bc)));
			l_bc += 2;
			NEXT_INSTRUCTION();

			INSTRUCTION(asBC_dTOi64)
			    : *(asINT64 *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  asINT64(*(double *)(l_fp - asBC_SWORDARG0(l_bc)));
			l_bc++;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_i64TOf)
			    : *(float *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  float(*(asINT64 *)(l_fp - asBC_SWORDARG1(l_bc)));
			l_bc += 2;
			NEXT_INSTRUCTION();
		...
		*(float*)(l_fp - asBC_SWORDARG0(l_bc)) = float(*(asQWORD*)(l_fp - asBC_SWORDARG1(l_bc)));
			...
			INSTRUCTION(asBC_i64TOd)
			    : *(double *)(l_fp - asBC_SWORDARG0(l_bc)) =
				  double(*(asINT64 *)(l_fp - asBC_SWORDARG0(l_bc)));
			l_bc++;
			NEXT_INSTRUCTION();
		...
		*(double*)(l_fp - asBC_SWORDARG0(l_bc)) = double(*(asQWORD*)(l_fp - asBC_SWORDARG0(l_bc)));
			...
			INSTRUCTION(asBC_DIVi64) :
			{
				...
				*(asINT64 *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    *(asINT64 *)(l_fp - asBC_SWORDARG1(l_bc)) / divider;
			}
			l_bc += 2;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_CallPtr) :
			{
				...
				else
				{
					...
					else if (func->funcType == asFUNC_DELEGATE)
					{
						...
						asASSERT(m_regs.stackPointer - AS_PTR_SIZE >=
							 m_stackBlocks[m_stackIndex]);
						m_regs.stackPointer -= AS_PTR_SIZE;
						*(asPWORD *)m_regs.stackPointer = asPWORD(func->objForDelegate);
						...
						{
							m_regs.stackPointer +=
							    CallSystemFunction(func->funcForDelegate->id, this);
					...
					{
						m_regs.programPointer++;
						int funcId = m_engine->importedFunctions[func->id & ~FUNC_IMPORTED]
								 ->boundFunctionId;
						if (funcId > 0)
			...
			*(asQWORD *)l_sp = *(asQWORD *)(l_fp - asBC_SWORDARG0(l_bc));
			...
			INSTRUCTION(asBC_DIVu) :
			{
				...
				*(asUINT *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    *(asUINT *)(l_fp - asBC_SWORDARG1(l_bc)) / divider;
			}
			l_bc += 2;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_DIVu64) :
			{
				...
				*(asQWORD *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    *(asQWORD *)(l_fp - asBC_SWORDARG1(l_bc)) / divider;
			}
			l_bc += 2;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_JLowZ)
			    : if (*(asBYTE *)&m_regs.valueRegister == 0) l_bc += asBC_INTARG(l_bc) + 2;
			else l_bc += 2;
			NEXT_INSTRUCTION();
			...
			INSTRUCTION(asBC_POWi) :
			{
				bool isOverflow;
				*(int *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    as_powi(*(int *)(l_fp - asBC_SWORDARG1(l_bc)),
					    *(int *)(l_fp - asBC_SWORDARG2(l_bc)), isOverflow);
				if (isOverflow)
				{
					// Need to move the values back to the context
					m_regs.programPointer	 = l_bc;
			...
			INSTRUCTION(asBC_POWu) :
			{
				...
				*(asDWORD *)(l_fp - asBC_SWORDARG0(l_bc)) =
				    as_powu(*(asDWORD *)(l_fp - asBC_SWORDARG1(l_bc)),
					    *(asDWORD *)(l_fp - asBC_SWORDARG2(l_bc)), isOverflow);
				if (isOverflow)
				{
					// Need to move the values back to the context
					m_regs.programPointer	 = l_bc;
			...
			INSTRUCTION(asBC_POWf) :
			{
				float r					= powf(*(float *)(l_fp - asBC_SWORDARG1(l_bc)),
									       *(float *)(l_fp - asBC_SWORDARG2(l_bc)));
			...
			INSTRUCTION(asBC_POWd) :
			{
				double r				 = pow(*(double *)(l_fp - asBC_SWORDARG1(l_bc)),
									       *(double *)(l_fp - asBC_SWORDARG2(l_bc)));
			...
			//  void  obj::func(uint)
			{
				...
				else
				{
					...
					{
						ptr =
						    m_engine->CallObjectMethodRetPtr(obj, arg, m_callingSystemFunction);
					}
					catch (...)
					{
		...
		if (instr != asBC_JMP && instr != asBC_JMPP && (instr < asBC_JZ || instr > asBC_JNP) &&
		    instr != asBC_JLowZ && instr != asBC_JLowNZ && instr != asBC_CALL && instr != asBC_CALLBND &&
		    instr != asBC_CALLINTF && instr != asBC_RET && instr != asBC_ALLOC && instr != asBC_CallPtr &&
		    instr != asBC_JitEntry)
		{
	...
	if (m_regs.objectType)
	{
		...
		else
		{
			...
			asSTypeBehaviour *beh =
			    &(CastToObjectType(reinterpret_cast<asCTypeInfo *>(m_regs.objectType))->beh);
...
bool asCContext::IsVarInScope(asUINT varIndex, asUINT stackLevel)
{
	...
	for (int n = 0; n < (int)func->scriptData->objVariableInfo.GetLength(); n++)
	{
		...
		if (func->scriptData->objVariableInfo[n].programPos >= declaredAt)
		{
			...
			if (func->scriptData->objVariableInfo[n].programPos == declaredAt && !foundVarDecl &&
			...
			for (--n; n >= 0; n--)
			{
				...
				case asOBJ_UNINIT: // Object was destroyed
				{
					...
					for (asUINT v = 0; v < func->scriptData->variables.GetLength(); v++)
						if (func->scriptData->variables[v]->stackOffset ==
	...
	else if (bc == asBC_CallPtr)
	{
		...
		for (v = 0; v < m_currentFunction->scriptData->variables.GetLength(); v++)
			if (m_currentFunction->scriptData->variables[v]->stackOffset == var)
			{
				...
				func =
				    CastToFuncdefType(m_currentFunction->scriptData->variables[v]->type.GetTypeInfo())
					->funcdef;
				break;
			}
			...
			for (v = 0; v < m_currentFunction->parameterTypes.GetLength(); v++)
			{
				if (var == paramPos)
				{
					if (m_currentFunction->parameterTypes[v].IsFuncdef())
						func = CastToFuncdefType(
							   m_currentFunction->parameterTypes[v].GetTypeInfo())
							   ->funcdef;
					break;
				}
	...
	for (asUINT n = 0; n < func->parameterTypes.GetLength(); n++)
	{
		    ...
		    !func->parameterTypes[n].IsReference())
		{
			...
			if (*(asPWORD *)&m_regs.stackPointer[offset])
			{
				...
				{
					asASSERT((func->parameterTypes[n].GetTypeInfo()->flags & asOBJ_NOCOUNT) ||
						 beh->release);

					if (beh->release)
						m_engine->CallObjectMethod(
						    (void *)*(asPWORD *)&m_regs.stackPointer[offset], beh->release);
				}
				else
				{
					if (beh->destruct)
						m_engine->CallObjectMethod(
						    (void *)*(asPWORD *)&m_regs.stackPointer[offset], beh->destruct);
...
bool asCContext::CleanStackFrame(bool catchException)
{
	...
	if (!m_isStackMemoryNotAllocated && m_regs.programPointer)
	{
		...
		{
			asUINT currPos =
			    asUINT(m_regs.programPointer - m_currentFunction->scriptData->byteCode.AddressOf());
		...
		for (asUINT n = 0; n < m_currentFunction->scriptData->variables.GetLength(); n++)
		{
			...
			if (m_currentFunction->scriptData->variables[n]->onHeap)
			{
				...
				if (*(asPWORD *)&m_regs.stackFramePointer[-pos])
				{
					...
					if (m_currentFunction->scriptData->variables[n]->type.GetTypeInfo() &&
					    !m_currentFunction->scriptData->variables[n]->type.IsReference())
					{
						// Call the object's destructor
						if (m_currentFunction->scriptData->variables[n]
							->type.GetTypeInfo()
							->flags &
						    asOBJ_FUNCDEF)
						{
							(*(asCScriptFunction **)&m_regs.stackFramePointer[-pos])
							    ->Release();
						}
						else if (m_currentFunction->scriptData->variables[n]
							     ->type.GetTypeInfo()
							     ->flags &
							 asOBJ_REF)
						{
							asSTypeBehaviour *beh =
							    &CastToObjectType(
								 m_currentFunction->scriptData->variables[n]
								     ->type.GetTypeInfo())
								 ->beh;
							asASSERT((m_currentFunction->scriptData->variables[n]
								      ->type.GetTypeInfo()
								      ->flags &
								  asOBJ_NOCOUNT) ||
								 beh->release);
							if (beh->release)
								m_engine->CallObjectMethod(
								    (void *)*(asPWORD *)&m_regs.stackFramePointer[-pos],
								    beh->release);
						}
						else
						{
							...
							if (beh->destruct)
								...
								m_engine->DestroyList(
								    (asBYTE *)*(
									asPWORD *)&m_regs.stackFramePointer[-pos],
								    CastToObjectType(
									m_currentFunction->scriptData->variables[n]
									    ->type.GetTypeInfo()));
			...
			else
			{
				...
				if (liveObjects[n] > 0)
				{
					asASSERT(m_currentFunction->scriptData->variables[n]->type.GetTypeInfo() &&
						 m_currentFunction->scriptData->variables[n]
							 ->type.GetTypeInfo()
							 ->GetFlags() &
						     asOBJ_VALUE);
					...
					asSTypeBehaviour *beh =
					    &CastToObjectType(
						 m_currentFunction->scriptData->variables[n]->type.GetTypeInfo())
						 ->beh;
					if (beh->destruct)
						m_engine->CallObjectMethod(
						    (void *)(asPWORD *)&m_regs.stackFramePointer[-pos], beh->destruct);
				}
			}
		}
	}
	...
	{
		m_regs.stackPointer =
		    m_regs.stackFramePointer - tryCatchInfo->stackSize - m_currentFunction->scriptData->variableSpace;
	...
	{
		if ((m_currentFunction->parameterTypes[n].IsObject() ||
		     m_currentFunction->parameterTypes[n].IsFuncdef()) &&
		    !m_currentFunction->parameterTypes[n].IsReference())
		{
			if (*(asPWORD *)&m_regs.stackFramePointer[offset])
			{
				...
				{
					asASSERT((m_currentFunction->parameterTypes[n].GetTypeInfo()->flags &
						  asOBJ_NOCOUNT) ||
						 beh->release);

					if (beh->release)
						m_engine->CallObjectMethod(
						    (void *)*(asPWORD *)&m_regs.stackFramePointer[offset],
						    beh->release);
				}
				else
				{
...
void asCContext::HandleAppException()
{
	...
	if (m_engine->translateExceptionCallback)
	{
		...
		if (m_engine->translateExceptionCallbackFunc.callConv < ICC_THISCALL)
			m_engine->CallGlobalFunction(this, m_engine->translateExceptionCallbackObj,
						     &m_engine->translateExceptionCallbackFunc, 0);
		else
			m_engine->CallObjectMethod(m_engine->translateExceptionCallbackObj, this,
						   &m_engine->translateExceptionCallbackFunc, 0);
	}
...
// interface
int asCContext::GetVar(asUINT varIndex, asUINT stackLevel, const char **name, int *typeId,
		       asETypeModifiers *typeModifiers, bool *isVarOnHeap, int *stackOffset)
{
	...
	if (typeModifiers)
	{
		*typeModifiers = asTM_NONE;
		...
		if (func->scriptData && func->scriptData->variables[varIndex]->type.IsReadOnly())
		{
...
int asCContext::StartDeserialization()
{
	...
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_s_d, "StartDeserialization", errorNames[-asCONTEXT_ACTIVE],
			   asCONTEXT_ACTIVE);
		m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
		return asCONTEXT_ACTIVE;
	}
...
// internal
int asCContext::DeserializeProgramPointer(int programPointer, asCScriptFunction *currentFunction, void *object,
					  asDWORD *&p, asCScriptFunction *&realFunc)
{
...
int asCContext::FinishDeserialization()
{
	...
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_s_d, "FinishDeserialization", errorNames[-asCONTEXT_NOT_PREPARED],
			   asCONTEXT_NOT_PREPARED);
		m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
		return asCONTEXT_NOT_PREPARED;
	}
	...
	{
		asCString str;
		str.Format(TXT_FAILED_IN_FUNC_s_WITH_s_s_d, "FinishDeserialization", "No function set",
			   errorNames[-asCONTEXT_NOT_PREPARED], asCONTEXT_NOT_PREPARED);
		m_engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
...
int asCContext::GetArgsOnStackCount(asUINT stackLevel)
{
	...
	if (calledFunc)
		stackPos -= calledFunc->GetSpaceNeededForArguments() +
			    (calledFunc->DoesReturnOnStack() ? AS_PTR_SIZE : 0) +
			    (calledFunc->GetObjectType() ? AS_PTR_SIZE : 0);
	...
	while (stackPos > 0)
	{
		...
		while (stackPos > 0)
		{
			if (++param < int(calledFunc->GetParamCount()))
			{
				...
				else if (typeId == asTYPEID_UINT64 || typeId == asTYPEID_INT64 ||
...
int asCContext::GetArgOnStack(asUINT stackLevel, asUINT arg, int *outTypeId, asUINT *outFlags, void **outAddress)
{
	...
	if (calledFunc)
		sp += calledFunc->GetSpaceNeededForArguments() + (calledFunc->DoesReturnOnStack() ? AS_PTR_SIZE : 0) +
		      (calledFunc->GetObjectType() ? AS_PTR_SIZE : 0);

	// Check that the cache for GetArgsOnStack is up-to-date
	if (m_argsOnStackCacheFunc != func ||
	    m_argsOnStackCacheProgPos != asUINT(progPointer - &func->scriptData->byteCode[0]))
	...
	if (outAddress)
		*outAddress = sp + stackDelta;
	if (outTypeId)
		*outTypeId = m_argsOnStackCache[arg * 2];
	if (outFlags)
		*outFlags = m_argsOnStackCache[arg * 2 + 1];
