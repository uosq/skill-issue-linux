...
int asCGarbageCollector::ReportAndReleaseUndestroyedObjects()
{
	...
	for (asUINT n = 0; n < gcOldObjects.GetLength(); n++)
	{
		...
		asCString msg;
		msg.Format(TXT_d_GC_CANNOT_FREE_OBJ_OF_TYPE_s_REF_COUNT_d, gcObj.seqNbr, gcObj.type->name.AddressOf(),
			   refCount - 1);
		...
		if (gcObj.type->name == "$func")
		{
			...
			msg.Format(TXT_PREV_FUNC_IS_NAMED_s_TYPE_IS_d,
				   reinterpret_cast<asCScriptFunction *>(gcObj.obj)->GetName(),
				   reinterpret_cast<asCScriptFunction *>(gcObj.obj)->GetFuncType());
		...
		case destroyGarbage_haveMore:
		{
			...
			if (++destroyOldIdx < gcOldObjects.GetLength())
			{
				...
				if (gcObj.type->beh.gcGetRefCount == 0)
				{
					...
					asCString msg;
					msg.Format(TXT_d_GC_CANNOT_FREE_OBJ_OF_TYPE_s, gcObj.seqNbr,
						   gcObj.type->name.AddressOf());
					engine->WriteMessage("", 0, 0, asMSGTYPE_ERROR, msg.AddressOf());
		...
		case buildMap_loop:
		{
			...
			if (detectIdx < gcOldObjects.GetLength())
			{
				...
				if (gcObj.type->beh.gcGetRefCount)
					refCount =
					    engine->CallObjectMethodRetInt(gcObj.obj, gcObj.type->beh.gcGetRefCount);
