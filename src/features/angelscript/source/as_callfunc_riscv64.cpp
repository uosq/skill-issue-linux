...
extern "C" asDBLQWORD CallRiscVFunc(asFUNCTION_t func, int retfloat, asQWORD *argValues, int numRegularValues,
				    int numFloatValues, int numStackValues);
...
bool PushToFloatRegs(asQWORD val, asQWORD *argValues, asUINT &numFloatRegistersUsed, asUINT &numRegularRegistersUsed,
		     asUINT &numStackValuesUsed)
{
				 ...
				 void *retPointer, asQWORD &retQW2, void *secondObj)
{
	...
	if (callConv == ICC_CDECL_OBJFIRST || callConv == ICC_CDECL_OBJFIRST_RETURNINMEM || callConv == ICC_THISCALL ||
	    callConv == ICC_VIRTUAL_THISCALL || callConv == ICC_THISCALL_RETURNINMEM ||
	    callConv == ICC_VIRTUAL_THISCALL_RETURNINMEM || callConv == ICC_THISCALL_OBJLAST ||
	    callConv == ICC_THISCALL_OBJLAST_RETURNINMEM || callConv == ICC_VIRTUAL_THISCALL_OBJLAST ||
	...
	else if (callConv == ICC_THISCALL_OBJFIRST || callConv == ICC_VIRTUAL_THISCALL_OBJFIRST ||
	...
	for (asUINT n = 0; n < descr->parameterTypes.GetLength(); n++)
	{
		...
		if (parmType.IsReference() || parmType.IsObjectHandle() || parmType.IsIntegerType() ||
		    parmType.IsUnsignedType() || parmType.IsBooleanType())
		{
			...
			if (parmType.GetTokenType() == ttQuestion)
			{
				...
				PushToRegularRegs(*(asQWORD *)&args[argsPos], argValues, numRegularRegistersUsed,
						  numStackValuesUsed);
				PushToRegularRegs((asQWORD)args[argsPos + AS_PTR_SIZE], argValues,
						  numRegularRegistersUsed, numStackValuesUsed);
			}
			else
			{
				if (parmDWords == 1)
					...
					PushToRegularRegs(*(asQWORD *)&args[argsPos], argValues,
							  numRegularRegistersUsed, numStackValuesUsed);
			}
		}
		else if (parmType.IsFloatType() || parmType.IsDoubleType())
		{
			...
			if (parmDWords == 1)
				PushToFloatRegs(0xFFFFFFFF00000000ull | (asQWORD)args[argsPos], argValues,
						numFloatRegistersUsed, numRegularRegistersUsed, numStackValuesUsed);
			else
				PushToFloatRegs(*(asQWORD *)&args[argsPos], argValues, numFloatRegistersUsed,
						numRegularRegistersUsed, numStackValuesUsed);
		}
		else if (parmType.IsObject())
		{
			if (parmType.GetTypeInfo()->flags & COMPLEX_MASK)
			{
				...
				PushToRegularRegs(*(asQWORD *)&args[argsPos], argValues, numRegularRegistersUsed,
				 ...
				 !(parmType.GetTypeInfo()->flags & asOBJ_APP_CLASS_UNION) &&
				 ((parmType.GetSizeInMemoryDWords() <= 2 &&
				   !(parmType.GetTypeInfo()->flags & asOBJ_APP_CLASS_ALIGN8)) ||
				   ...
				   (parmType.GetTypeInfo()->flags & asOBJ_APP_CLASS_ALIGN8))))
			{
				...
				if (!(parmType.GetTypeInfo()->flags & asOBJ_APP_CLASS_ALIGN8))
				{
					...
					PushToFloatRegs(arg1, argValues, numFloatRegistersUsed, numRegularRegistersUsed,
							numStackValuesUsed);
					PushToFloatRegs(arg2, argValues, numFloatRegistersUsed, numRegularRegistersUsed,
							numStackValuesUsed);
				}
				else
				{
			...
			else
			{
				...
				if ((maxRegularRegisters - numRegularRegistersUsed) > parmQWords)
				{
					if (sizeInMemoryDWords == 1)
						argValues[numRegularRegistersUsed] =
						    (asQWORD) * *(asDWORD **)&args[argsPos];
					else
						memcpy(&argValues[numRegularRegistersUsed], *(void **)&args[argsPos],
						       sizeInMemoryDWords * 4);
					numRegularRegistersUsed += parmQWords;
				}
				else if ((maxValuesOnStack - numStackValuesUsed) > parmQWords)
				{
					if (sizeInMemoryDWords == 1)
						stackValues[numStackValuesUsed] =
						    (asQWORD) * *(asDWORD **)&args[argsPos];
					else
						memcpy(&stackValues[numStackValuesUsed], *(void **)&args[argsPos],
						       sizeInMemoryDWords * 4);
	...
	asDBLQWORD ret = CallRiscVFunc(func, retfloat, argValues, numRegularRegistersUsed, numFloatRegistersUsed,
				       numStackValuesUsed);
	retQW2	       = ret.qw2;
	...
	if (retfloat && retTypeInfo && !(retTypeInfo->flags & asOBJ_APP_CLASS_ALIGN8) &&
	    retTypeInfo->flags & asOBJ_APP_CLASS_ALLFLOATS)
	{
		ret.qw1 &= 0xFFFFFFFF;
		ret.qw1 |= (retQW2 << 32);
	}
