				 ...
				 void *retPointer, asQWORD & /*retQW2*/, void *secondObject)
{
	...
	if (callConv == ICC_CDECL_OBJFIRST || callConv == ICC_CDECL_OBJFIRST_RETURNINMEM || callConv == ICC_THISCALL ||
	    callConv == ICC_THISCALL_RETURNINMEM || callConv == ICC_VIRTUAL_THISCALL ||
	    callConv == ICC_VIRTUAL_THISCALL_RETURNINMEM || callConv == ICC_THISCALL_OBJFIRST ||
	    callConv == ICC_VIRTUAL_THISCALL_OBJFIRST || callConv == ICC_THISCALL_OBJFIRST_RETURNINMEM ||
	    callConv == ICC_VIRTUAL_THISCALL_OBJFIRST_RETURNINMEM || callConv == ICC_THISCALL_OBJLAST ||
	    callConv == ICC_VIRTUAL_THISCALL_OBJLAST || callConv == ICC_THISCALL_OBJLAST_RETURNINMEM ||
	...
	for (asUINT n = 0; n < descr->parameterTypes.GetLength(); n++)
	{
		...
		if (paramType.IsObject() && !paramType.IsObjectHandle() && !paramType.IsReference())
		{
			...
			else
			{
				...
				memcpy(&argBuffer[argOffset], *(void **)(args + spos),
				       paramType.GetSizeInMemoryBytes());
		...
		else
		{
			...
			if (paramType.GetSizeOnStackDWords() > 1)
			{
				...
				if (argOffset & 1)
					argOffset++;
				*reinterpret_cast<asQWORD *>(&argBuffer[argOffset]) =
				    *reinterpret_cast<asQWORD *>(&args[spos]);
				argOffset += 2;
				spos += 2;
			}
			else
