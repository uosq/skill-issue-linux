...
X64_CallFunction(const asQWORD *args, int cnt, funcptr_t func, asQWORD &retQW2, bool returnFloat)
{
	...
	__asm__ __volatile__(
	    ...
	    : "%xmm0", "%xmm1", "%xmm2", "%xmm3", "%xmm4", "%xmm5", "%xmm6", "%xmm7", "%rdi", "%rsi", "%rax", "%rdx",
	      "%rcx", "%r8", "%r9", "%r10", "%r11", "%r15");
				 ...
				 void *retPointer, asQWORD &retQW2, void *secondObject)
{
	...
	for (int a = 0; a < argumentCount; ++a)
	{
		...
		else
		{
			...
			if ((parmType.GetTypeInfo()->flags & COMPLEX_MASK) || parmType.GetSizeInMemoryDWords() > 4)
			{
				 ...
				 (parmType.GetTypeInfo()->flags & asOBJ_APP_PRIMITIVE))
			{
				...
				if (parmType.GetSizeInMemoryDWords() > 2)
				{
					argsType[argIndex]     = x64INTARG;
					argsType[argIndex + 1] = x64INTARG;
					memcpy(paramBuffer + argIndex, *(asDWORD **)stack_pointer,
					       parmType.GetSizeInMemoryBytes());
					argIndex += 2;
				}
				else
				{
