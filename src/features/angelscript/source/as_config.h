...
#define COMPLEX_RETURN_MASK                                                                                            \
	(asOBJ_APP_CLASS_CONSTRUCTOR | asOBJ_APP_CLASS_DESTRUCTOR | asOBJ_APP_CLASS_ASSIGNMENT |                       \
	 asOBJ_APP_CLASS_COPY_CONSTRUCTOR | asOBJ_APP_ARRAY | asOBJ_APP_CLASS_MORE_CONSTRUCTORS)
...
#define COMPLEX_RETURN_MASK                                                                                            \
	(asOBJ_APP_CLASS_CONSTRUCTOR | asOBJ_APP_CLASS_DESTRUCTOR | asOBJ_APP_CLASS_ASSIGNMENT | asOBJ_APP_ARRAY |     \
...
// Metrowerks CodeWarrior (experimental, let me know if something isn't working)
    ...
    !defined(                                                                                                          \
	EPPC) // JWC -- If Wii DO NOT use this even when using Metrowerks Compiler. Even though they are called Freescale...
#define MULTI_BASE_OFFSET(x) (*((asDWORD *)(&x) + 1))
#define HAVE_VIRTUAL_BASE_OFFSET
#define VIRTUAL_BASE_OFFSET(x) (*((asDWORD *)(&x) + 3))
#define THISCALL_RETURN_SIMPLE_IN_MEMORY
...
#if (defined(__GNUC__) && !defined(__SNC__) && !defined(_MSC_VER)) || defined(EPPC) ||                                 \
    defined(__CYGWIN__) // JWC -- use this instead for Wii
#define GNU_STYLE_VIRTUAL_METHOD
...
#if !defined(AS_BIG_ENDIAN) && defined(__BYTE_ORDER) && __BYTE_ORDER == __BIG_ENDIAN || defined(__BIG_ENDIAN__) ||     \
...
// conventions, then compile with AS_MAX_PORTABILITY
     ...
     !defined(AS_XENON) && !defined(AS_X64_GCC) && !defined(AS_X64_MSVC) && !defined(AS_ARM) && !defined(AS_ARM64) &&  \
     !defined(AS_X64_MINGW) && !defined(AS_RISCV64) && !defined(AS_E2K))
#ifndef AS_MAX_PORTABILITY
#define AS_MAX_PORTABILITY
#endif
#endif
...
#define asASSERT(x)                                                                                                    \
	...
	{                                                                                                              \
		if (!(x))                                                                                              \
		{                                                                                                      \
			__android_log_print(ANDROID_LOG_ERROR, "AngelScript", "Assert failed at %s:%d - %s", __FILE__, \
					    __LINE__, #x);                                                             \
			exit(1);                                                                                       \
		}                                                                                                      \
	} while (0)
