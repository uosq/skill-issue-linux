...
namespace AimbotMelee
{
	void Run (CTFPlayer *pLocal, CTFWeaponBase *pWeapon, CUserCmd *pCmd,
		  AimbotState &state)
	{
		if (Settings::Aimbot.melee
			== static_cast<int> (MeleeMode::NONE)
		    || pWeapon->GetWeaponID () == TF_WEAPON_KNIFE)
			return;

		std::vector<PotentialTarget> targets;

		bool bIsSword	= static_cast<int> (AttributeHookValue (
		    0, "is_a_sword", pWeapon, nullptr, true));
		...
		float fBoundsScale = AttributeHookValue (
		    1.0f, "melee_bounds_multiplier", pWeapon, nullptr, true);
		swingMins.Set (-18 * fBoundsScale, -18 * fBoundsScale,
			       -18 * fBoundsScale);
		swingMaxs.Set (18 * fBoundsScale, 18 * fBoundsScale,
			       18 * fBoundsScale);
		...
		MeleeMode mode
		    = static_cast<MeleeMode> (Settings::Aimbot.melee);
		...
		for (const EntityListEntry &entry :
		     AimbotUtils::GetTargets (bCanHitTeammates, localTeam))
		{
			...
			helper::engine::TraceHull (
			    shootPos, shootPos + (dir * range), swingMins,
			    swingMaxs, MASK_SHOT_HULL, &filter, &trace);
		...
		if (helper::localplayer::IsAttacking (pLocal, pWeapon, pCmd))
		{
			...
			if (Settings::Aimbot.mode
			    == static_cast<int> (AimbotMode::SILENT))
