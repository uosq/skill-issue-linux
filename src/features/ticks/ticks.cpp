...
ADD_SIG (net_time_addr, "engine.so",
	 "48 8D 05 ? ? ? ? 66 0F EF C9 F3 0F 5A 8F DC 93 00 00")
ADD_SIG (host_frametime_unbounded_addr, "engine.so",
	 "48 8D 05 ? ? ? ? F3 0F 10 08 48 8B 07 48 8B 40 ?")
ADD_SIG (host_frametime_stddeviation_addr, "engine.so",
	 "48 8D 05 ? ? ? ? 48 8D 15 ? ? ? ? 49 8B BC 24 ? ? ? ?")
ADD_SIG (host_state_addr, "engine.so", "48 8D 05 ? ? ? ? 48 83 38 00 74 ? E9")
ADD_SIG (Con_NXPrintf, "engine.so",
	 "55 49 89 F2 48 89 E5 41 55 41 54 49 89 FC 48 81 EC D0 10 00 00")
ADD_SIG (Host_ShouldRun, "engine.so",
	 "48 8B 15 ? ? ? ? B8 01 00 00 00 8B 72 58")
...
{
	CUserCmd *pCmd = &(static_cast<IInput *> (interfaces::CInput))
			      ->m_pCommands[sequence_number % 90];
	if (pCmd == nullptr)
		return;
	...
	if (reinterpret_cast<CClientState *> (interfaces::ClientState)
		->chokedcommands
	    >= 21)
		m_bSendPacket = true;
...
void TickManager::CL_SendMove (void)
{
	...
	moveMsg.m_nBackupCommands
	    = std::clamp (0, backupcmds, MAX_BACKUP_COMMANDS);
	...
	{
		bool isnewcmd
		    = to >= (nextcommandnr - moveMsg.m_nNewCommands + 1);

		// first valid command number is 1
		bOK  = bOK
		       && interfaces::ClientDLL->WriteUsercmdDeltaToBuffer (
			   &moveMsg.m_DataOut, from, to, isnewcmd);
		from = to;
	}
	...
	{
		if (extracmds)
			static_cast<CNetChannel *> (cl->m_NetChannel)
			    ->m_nChokedPackets -= extracmds;
...
void TickManager::CL_Move (float accumulated_extra_samples, bool bFinalTick)
{
	...
	static ConVar *host_limitlocal
	    = interfaces::Cvar->FindVar ("host_limitlocal");
	static ConVar *cl_cmdrate = interfaces::Cvar->FindVar ("cl_cmdrate");

	// returns the right thing
	double net_time = *reinterpret_cast<double *> (vtable::ResolveRIP (
	    uintptr_t (Sigs::net_time_addr.GetPointer ()), 3, 7));
	float host_frametime_unbounded
	    = *reinterpret_cast<float *> (vtable::ResolveRIP (
		uintptr_t (Sigs::host_frametime_unbounded_addr.GetPointer ()),
		3, 7));
	float host_frametime_stddeviation
	    = *reinterpret_cast<float *> (vtable::ResolveRIP (
		uintptr_t (
		    Sigs::host_frametime_stddeviation_addr.GetPointer ()),
		3, 7));
	CCommonHostState *host_state
	    = reinterpret_cast<CCommonHostState *> (vtable::ResolveRIP (
		uintptr_t (Sigs::host_state_addr.GetPointer ()), 3, 7));
	...
	if (!reinterpret_cast<Host_ShouldRunFn> (
		Sigs::Host_ShouldRun.GetPointer ()) ())
		return;
	...
	if ((!cl->m_NetChannel->IsLoopback () || host_limitlocal->GetInt ())
	    && ((net_time < cl->m_flNextCmdTime)
		|| !cl->m_NetChannel->CanPacket () || !bFinalTick))
	{
		m_bSendPacket = false;
	}
	...
	{
		int nextcommandnr
		    = cl->lastoutgoingcommand + m_iChokedCommands + 1;
	...
	{
		using Con_NXPrintf
		    = void (*) (const con_nprint_t *, const char *fmt, ...);
		static auto original = reinterpret_cast<Con_NXPrintf> (
		    Sigs::Con_NXPrintf.GetPointer ());
		...
		float flRemainingTime
		    = flTimeOut
		      - cl->m_NetChannel->GetTimeSinceLastReceived ();

		original (&np, "WARNING:  Connection Problem");

		np.index = 3;
		original (&np, "Auto-disconnect in %.1f seconds",
			  flRemainingTime);
	...
	if (cl->m_nSignonState == SIGNONSTATE_FULL)
	{
		...
		const float maxDelta = std::min (host_state->interval_per_tick,
						 commandInterval);
		const float delta    = std::clamp (
		    static_cast<float> (net_time - cl->m_flNextCmdTime), 0.0f,
		    maxDelta);
...
void TickManager::Run (float accumulated_extra_samples, bool bFinalTick)
{
	...
	if (Warp::m_iDesiredState == WarpState::RECHARGING
	    && Warp::m_iStoredTicks < Warp::GetMaxTicks ())
	{
		Warp::m_iStoredTicks++;
	...
	if (Warp::m_iDesiredState == WarpState::RUNNING
