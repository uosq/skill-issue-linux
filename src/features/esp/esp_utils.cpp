...
bool ESP_Utils::IsValidPlayer (CTFPlayer *pLocal, CBaseEntity *entity)
{
	...
	if (!interfaces::CInput->CAM_IsThirdPerson ()
	    && entity->GetIndex () == pLocal->GetIndex ())
		return false;
	...
	const ESPTeamSelectionMode mode
	    = static_cast<ESPTeamSelectionMode> (Settings::ESP.team_selection);
...
bool ESP_Utils::IsValidBuilding (CTFPlayer *pLocal, CBaseObject *entity)
{
	...
	if (mode >= ESPTeamSelectionMode::MAX
	    || mode <= ESPTeamSelectionMode::INVALID)
		return false;
...
bool ESP_Utils::IsValidEntity (CTFPlayer *pLocal, const EntityListEntry &entry)
{
	...
	if (entry.flags & EntityFlags::IsPlayer)
		return IsValidPlayer (pLocal,
				      static_cast<CTFPlayer *> (entry.ptr));

	if (entry.flags & EntityFlags::IsBuilding)
		return IsValidBuilding (
		    pLocal, static_cast<CBaseObject *> (entry.ptr));
