...
class HealthTextElement : public IBaseElement
{
	...
	{
		helper::draw::TextShadow (
		    pos.x, pos.y, GetColor (ent, data),
		    std::to_string (data.health) + "/"
			+ std::to_string (data.maxhealth));
	}

	Vec2 GetSize (const ESP_Data &data) const override
	{
		int w, h;
		helper::draw::GetTextSize (
		    std::to_string (data.health) + "/"
			+ std::to_string (data.maxhealth),
		    w, h);
		return Vec2 (w, h);
	...
	Color GetColor (CBaseEntity *pEnt, const ESP_Data &data) const override
	{
		...
		float ratio = static_cast<float> (data.health)
