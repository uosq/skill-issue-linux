...
namespace ESP
{
	...
	void Init ()
	{
		constexpr int iFONT_FLAGS
		    = EFONTFLAG_CUSTOM | EFONTFLAG_ANTIALIAS;

		FontManager::CreateFont ("esp font tf2", "TF2 BUILD", 16, 400,
					 iFONT_FLAGS);
		FontManager::CreateFont ("esp font arial", "Arial", 16, 400,
					 iFONT_FLAGS);

		m_builtinElements.reserve (6);

		m_builtinElements.push_back (std::make_unique<NameElement> ());
		m_builtinElements.push_back (
		    std::make_unique<HealthTextElement> ());
		m_builtinElements.push_back (
		    std::make_unique<ZoomedCondElement> ());
		m_builtinElements.push_back (
		    std::make_unique<UberedCondElement> ());
		m_builtinElements.push_back (
		    std::make_unique<JaratedCondElement> ());
		m_builtinElements.push_back (
		    std::make_unique<BonkedCondElement> ());
	...
	bool GetData (const EntityListEntry &entry, ESP_Data &out)
	{
		...
		{
			if (ent == EntityList::GetLocal ()
			    && !Thirdperson::IsThirdPerson (
				static_cast<CTFPlayer *> (ent)))
				return false;
		...
		out.name = ent->GetClientClass ()->networkName != nullptr
			       ? ent->GetClientClass ()->networkName
			       : "unknown";
		return false;
	}
	...
	void PaintBox (Color color, const ESP_Data &data)
	{
		...
		helper::draw::OutlinedRect (
		    data.top.x - data.width, data.bottom.y - data.height,
		    data.bottom.x + data.width, data.bottom.y);
	}

	void PaintName (Color color, const Vector &top, int w, int h,
			const std::string &name)
	{
		int textw, texth;
		helper::draw::GetTextSize (name, textw, texth);
		helper::draw::TextShadow (top.x - (textw * 0.5f),
					  top.y - texth - 2, color, name);
	}
	...
	void PaintHealthbar (const ESP_Data &data)
	{
		...
		const float healthRatio = std::clamp (
		    static_cast<float> (data.health) / data.maxhealth, 0.0f,
		    1.0f);

		// draw background
		interfaces::Surface->DrawSetColor (background);
		interfaces::Surface->DrawFilledRect (
		    x0 - barMargin, y0 - barMargin, x1 + barMargin,
		    y1 + barMargin);
		...
		interfaces::Surface->DrawFilledRect (
		    x0, y1 - static_cast<int> (data.height * healthRatio), x1,
		    y1);
	}
	...
	void Run (CTFPlayer *pLocal)
	{
		...
		for (const auto &entry : EntityList::GetEntities ())
		{
			...
			{ // Health bar
				auto mode = static_cast<HealthMode> (
				    Settings::ESP.health);
				if (data.maxhealth > 0
				    && (mode == HealthMode::BAR
					|| mode == HealthMode::BOTH))
					PaintHealthbar (data);
			}
			...
			auto run = [&] (const auto &elements) {
				for (const auto &element : elements)
				{
					...
					{
						Vec2 pos = Vec2 (
						    data.top.x - size.x / 2.0f,
						    data.top.y
							- context.topOffset
							- iGAP);
						element->Draw (pos, ent, data,
							       context);
						context.topOffset
						    += element->GetSize (data)
							   .y;
						break;
					}
					...
					case ESP_ALIGNMENT::BOTTOM:
					{
						Vec2 pos = Vec2 (
						    data.top.x - size.x / 2.0f,
						    data.top.y + data.height
							+ context.bottomOffset
							+ iGAP);
						element->Draw (pos, ent, data,
							       context);
						context.bottomOffset
	...
	{
		switch (static_cast<ESPTeamSelectionMode> (
