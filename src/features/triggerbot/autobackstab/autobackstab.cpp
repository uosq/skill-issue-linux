...
bool AutoBackstab::IsBehindEntity (Vector localCenter, Vector targetCenter,
...
bool AutoBackstab::IsBehindEntity (CTFPlayer *pLocal, CTFPlayer *pTarget)
{
	...
	return posVsTargetView
	       > 0.0f; // for some reason this is positive, but in the tf2's
		       // source code it is negative wtf
}

bool AutoBackstab::IsBehindAndFacingEntity (CTFPlayer *pLocal,
					    CTFPlayer *pTarget)
{
	Vector dir = (pTarget->GetCenter () - pLocal->GetCenter ());
	dir.z	   = 0;
	...
	bool isBehind = posVsTargetView
			> 0.0f; // for some reason this is positive, but in the
...
bool AutoBackstab::IsBehindAndFacingEntity (Vector localCenter,
					    Vector targetCenter,
...
bool AutoBackstab::CanBackstabEntity (CTFPlayer *pLocal, CTFPlayer *pTarget)
{
	...
	int iNoBackstab = AttributeHookValue (1.0, "cannot_be_backstabbed",
					      pTarget, nullptr, true);
	if (iNoBackstab == 0)
		return false;
...
void LegitBackstab (CTFPlayer *pLocal, CTFWeaponBase *pWeapon, CUserCmd *pCmd)
{
	...
	{
		if ((entry.flags
		     & (EntityFlags::IsAlive | EntityFlags::IsPlayer))
		    == 0)
			continue;
		...
		{
			if (!AutoBackstab::IsBehindAndFacingEntity (
				localCenter, record.m_vecAbsCenter,
				localViewAngles, record.m_vecViewAngles))
				continue;

			if ((record.m_vecAbsCenter - localCenter).Length ()
			    > (48 * 2))
				continue;
...
void RageBackstab (CTFPlayer *pLocal, CTFWeaponBase *pWeapon, CUserCmd *pCmd,
		   bool *pSendPacket)
{
	Vector shootPos	   = pLocal->GetEyePos ();
	Vector localCenter = pLocal->GetCenter ();
	...
	for (const auto &entry : EntityList::GetEnemies ())
	{
		...
		for (const auto &record : records)
		{
			if (!AutoBackstab::IsBehindEntity (
				localCenter, record.m_vecAbsCenter,
			...
			{
				Vector dir
				    = record.m_vecAbsCenter - localCenter;
				pCmd->viewangles = dir.ToAngle ();
				pCmd->tick_count
				    = TIME_TO_TICKS (record.m_flSimTime);
				*pSendPacket = false;
				return;
			}
		}
...
void AutoBackstab::Run (CTFPlayer *pLocal, CTFWeaponBase *pWeapon,
