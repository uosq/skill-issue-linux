...
float GetAirAccelerate ()
{
	static ConVar *sv_airaccelerate
	    = interfaces::Cvar->FindVar ("sv_airaccelerate");
...
float GetForwardSpeed ()
{
	static ConVar *cl_forwardspeed
	    = interfaces::Cvar->FindVar ("cl_forwardspeed");
...
float GetSideSpeed ()
{
	static ConVar *cl_sidespeed
	    = interfaces::Cvar->FindVar ("cl_sidespeed");
...
namespace Autostrafe
{
	void Run (CTFPlayer *pLocal, CUserCmd *pCmd)
	{
		...
		float flTerminal
		    = flAirAccelerate / flMaxSpeed * 100.0f / flSpeed;
		...
		float flVelDirection
		    = atan2f (vecVelocity.y, vecVelocity.x) - flYaw;
		float flTargetAngle
		    = atan2f (-pCmd->sidemove, pCmd->forwardmove);
		float flDelta
		    = Math::NormalizeRad (flVelDirection - flTargetAngle);

		float flMoveDirection = flDelta < 0
					    ? flVelDirection + flGoodDelta
					    : flVelDirection - flGoodDelta;
