...
{
	bool ret;
	DETOUR_ORIG_GET (&entity_shoulddraw, ret,
			 CBasePlayer_ShouldDrawLocalPlayer);

	if (auto pLocal = EntityList::GetLocal ();
	    pLocal != nullptr && pLocal->IsAlive ())
		return ret
		       || (pLocal->InCond (TF_COND_ZOOMED)
			   && Settings::Misc.no_zoom);
...
{
	detour_init (&entity_shoulddraw,
		     Sigs::CBasePlayer_ShouldDrawLocalPlayer.GetPointer (),
		     (void *)&Hooked_CBasePlayer_ShouldDrawLocalPlayer);

	if (!detour_enable (&entity_shoulddraw))
		Logs::Error (
		    "Couldn't hook C_BasePlayer::ShouldDrawLocalPlayer");
}