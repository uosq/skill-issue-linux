...
DETOUR_DECL_TYPE (int, original_GetWindowSize, SDL_Window *window, int *w,
		  int *h);
...
static void SetupImGuiStyle ()
{
	...
	style.Colors[ImGuiCol_FrameBgHovered]
	    = ImVec4 (0.41201305f, 0.41201568f, 0.41201717f, 1.0f);
	style.Colors[ImGuiCol_FrameBgActive]
	    = ImVec4 (0.5536425f, 0.55364627f, 0.55364805f, 1.0f);
	...
	style.Colors[ImGuiCol_SliderGrabActive]
	    = ImVec4 (0.0f, 0.59557945f, 0.91845495f, 1.0f);
...
inline int Hooked_GetWindowSize (SDL_Window *window, int *w, int *h)
{
	...
	int ret = 0;
	DETOUR_ORIG_GET (&windowsizedetour, ret, original_GetWindowSize,
			 window, w, h);
	return ret;
}
...
inline void HookSDL ()
{
	...
	if (original_SwapWindow == nullptr)
		return interfaces::Cvar->ConsolePrintf (
		    "Couldn't get SwapWindow\n");

	void *original_PollEvent = dlsym (sdl, "SDL_PollEvent");
	if (original_PollEvent == nullptr)
		return interfaces::Cvar->ConsolePrintf (
		    "Couldn't get PollEvent\n");

	void *original_GetWindowSize = dlsym (sdl, "SDL_GetWindowSize");
	if (original_GetWindowSize == nullptr)
		return interfaces::Cvar->ConsolePrintf (
		    "Couldn't get GetWindowSize\n");

	detour_init (&swapdetour, original_SwapWindow,
		     (void *)&Hooked_SwapWindow);
	if (!detour_enable (&swapdetour))
		return interfaces::Cvar->ConsolePrintf (
		    "Couldn't hook SwapWindow\n");

	detour_init (&polldetour, original_PollEvent,
		     (void *)&Hooked_PollEvent);
	if (!detour_enable (&polldetour))
		return interfaces::Cvar->ConsolePrintf (
		    "Couldn't hook PollEvent\n");

	detour_init (&windowsizedetour, original_GetWindowSize,
		     (void *)&Hooked_GetWindowSize);
	if (!detour_enable (&windowsizedetour))
		return interfaces::Cvar->ConsolePrintf (
		    "Couldn't hook GetWindowSize\n");
