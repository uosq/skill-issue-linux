...
DETOUR_DECL_TYPE (void, original_FireGameEvent, void *self,
...
inline void Hooked_FireGameEvent (void *self, IGameEvent *event)
{
	if (event == nullptr)
	{
		DETOUR_ORIG_CALL (&firegameevent_ctx, original_FireGameEvent,
				  self, event);
		return;
	}
	...
	DETOUR_ORIG_CALL (&firegameevent_ctx, original_FireGameEvent, self,
			  event);
}
...
inline void HookFireGameEvent (void)
{
	...
	detour_init (&firegameevent_ctx,
		     Sigs::ClientModeShared_FireGameEvent.GetPointer (),
		     (void *)&Hooked_FireGameEvent);
	detour_enable (&firegameevent_ctx);

#ifdef DEBUG
	interfaces::Cvar->ConsolePrintf (
	    "ClientModeShared::FireGameEvent hooked\n");
#endif
}