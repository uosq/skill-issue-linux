...
DETOUR_DECL_TYPE (void, RunCommand, IPrediction *self, CTFPlayer *player,
		  CUserCmd *ucmd, void *moveHelper);
inline detour_ctx_t runcommand_ctx;

inline void Hooked_RunCommand (IPrediction *self, CTFPlayer *player,
			       CUserCmd *ucmd, void *moveHelper)
{
	if (Warp::m_bShifting)
	...
	DETOUR_ORIG_CALL (&runcommand_ctx, RunCommand, self, player, ucmd,
			  moveHelper);
}
...
{
	detour_init (&runcommand_ctx,
		     Sigs::CPrediction_RunCommand.GetPointer (),
		     (void *)&Hooked_RunCommand);
	if (!detour_enable (&runcommand_ctx))
	{
		interfaces::Cvar->ConsolePrintf (
		    "Failed to hook CPrediction::RunCommand\n");
		return;
	}
