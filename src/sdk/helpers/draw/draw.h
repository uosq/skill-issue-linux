...
namespace helper
{
	namespace draw
	{
		inline HFont CreateFont (const std::string &id,
					 const std::string &name, int height,
					 int weight, int flags = 0)
		{
			return FontManager::CreateFont (id, name, height,
							weight, flags);
		}
		...
		inline void Text (int x, int y, Color color, std::string text)
		{
			...
			interfaces::Surface->DrawPrintText (wtext.c_str (),
							    wtext.length ());
		}

		inline void TextShadow (int x, int y, Color color,
					std::string text)
		{
			// convert our string to wide string
			// fuck my life
		...
		inline void GetTextSize (std::string text, int &width,
					 int &height)
		{
			std::wstring wtext (text.begin (), text.end ());

			std::string currentFontID
			    = FontManager::GetCurrentFontID ();
			int font = FontManager::GetFont (currentFontID);

			interfaces::Surface->GetTextSize (font, wtext.c_str (),
							  width, height);
		}
