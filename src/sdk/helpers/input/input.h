...
namespace helper
{
	namespace input
	{
		...
		static bool CheckButtonPressedTick (ButtonCode_t btn)
		{
			...
			int tick
			    = interfaces::InputSystem->GetButtonPressedTick (
				btn);
		...
		{
			ButtonCode_t btn
			    = interfaces::InputSystem->StringToButtonCode (
				key.c_str ());

			if (IsButtonValid (btn)
			    && CheckButtonPressedTick (btn))
				return true;

			btn = interfaces::InputSystem->StringToButtonCode (
			    ("kp_" + key).c_str ());
			if (IsButtonValid (btn)
			    && CheckButtonPressedTick (btn))
				return true;
