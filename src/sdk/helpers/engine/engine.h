	...
	namespace engine
	{
		...
		inline bool GetViewMatrix(VMatrix &out)
		{
			...
			if (interfaces::ClientDLL->GetPlayerView(view))
			{
				...
				interfaces::RenderView->GetMatricesForView(view, &mWorldToView, &mViewToProjection,
									   &out, &mWorldToPixels);
				return true;
			}
			return false;
		...
		inline bool WorldToScreen(const Vector &pos, Vec2 &screenPosOut)
		{
			...
			screenPosOut.x =
			    (screenW / 2.0f) +
			    ((vm[0][0] * pos.x + vm[0][1] * pos.y + vm[0][2] * pos.z + vm[0][3]) * fl1dbw) * screenW /
				2.0f +
			    0.5f;
			screenPosOut.y =
			    (screenH / 2.0f) -
			    ((vm[1][0] * pos.x + vm[1][1] * pos.y + vm[1][2] * pos.z + vm[1][3]) * fl1dbw) * screenH /
				2.0f +
			    0.5f;
		...
		inline void Trace(Vector start, Vector end, unsigned int mask, ITraceFilter *pFilter,
				  CGameTrace *pTrace)
		{
			Ray_t ray;
			ray.Init(start, end);
		...
		inline void TraceHull(Vector start, Vector end, Vector hullmin, Vector hullmax, unsigned mask,
