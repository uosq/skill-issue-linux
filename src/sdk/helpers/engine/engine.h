...
namespace helper
{
	namespace engine
	{
		...
		inline void FixMovement (CUserCmd *pCmd, const Vec3 &vCurAngle,
					 const Vec3 &vTargetAngle)
		{
			bool bCurOOB
			    = fabsf (Math::NormalizeAngle (vCurAngle.x))
			      > 90.f;
			bool bTargetOOB
			    = fabsf (Math::NormalizeAngle (vTargetAngle.x))
			      > 90.f;

			Vec3 vMove     = { pCmd->forwardmove,
					   pCmd->sidemove * (bCurOOB ? -1 : 1),
					   pCmd->upmove };
			...
			float flTargetYaw
			    = vTargetAngle.y + (bTargetOOB ? 180.f : 0.f);
			float flYaw
			    = DEG2RAD (flTargetYaw - flCurYaw + vMoveAng.y);

			pCmd->forwardmove = cos (flYaw) * flSpeed;
			pCmd->sidemove
			    = sin (flYaw) * flSpeed * (bTargetOOB ? -1 : 1);
		}

		inline void FixMovement (CUserCmd *pCmd,
					 const Vec3 &vTargetAngle)
		{
		...
		inline bool GetViewMatrix (VMatrix &out)
		{
			...
			{
				VMatrix mWorldToView, mViewToProjection,
				    mWorldToPixels;
				interfaces::RenderView->GetMatricesForView (
				    view, &mWorldToView, &mViewToProjection,
				    &out, &mWorldToPixels);
				return true;
			}
			return false;
		...
		inline bool WorldToScreen (const Vector &pos,
					   Vec2 &screenPosOut)
		{
			VMatrix vm;
			if (!GetViewMatrix (vm))
			...
			float flW = vm[3][0] * pos.x + vm[3][1] * pos.y
				    + vm[3][2] * pos.z + vm[3][3];
			if (flW < 0.001f)
				return false;
		...
		inline void Trace (Vector start, Vector end, unsigned int mask,
				   ITraceFilter *pFilter, CGameTrace *pTrace)
		{
			Ray_t ray;
			ray.Init (start, end);
			interfaces::EngineTrace->TraceRay (ray, mask, pFilter,
							   pTrace);
		}

		inline void TraceHull (Vector start, Vector end,
				       Vector hullmin, Vector hullmax,
				       unsigned mask, ITraceFilter *pFilter,
				       CGameTrace *pTrace)
		{
			Ray_t ray;
