...
void Netvars::Dump (const char *baseClass, RecvTable *table, uint32_t offset)
{
	...
	auto it
	    = std::find_if (m_netvarUIVector.begin (), m_netvarUIVector.end (),
			    [&] (const NetvarClassEntry &e) {
				    return e.className == baseClass;
			    });
	...
	for (int i = 0; i < table->propsCount; ++i)
	{
		...
		if (prop->recvType == SendPropType::DATATABLE
		    && prop->dataTable && prop->dataTable->tableName[0] == 'D')
			Dump (baseClass, prop->dataTable,
			      offset + prop->offset);

		std::string netvarName
		    = std::string (baseClass) + "->" + prop->varName;
		...
		// for imgui
		it->members.push_back (prop->varName + std::string (" = 0x")
				       + std::to_string (finalOffset));
	}
}

void Netvars::DumpToFile (const char *baseClass, RecvTable *table,
			  std::ofstream &file, uint32_t offset)
{
	for (int i = 0; i < table->propsCount; ++i)
	{
		const auto prop = &table->props[i];
		...
		if (prop->recvType == SendPropType::DATATABLE
		    && prop->dataTable && prop->dataTable->tableName[0] == 'D')
		{
			DumpToFile (baseClass, prop->dataTable, file,
				    offset + prop->offset);
		}
		...
		// Dump to file
		file << netvarName << " = 0x" << std::hex << finalOffset
		     << std::dec << '\n';
	}
}
...
{
	interfaces::Cvar->ConsoleColorPrintf (Color_t{ 0, 255, 255, 255 },
					      "Dumping netvars...\n");
	...
	if (!file.is_open ())
	{
		interfaces::Cvar->ConsoleColorPrintf (
		    Color_t{ 255, 0, 0, 255 }, "Failed to open netvars.txt\n");
		return;
	}
	     ...
	     clientClass; clientClass = clientClass->next)
	{
		if (clientClass->recvTable)
		{
			...
			DumpToFile (clientClass->networkName,
				    clientClass->recvTable, file);
		}
	}

	file.close ();

	interfaces::Cvar->ConsoleColorPrintf (Color_t{ 150, 255, 150, 255 },
