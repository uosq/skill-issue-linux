...
class CBaseObject : public CBaseEntity
{
	...
	NETVAR (m_flPercentageConstructed,
		"CBaseObject->m_flPercentageConstructed", float);
	...
	NETVAR (m_iDesiredBuildRotations,
		"CBaseObject->m_iDesiredBuildRotations", int);
	NETVAR (m_bServerOverridePlacement,
		"CBaseObject->m_bServerOverridePlacement", bool);
	NETVAR (m_iUpgradeLevel, "CBaseObject->m_iUpgradeLevel", int);
	NETVAR (m_iUpgradeMetal, "CBaseObject->m_iUpgradeMetal", int);
	NETVAR (m_iUpgradeMetalRequired,
		"CBaseObject->m_iUpgradeMetalRequired", int);
	NETVAR (m_iHighestUpgradeLevel, "CBaseObject->m_iHighestUpgradeLevel",
		int);
	NETVAR (m_iObjectMode, "CBaseObject->m_iObjectMode", int);
	NETVAR (m_bDisposableBuilding, "CBaseObject->m_bDisposableBuilding",
		bool);
	NETVAR (m_bWasMapPlaced, "CBaseObject->m_bWasMapPlaced", bool);
	NETVAR (m_bPlasmaDisable, "CBaseObject->m_bPlasmaDisable", bool);

	std::string GetName () { return GetClientClass ()->networkName; }

	Vector GetCenter ()
	{
		return GetAbsOrigin ()
		       + (m_vecBuildMins () + m_vecBuildMaxs ()) * 0.5f;
	}
};