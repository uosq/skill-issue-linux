...
inline FileWeaponInfo_t *original_GetFileWeaponInfoFromHandle (void *handle)
{
	...
	static auto orig = (GetFileWeaponInfoFromHandleFn)sigscan_module (
	    "client.so", "66 3B 3D ? ? ? ? 48 8D 05");
	return orig (handle);
}
...
class CTFWeaponBase : public CBaseCombatWeapon
{
	...
	NETVAR (m_bReloadedThroughAnimEvent,
		"CTFWeaponBase->m_bReloadedThroughAnimEvent", bool)
	NETVAR (m_bDisguiseWeapon, "CTFWeaponBase->m_bDisguiseWeapon", bool)
	NETVAR (m_flLastCritCheckTime, "CTFWeaponBase->m_flLastCritCheckTime",
		float)
	NETVAR (m_flReloadPriorNextFire,
		"CTFWeaponBase->m_flReloadPriorNextFire", float)
	NETVAR (m_flLastFireTime, "CTFWeaponBase->m_flLastFireTime", float)
	NETVAR (m_flEffectBarRegenTime,
		"CTFWeaponBase->m_flEffectBarRegenTime", float)
	NETVAR (m_flObservedCritChance,
		"CTFWeaponBase->m_flObservedCritChance", float)
	NETVAR (m_flEnergy, "CTFWeaponBase->m_flEnergy", float)
	NETVAR (m_hExtraWearable, "CTFWeaponBase->m_hExtraWearable", EHANDLE)
	NETVAR (m_hExtraWearableViewModel,
		"CTFWeaponBase->m_hExtraWearableViewModel", EHANDLE)
	NETVAR (m_bBeingRepurposedForTaunt,
		"CTFWeaponBase->m_bBeingRepurposedForTaunt", bool)
	NETVAR (m_nKillComboClass, "CTFWeaponBase->m_nKillComboClass", int)
	NETVAR (m_nKillComboCount, "CTFWeaponBase->m_nKillComboCount", int)
	NETVAR (m_flInspectAnimEndTime,
		"CTFWeaponBase->m_flInspectAnimEndTime", float)
	...
	NETVAR_OFFSET (m_flSmackTime, "CTFWeaponBase->m_nInspectStage", float,
		       28);
	NETVAR_OFFSET (m_flCritTokenBucket, "CTFWeaponBase->m_iReloadMode",
		       float, -244);
	NETVAR_OFFSET (m_nCritChecks, "CTFWeaponBase->m_iReloadMode", int,
		       -240);
	NETVAR_OFFSET (m_nCritSeedRequests, "CTFWeaponBase->m_iReloadMode",
		       int, -236);
	NETVAR_OFFSET (m_iWeaponMode, "CTFWeaponBase->m_iReloadMode", int, -4);
	NETVAR_OFFSET (m_flCritTime, "CTFWeaponBase->m_flLastCritCheckTime",
		       float, -4);
	NETVAR_OFFSET (m_iCurrentSeed, "CTFWeaponBase->m_flLastCritCheckTime",
		       int, 8);
	NETVAR_OFFSET (m_flLastRapidFireCritCheckTime,
		       "CTFWeaponBase->m_flLastCritCheckTime", float, 12);
	...
	const FileWeaponInfo_t *GetWeaponInfo ()
	{
		...
		uintptr_t handleAddress
		    = reinterpret_cast<uintptr_t> (this) + 0xf12;
	...
	int GetWeaponID ()
	{
		...
		*/
		...
		auto fn
		    = reinterpret_cast<Fn> (vtable[0xE18 / sizeof (void *)]);
		return fn (this);
	}
	...
	{
		if (GetSlot () == EWeaponSlot::SLOT_MELEE
		    || GetWeaponID () == TF_WEAPON_BUILDER)
	...
	bool CanPrimaryAttack ()
	{
		...
		return m_flNextPrimaryAttack () <= curtime
	...
	bool CanAmbassadorHeadshot ()
	{
		if (GetWeaponID () == TF_WEAPON_REVOLVER
		    && AttributeHookValue (0, "set_weapon_mode", this, nullptr,
					   true))
	...
	bool CanHitTeammates ()
	{
		...
		static ConVar *mp_friendlyfire
		    = interfaces::Cvar->FindVar ("mp_friendlyfire");
	...
	bool CanAirblast ()
	{
		int iAirblastDisabled = static_cast<int> (AttributeHookValue (
		    0, "airblast_disabled", this, nullptr, true));
...
class CTFKnife : public CTFWeaponBase
{
	...
	NETVAR (m_flKnifeRegenerateDuration,
		"CTFKnife->m_flKnifeRegenerateDuration", float);
	NETVAR (m_flKnifeMeltTimestamp, "CTFKnife->m_flKnifeMeltTimestamp",
		float);
};
...
class CWeaponMedigun : public CTFWeaponBase
{
	...
	NETVAR (m_nChargeResistType, "CWeaponMedigun->m_nChargeResistType",
		int);
	NETVAR (m_hLastHealingTarget, "CWeaponMedigun->m_hLastHealingTarget",
		EHANDLE);
...
{
      public:
	NETVAR (m_iPipebombCount, "CTFPipebombLauncher->m_iPipebombCount",
		int);
	NETVAR (m_flChargeBeginTime,
		"CTFPipebombLauncher->m_flChargeBeginTime", float);
	//int GetDetonateType();
};
...
class CTFSniperRifle : public CTFWeaponBase
{
	...
	{
		bool isMachina
		    = m_iItemDefinitionIndex () == Sniper_m_TheMachina;
...
{
      public:
	NETVAR (m_flDetonateTime, "CTFGrenadeLauncher->m_flDetonateTime",
		float);
...
{
      public:
	NETVAR (m_flChargeBeginTime, "CTFParticleCannon->m_flChargeBeginTime",
		float);
...
{
      public:
	NETVAR (m_flChargeBeginTime, "CTFThrowable->m_flChargeBeginTime",
		float);
};
...
class CTFSpellBook : public CTFThrowable
{
	...
	NETVAR (m_iSelectedSpellIndex, "CTFSpellBook->m_iSelectedSpellIndex",
		int);
