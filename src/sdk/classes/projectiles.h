...
{
      public:
	NETVAR (m_hOriginalLauncher, "CBaseProjectile->m_hOriginalLauncher",
		EHANDLE);
};
...
{
      public:
	NETVAR (m_vInitialVelocity,
		"CTFWeaponBaseGrenadeProj->m_vInitialVelocity", Vector);
	...
	NETVAR (m_angRotation, "CTFWeaponBaseGrenadeProj->m_angRotation",
		Vector);
	NETVAR (m_hDeflectOwner, "CTFWeaponBaseGrenadeProj->m_hDeflectOwner",
		EHANDLE);
};
...
class CTFGrenadePipebombProjectile : public CTFWeaponBaseGrenadeProj
{
	...
	NETVAR (m_hLauncher, "CTFGrenadePipebombProjectile->m_hLauncher",
		EHANDLE);
	NETVAR (m_bDefensiveBomb,
		"CTFGrenadePipebombProjectile->m_bDefensiveBomb", bool);

	float GetLiveTime ()
	{
		static ConVar *tf_grenadelauncher_livetime
		    = interfaces::Cvar->FindVar (
			"tf_grenadelauncher_livetime");
		float flLiveTime = tf_grenadelauncher_livetime->GetFloat ();

		AttributeHookValue (flLiveTime, "sticky_arm_time",
				    HandleAs<CBaseEntity *> (m_hLauncher ()),
				    nullptr, true);
