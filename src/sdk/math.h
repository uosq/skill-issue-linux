...
/*
Yes, this is pasted from Amalgam
Source: https://github.com/rei-2/Amalgam/blob/bffae9999cf35a5fbdeb92387b9fae58796b8939/Amalgam/src/Utils/Math/Math.h#L1
*/
...
#define floatCompare(x, y)                                                    \
	(fabsf (x - y)                                                        \
	 <= FLT_EPSILON * fmaxf (1.f, fmaxf (fabsf (x), fabsf (y))))
...
namespace Math
{
	...
	inline float RemapVal (float flVal, float a, float b, float c, float d,
			       bool bClamp = true)
	{
		if (a == b)
			return flVal >= b ? d : c;
	...
	inline float SimpleSplineRemapVal (float flVal, float a, float b,
					   float c, float d,
					   bool bClamp = true)
	{
		if (a == b)
	...
	inline double FastSqrt (double n) { return std::sqrt (n); }
	...
	{
		return std::isfinite (flAngle)
			   ? std::remainder (flAngle, flRange)
			   : 0.f;
	}
	...
	inline void VectorAngles (const Vec3 &vForward, Vec3 &vAngles)
	{
		...
		} else
		{
			...
			flPitch = RAD2DEG (
			    atan2f (-vForward.z, vForward.Length2D ()));
			if (flPitch < 0)
				flPitch += 360;
		}
	...
	inline void AngleVectors (const Vec3 &vAngles,
				  Vec3 *pForward = nullptr,
				  Vec3 *pRight = nullptr, Vec3 *pUp = nullptr)
	{
			       ...
			       bool bClamp = true)
	{
		...
		float flHyp = std::sqrt ((vDelta.x * vDelta.x)
					 + (vDelta.y * vDelta.y));

		Vec3 vAngles
		    = { atanf (vDelta.z / flHyp) * float (M_RADPI),
			atanf (vDelta.y / vDelta.x) * float (M_RADPI), 0.f };
	...
	inline float CalcFov (const Vec3 &vFromAng, const Vec3 &vToAng)
	{
		...
		float flResult
		    = RAD2DEG (acos (vFromForward.Dot (vToForward)));
		if (!std::isfinite (flResult) || std::isinf (flResult)
		    || std::isnan (flResult))
			flResult = 0.f;
	...
	inline Vec3 RotatePoint (Vec3 vPoint, Vec3 vOrigin, Vec3 vAngles)
	{
		...
		Vec3 vX = { cy * cp, cy * sp * sr - sy * cr,
			    cy * sp * cr + sy * sr };
		Vec3 vY = { sy * cp, sy * sp * sr + cy * cr,
			    sy * sp * cr - cy * sr };
		Vec3 vZ = { -sp, cp * sr, cp * cr };

		return Vec3 (vX.Dot (vPoint), vY.Dot (vPoint), vZ.Dot (vPoint))
		       + vOrigin;
	}

	inline void VectorTransform (const Vec3 &vIn, const matrix3x4 &mMatrix,
				     Vec3 &vOut)
	{
		for (auto i = 0; i < 3; i++)
	...
	inline void MatrixSetColumn (const Vec3 &vIn, int iColumn,
				     matrix3x4 &mOut)
	{
	...
	inline void MatrixAngles (const matrix3x4 &mMatrix, Vec3 &vAngles)
	{
		...
		if (flLen > 0.001f)
		{
			...
			vAngles.y
			    = RAD2DEG (std::atan2 (vForward.y, vForward.x));
	...
	inline bool RayToOBB (const Vec3 &vOrigin, const Vec3 &vDirection,
			      const Vec3 &vMins, const Vec3 &vMaxs,
			      const matrix3x4 &mMatrix, float flScale = 1.f)
	{
		...
		Vec3 vDelta
		    = Vec3 (mMatrix[0][3], mMatrix[1][3], mMatrix[2][3])
		      - vOrigin;

		Vec3 X = Vec3 (mMatrix[0][0], mMatrix[1][0], mMatrix[2][0])
			 / flScale;
		Vec3 Y = Vec3 (mMatrix[0][1], mMatrix[1][1], mMatrix[2][1])
			 / flScale;
		Vec3 Z = Vec3 (mMatrix[0][2], mMatrix[1][2], mMatrix[2][2])
			 / flScale;
		if (flScale != 1.f)
			X /= flScale, Y /= flScale, Z /= flScale;

		Vec3 f = Vec3 (X.Dot (vDirection), Y.Dot (vDirection),
			       Z.Dot (vDirection));
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			{
				if (-e[i] + vMins[i] * flScale > 0.f
				    || -e[i] + vMaxs[i] * flScale < 0.f)
					return false;
		...
		float flTMin
		    = fmaxf (fmaxf (fminf (t[0], t[1]), fminf (t[2], t[3])),
			     fminf (t[4], t[5]));
		float flTMax
		    = fminf (fminf (fmaxf (t[0], t[1]), fmaxf (t[2], t[3])),
			     fmaxf (t[4], t[5]));
	...
	inline void ConcatTransforms (const matrix3x4 &mIn1,
				      const matrix3x4 &mIn2, matrix3x4 &mOut)
	{
		if (&mIn1 == &mOut)
		{
			matrix3x4 mIn1b;
		...
		mOut[0][0] = mIn1[0][0] * mIn2[0][0] + mIn1[0][1] * mIn2[1][0]
			     + mIn1[0][2] * mIn2[2][0];
		mOut[0][1] = mIn1[0][0] * mIn2[0][1] + mIn1[0][1] * mIn2[1][1]
			     + mIn1[0][2] * mIn2[2][1];
		mOut[0][2] = mIn1[0][0] * mIn2[0][2] + mIn1[0][1] * mIn2[1][2]
			     + mIn1[0][2] * mIn2[2][2];
		mOut[0][3] = mIn1[0][0] * mIn2[0][3] + mIn1[0][1] * mIn2[1][3]
			     + mIn1[0][2] * mIn2[2][3] + mIn1[0][3];
		mOut[1][0] = mIn1[1][0] * mIn2[0][0] + mIn1[1][1] * mIn2[1][0]
			     + mIn1[1][2] * mIn2[2][0];
		mOut[1][1] = mIn1[1][0] * mIn2[0][1] + mIn1[1][1] * mIn2[1][1]
			     + mIn1[1][2] * mIn2[2][1];
		mOut[1][2] = mIn1[1][0] * mIn2[0][2] + mIn1[1][1] * mIn2[1][2]
			     + mIn1[1][2] * mIn2[2][2];
		mOut[1][3] = mIn1[1][0] * mIn2[0][3] + mIn1[1][1] * mIn2[1][3]
			     + mIn1[1][2] * mIn2[2][3] + mIn1[1][3];
		mOut[2][0] = mIn1[2][0] * mIn2[0][0] + mIn1[2][1] * mIn2[1][0]
			     + mIn1[2][2] * mIn2[2][0];
		mOut[2][1] = mIn1[2][0] * mIn2[0][1] + mIn1[2][1] * mIn2[1][1]
			     + mIn1[2][2] * mIn2[2][1];
		mOut[2][2] = mIn1[2][0] * mIn2[0][2] + mIn1[2][1] * mIn2[1][2]
			     + mIn1[2][2] * mIn2[2][2];
		mOut[2][3] = mIn1[2][0] * mIn2[0][3] + mIn1[2][1] * mIn2[1][3]
			     + mIn1[2][2] * mIn2[2][3] + mIn1[2][3];
	}
	...
	inline std::vector<float> SolveQuadratic (float a, float b, float c)
	{
		...
		a *= 2;
		b = -b;
		return { static_cast<float> ((b + sqrt (flRoot)) / a),
			 static_cast<float> ((b - sqrt (flRoot)) / a) };
	}
	...
	inline std::vector<float> SolveQuartic (float a, float b, float c,
						float d, float e)
	{
		...
		float m
		    = SolveCubic (p,
				  powf (p, 2) / 4 + powf (b, 4) / (256.f / 3)
				      - e + b * d / 4 - powf (b, 2) * c / 16,
				  -powf (q, 2) / 8);
		if (m < 0.f)
			return vRoots;
		...
		} else
		{
			...
			{
				float flDelta
				    = sqrt (2 * (-m - p + q / sqrt_2m));
				vRoots.push_back ((-sqrt_2m + flDelta) / 2
						  - b / 4);
				vRoots.push_back ((-sqrt_2m - flDelta) / 2
						  - b / 4);
			}
