...
#define EFONTFLAG_NONE 0
#define EFONTFLAG_ITALIC 0x001
#define EFONTFLAG_UNDERLINE 0x002
#define EFONTFLAG_STRIKEOUT 0x004
#define EFONTFLAG_SYMBOL 0x008
#define EFONTFLAG_ANTIALIAS 0x010
#define EFONTFLAG_GAUSSIANBLUR 0x020
#define EFONTFLAG_ROTARY 0x040
#define EFONTFLAG_DROPSHADOW 0x080
#define EFONTFLAG_ADDITIVE 0x100
#define EFONTFLAG_OUTLINE 0x200
#define EFONTFLAG_CUSTOM 0x400
#define EFONTFLAG_BITMAP 0x800
...
class ISurface : public IAppSystem
{
	...
	virtual bool *CreateHTMLWindow(void *events, VPANEL context) = 0;
	...
	virtual void DrawSetTextureRGBA(int id, const unsigned char *rgba, int wide, int tall, int hardwareFilter,
					bool forceReload)						    = 0;
	...
	virtual void SetAsToolBar(VPANEL panel,
				  bool state) = 0; // removes the window's task bar entry (for context menu's, etc.)
	virtual void CreatePopup(VPANEL panel, bool minimised, bool showTaskbarIcon = true, bool disabled = false,
	...
	virtual bool SetFontGlyphSet(HFont font, const char *windowsFontName, int tall, int weight, int blur,
				     int scanlines, int flags, int nRangeMin = 0, int nRangeMax = 0) = 0;
	...
	virtual void DrawTexturedPolyLine(const Vertex_t *p,
					  int n)      = 0; // (Note: this connects the first and last points).
	virtual void DrawTexturedSubRect(int x0, int y0, int x1, int y1, float texs0, float text0, float texs1,
					 float text1) = 0;
	...
	virtual bool CreateCursorFromFile(char const *curOrAniFile, char const *pPathID = 0)			    = 0;
	...
	virtual void DrawFilledRectFastFade(int x0, int y0, int x1, int y1, int fadeStartPt, int fadeEndPt,
					    unsigned int alpha0, unsigned int alpha1, bool bHorizontal)		    = 0;
	virtual void DrawFilledRectFade(int x0, int y0, int x1, int y1, unsigned int alpha0, unsigned int alpha1,
					bool bHorizontal)							    = 0;
	virtual void DrawSetTextureRGBAEx(int id, const unsigned char *rgba, int wide, int tall, void *imageFormat) = 0;
	virtual void DrawSetTextScale(float sx, float sy)							    = 0;
	virtual bool SetBitmapFontGlyphSet(HFont font, const char *windowsFontName, float scalex, float scaley,
					   int flags)								    = 0;
	...
	virtual void GetKernedCharWidth(HFont font, wchar_t ch, wchar_t chBefore, wchar_t chAfter, float &wide,
					float &abcA)								    = 0;
	...
	virtual void DrawUpdateRegionTextureRGBA(int nTextureID, int x, int y, const unsigned char *pchData, int wide,
						 int tall, void *imageFormat)					    = 0;
	...
	void StartDrawing(void)
	{
		...
		static StartDrawingFn StartDrawingFunc =
		    (StartDrawingFn)sigscan_module("vguimatsurface.so", "55 48 89 E5 41 54 53 48 89 FB 48 83 EC 20");
	...
	void FinishDrawing(void)
	{
		...
		static FinishDrawingFn FinishDrawingFunc =
		    (FinishDrawingFn)sigscan_module("vguimatsurface.so", "55 31 FF 48 89 E5 41 54");
