...
struct MatchMakingKeyValuePair_t
{
	...
	{
		strncpy_s(m_szKey, pchKey,
			  sizeof(m_szKey)); // this is a public header, use basic c library string funcs only!
...
inline const char *servernetadr_t::ToString(uint32 unIP, uint16 usPort) const
{
	...
	snprintf(s[nBuf], sizeof(s[nBuf]), "%u.%u.%u.%u:%i", (int)(ipByte[0]), (int)(ipByte[1]), (int)(ipByte[2]),
		 (int)(ipByte[3]), usPort);
#else
	snprintf(s[nBuf], sizeof(s[nBuf]), "%u.%u.%u.%u:%i", (int)(ipByte[3]), (int)(ipByte[2]), (int)(ipByte[1]),
		 (int)(ipByte[0]), usPort);
#endif
...
class gameserveritem_t
{
	...
	uint32
	    m_ulTimeLastPlayed; ///< time (in unix time) when this server was last played on (for favorite/history servers)
