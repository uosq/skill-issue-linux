...
{
	uint8_t strength
	    [SCE_PAD_TRIGGER_EFFECT_CONTROL_POINT_NUM]; /*E strength that the motor arm pushes back target trigger at position(0~8 (0: Same as Off mode)).
...
typedef struct ScePadTriggerEffectMultiplePositionVibrationParam
{
	...
	uint8_t amplitude
	    [SCE_PAD_TRIGGER_EFFECT_CONTROL_POINT_NUM]; /*E vibration amplitude at position(0~8 (0: Same as Off mode)).
...
typedef struct ScePadTriggerEffectParam
{
	...
	ScePadTriggerEffectCommand command
	    [SCE_PAD_TRIGGER_EFFECT_TRIGGER_NUM]; /*E command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2] is for L2 trigger setting
