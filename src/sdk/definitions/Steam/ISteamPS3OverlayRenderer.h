...
#define STEAM_COLOR_RED(color) (int)(((color) >> 16) & 0xff)

#define STEAM_COLOR_GREEN(color) (int)(((color) >> 8) & 0xff)

#define STEAM_COLOR_BLUE(color) (int)((color) & 0xff)

#define STEAM_COLOR_ALPHA(color) (int)(((color) >> 24) & 0xff)
...
class ISteamPS3OverlayRenderHost
{
	...
	virtual void DrawTexturedRect(int x0, int y0, int x1, int y1, float u0, float v0, float u1, float v1,
				      int32 iTextureID, DWORD colorStart, DWORD colorEnd,
				      EOverlayGradientDirection eDirection) = 0;
	...
	virtual void LoadOrUpdateTexture(int32 iTextureID, bool bIsFullTexture, int x0, int y0, uint32 uWidth,
					 uint32 uHeight, int32 iBytes, char *pData) = 0;
...
class ISteamPS3OverlayRender
{
	...
	virtual bool BHostInitialize(uint32 unScreenWidth, uint32 unScreenHeight, uint32 unRefreshRate,
				     ISteamPS3OverlayRenderHost *pRenderHost, void *CellFontLib) = 0;
