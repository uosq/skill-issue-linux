...
void DrawAimbotTab ()
{
	...
	ImGui::PushStyleVar (ImGuiStyleVar_Alpha,
			     Settings::Aimbot.key->IsEnabled () ? 1.0f : 0.5f);
	{
		// ImGui::InputText("Key", Settings::Aimbot.key,
		// sizeof(Settings::Aimbot.key));

		ImGui::Checkbox ("Autoshoot", &Settings::Aimbot.autoshoot);
		ImGui::Checkbox ("Viewmodel Aim",
				 &Settings::Aimbot.viewmodelaim);
		ImGui::Checkbox ("Draw FOV Indicator",
				 &Settings::Aimbot.draw_fov_indicator);
		ImGui::Checkbox ("Wait For Charge",
				 &Settings::Aimbot.waitforcharge);
		ImGui::Checkbox ("Draw Path", &Settings::Aimbot.path);
		ImGui::SliderFloat ("Fov", &Settings::Aimbot.fov, 0.0f,
				    180.0f);
		ImGui::SliderFloat ("Max Sim Time",
				    &Settings::Aimbot.max_sim_time, 0.0f,
				    5.0f);
		ImGui::SliderFloat ("Smoothness", &Settings::Aimbot.smoothness,
				    0.0f, 100.0f);

		{
			...
			ImGui::Combo ("Indicator Style",
				      &Settings::Aimbot.indicator, items, 4);
		}

		{
			constexpr const char *items[]{ "None", "Legit",
						       "Rage" };
			ImGui::Combo ("Melee Aimbot", &Settings::Aimbot.melee,
				      items, 3);
		}

		{
			...
			ImGui::Combo ("Method", &Settings::Aimbot.mode, items,
				      4);
		}

		{
			...
			ImGui::Combo ("Team Selection",
				      &Settings::Aimbot.teamMode, items, 3);
		}
		...
		ImGui::SameLine ();
		ImGui::Checkbox ("Ubercharged",
				 &Settings::Aimbot.ignoreubered);
		ImGui::SameLine ();
		...
		ImGui::Checkbox ("Hold Minigun Spin",
				 &Settings::Aimbot.hold_minigun_spin);
	}
	ImGui::PopStyleVar ();
}
...
void DrawESPTab ()
{
	if (ImGui::BeginTable ("##ESPContents", 2))
	{
		...
		ImGui::BeginDisabled (!Settings::ESP.enabled);
		{
			...
			ImGui::Checkbox ("Ignore Cloaked",
					 &Settings::ESP.ignorecloaked);
			ImGui::Checkbox ("Buildings",
					 &Settings::ESP.buildings);
			ImGui::Checkbox ("Weapon", &Settings::ESP.weapon);

			{
				constexpr const char *items[]{ "None", "Text",
							       "Bar", "Both" };
				ImGui::Combo ("Health", &Settings::ESP.health,
					      items, 4);
			}

			{
				constexpr const char *items[]{
					"Only Enemies", "Only Teammates",
					"Both"
				};
				ImGui::Combo ("Team Selection",
					      &Settings::ESP.team_selection,
					      items, 3);
			}
		}
		ImGui::EndDisabled ();
		...
		ImGui::CheckboxFlags (
		    "Zoom", &Settings::ESP.fconditions,
		    static_cast<int> (ESPConditionFlags::Zoomed));
		ImGui::CheckboxFlags (
		    "Ubercharge", &Settings::ESP.fconditions,
		    static_cast<int> (ESPConditionFlags::Ubered));
		ImGui::CheckboxFlags (
		    "Jarate", &Settings::ESP.fconditions,
		    static_cast<int> (ESPConditionFlags::Jarated));
		ImGui::CheckboxFlags (
		    "Bonk", &Settings::ESP.fconditions,
		    static_cast<int> (ESPConditionFlags::Bonked));
	...
	ImGui::SliderFloat ("Chams Alpha", &Settings::ESP.chams_alpha, 0.0f,
			    1.0f);
	...
	float red[3]	= { Settings::Colors.red_team.r () / 255.0f,
			    Settings::Colors.red_team.g () / 255.0f,
			    Settings::Colors.red_team.b () / 255.0f };
	float blu[3]	= { Settings::Colors.blu_team.r () / 255.0f,
			    Settings::Colors.blu_team.g () / 255.0f,
			    Settings::Colors.blu_team.b () / 255.0f };
	float target[3] = { Settings::Colors.aimbot_target.r () / 255.0f,
			    Settings::Colors.aimbot_target.g () / 255.0f,
			    Settings::Colors.aimbot_target.b () / 255.0f };
	float weapon[3] = { Settings::Colors.weapon.r () / 255.0f,
	...
	if (ImGui::ColorEdit3 ("RED Team", red))
		Settings::Colors.red_team.SetColor (
		    red[0] * 255.0f, red[1] * 255.0f, red[2] * 255.0f, 255.0f);

	if (ImGui::ColorEdit3 ("BLU Team", blu))
		Settings::Colors.blu_team.SetColor (
		    blu[0] * 255.0f, blu[1] * 255.0f, blu[2] * 255.0f, 255.0f);

	if (ImGui::ColorEdit3 ("Aimbot Target", target))
		Settings::Colors.aimbot_target.SetColor (
		    target[0] * 255.0f, target[1] * 255.0f, target[2] * 255.0f,
		    255.0f);

	if (ImGui::ColorEdit3 ("Weapon", weapon))
		Settings::Colors.weapon.SetColor (weapon[0] * 255.0f,
						  weapon[1] * 255.0f,
						  weapon[2] * 255.0f, 255.0f);
}
...
void DrawMiscTab ()
{
	...
	ImGui::SliderFloat4 ("Third Person Offset",
			     Settings::Misc.thirdperson_offset, -100.0f,
			     100.0f);
	...
	ImGui::Checkbox ("Backpack Expander",
			 &Settings::Misc.backpack_expander);
	ImGui::Checkbox ("Accept Item Drops",
			 &Settings::Misc.accept_item_drop);
	...
	ImGui::Checkbox ("Custom Fov Enabled",
			 &Settings::Misc.customfov_enabled);

	ImGui::BeginDisabled (!Settings::Misc.customfov_enabled);
	{
		ImGui::SliderFloat ("Custom Fov", &Settings::Misc.customfov,
				    54.0f, 120.0f);
	}
	ImGui::EndDisabled ();
	...
	ImGui::SliderFloat3 ("Viewmodel Offset",
			     Settings::Misc.viewmodel_offset, -20, 20.0f);
	ImGui::SliderFloat ("Viewmodel Interp",
			    &Settings::Misc.viewmodel_interp, 0.0f, 50.0f);

	ImGui::Separator ();

	{
		constexpr const char *items[]{ "None", "Last Record Only",
					       "All Records" };
		ImGui::Combo ("Backtrack", &Settings::Misc.backtrack, items,
			      3);
	}
	...
	ImGui::PushStyleVar (ImGuiStyleVar_Alpha,
			     Settings::AntiAim.warp_key->IsEnabled () ? 1.0f
								      : 0.5f);
	{
		gBinds.RenderHotkey ("Warp Key", Settings::AntiAim.warp_key);
		gBinds.RenderHotkey ("Warp Recharge Key",
				     Settings::AntiAim.warp_recharge_key);
		ImGui::SliderInt ("Warp Speed", &Settings::AntiAim.warp_speed,
				  1, 24);
	}
	ImGui::PopStyleVar ();
...
void DrawTriggerTab ()
{
	...
	ImGui::PushStyleVar (ImGuiStyleVar_Alpha,
			     Settings::Trigger.key->IsEnabled () ? 1.0f
								 : 0.5f);
	{
...

		{
			constexpr const char *items[]{ "None", "Legit",
						       "Rage" };
			ImGui::Combo ("Auto Backstab",
				      &Settings::Trigger.autobackstab, items,
				      3);
			ImGui::Combo ("Auto Airblast",
				      &Settings::Trigger.autoairblast, items,
				      3);
		}
	}
	ImGui::PopStyleVar ();
...
void DrawAntiaimTab ()
{
	...
	{
		{
			constexpr const char *items[]{ "None",	    "Up",
						       "Down",	    "Fake Up",
						       "Fake Down", "Random" };
			ImGui::Combo ("Pitch Mode",
				      &Settings::AntiAim.pitch_mode, items, 6);
		}

		{
			...
			ImGui::Combo ("Real Yaw",
				      &Settings::AntiAim.real_yaw_mode, items,
				      8);
			ImGui::Combo ("Fake Yaw",
				      &Settings::AntiAim.fake_yaw_mode, items,
				      8);
		}

		ImGui::SliderInt ("Spin Speed", &Settings::AntiAim.spin_speed,
				  0, 10);
	}
	ImGui::EndDisabled ();
	...
	ImGui::Checkbox ("Fake Lag Enabled",
			 &Settings::AntiAim.fakelag_enabled);

	ImGui::BeginDisabled (!Settings::AntiAim.fakelag_enabled);
	{
		ImGui::SliderInt ("Ticks", &Settings::AntiAim.fakelag_ticks, 1,
				  21);
	}
	ImGui::EndDisabled ();
}
...
void DrawLuaTab ()
{
	...
	if (ImGui::BeginTabBar ("LuaTab"))
	{
		if (ImGui::BeginTabItem ("Scripts"))
		{
			...
			{
				std::string dir
				    = std::string (interfaces::Engine
						       ->GetGameDirectory ())
				      + "/skill-issue/scripts/";
				ImGui::Text ("Add scripts in %s",
					     dir.c_str ());
			}
		...
		if (ImGui::BeginTabItem ("Restrictions"))
		{
			static const AccessOption options[]{
				{ "Allow Engine Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_ENGINE },
				{ "Allow Client Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_CLIENT },
				{ "Allow ClientState Library",
				  ScriptAccessMask::
				      SCRIPT_MASK_ALLOW_CLIENTSTATE },
				{ "Allow Common Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_COMMON },
				{ "Allow Draw Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_DRAW },
				{ "Allow EntityList Library",
				  ScriptAccessMask::
				      SCRIPT_MASK_ALLOW_ENTITYLIST },
				{ "Allow Hooks Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_HOOKS },
				{ "Allow ImGui Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_IMGUI },
				{ "Allow Input Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_INPUT },
				{ "Allow Materials Library",
				  ScriptAccessMask::
				      SCRIPT_MASK_ALLOW_MATERIALS },
				{ "Allow Render Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_RENDER },
				{ "Allow FileSystem Library",
				  ScriptAccessMask::
				      SCRIPT_MASK_ALLOW_FILESYSTEM },
				{ "Allow Math Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_MATH },
				{ "Allow Array Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_ARRAY },
				{ "Allow Dictionary Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_DICT },
				{ "Allow DateTime Library",
				  ScriptAccessMask::
				      SCRIPT_MASK_ALLOW_DATETIME },
				{ "Allow String Library",
				  ScriptAccessMask::SCRIPT_MASK_ALLOW_STRING },
			};
			...
			int columns = std::max (
			    1,
			    (int)(ImGui::GetContentRegionAvail ().x / 250.0f));

			if (ImGui::BeginTable ("##fdgidfojg", columns))
			{
				for (int i = 0; i < IM_ARRAYSIZE (options);
				     i++)
				{
					ImGui::TableNextColumn ();
					ImGui::CheckboxFlags (options[i].label,
							      &mask,
							      options[i].flag);
				}
...
void DrawParsedNetvarData (const std::vector<NetvarClassEntry> &classes)
{
	...
	{
		if (!bIsSearchEmpty
		    && !ContainsInsensitive (cls.className, strSearch))
			continue;
...
void DrawConfigTab ()
{
	...
	ImGui::InputText ("##NewConfigName", GUI::newConfigName,
			  sizeof (GUI::newConfigName));
	...
	if (ImGui::Button ("Create & Save", ImVec2 (buttonWidth, 0)))
	{
		...
		{
			std::string filename
			    = std::string (GUI::newConfigName) + ".ini";
	...
	if (ImGui::BeginChild ("##Configs"))
	{
		...
		for (int i = 0; i < GUI::configs.size (); ++i)
		{
			...
			if (ImGui::Selectable (GUI::configs[i].c_str (),
					       isSelected))
				GUI::selectedIndex = i;

			if (isSelected)
			{
				std::string fullPath = configFolder + "/"
						       + GUI::configs[i]
						       + ".ini";
...
void DrawLogsTab ()
{
	...
	for (const auto &entry : Logs::GetLogs ())
	{
		...
		case LogLevel::INFO:
			...
			ImGui::TextColored (color, "%s - %s", "Info",
					    entry.text.c_str ());
			break;
		case LogLevel::WARN:
			color = ImVec4 (1, 1, 0, 1);
			ImGui::TextColored (color, "%s - %s", "Warn",
					    entry.text.c_str ());
			break;
		case LogLevel::ERROR:
			color = ImVec4 (1, 0, 0, 1);
			ImGui::TextColored (color, "%s - %s", "Error",
					    entry.text.c_str ());
			break;
		}
	}
...
void GUI::RunSpectatorList () { Spectators::DrawList (); }
...
void GUI::RunPlayerList ()
{
	...
	ImGui::SetNextWindowSizeConstraints (ImVec2 (150.0f, 0.0f),
	...
	if (ImGui::Begin ("Player List", nullptr, flags))
	{
		for (const auto &entry : EntityList::GetEntities ())
		{
			...
			case TEAM_BLU:
			{
				...
				ImVec4 textColor (color.r () / 255.0f,
						  color.g () / 255.0f,
						  color.b () / 255.0f, 255);
	...
	if (ImGui::Begin ("Skill Issue", nullptr, ImGuiWindowFlags_NoCollapse))
	{
		if (ImGui::BeginTable ("MainTable", 2, 0))
		{
			ImGui::TableSetupColumn (
			    "Buttons", ImGuiTableColumnFlags_WidthFixed, 70);
			ImGui::TableSetupColumn (
			    "Content", ImGuiTableColumnFlags_WidthStretch);
