...
void ImGui::TextAlignedV(float align_x, float size_x, const char *fmt, va_list args)
{
	...
	if (size_x < text_size.x && IsItemHovered(ImGuiHoveredFlags_NoNavOverride |
						  ImGuiHoveredFlags_AllowWhenDisabled | ImGuiHoveredFlags_ForTooltip))
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args)
{
	...
	// Render
	RenderTextClipped(value_bb.Min + style.FramePadding, value_bb.Max, value_text_begin, value_text_end,
			  &value_size, ImVec2(0.0f, 0.0f));
...
void ImGui::SeparatorTextEx(ImGuiID id, const char *label, const char *label_end, float extra_w)
{
	...
	if (label_size.x > 0.0f)
	{
		...
		RenderTextEllipsis(window->DrawList, label_pos, ImVec2(bb.Max.x, bb.Max.y + style.ItemSpacing.y),
...
bool ImGui::BeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags)
{
	...
	if (!(flags & ImGuiComboFlags_NoArrowButton))
	{
		...
		if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
			RenderArrow(window->DrawList,
				    ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y), text_col,
				    ImGuiDir_Down, 1.0f);
	}
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect &bb, ImGuiComboFlags flags)
{
	...
	if (ImGuiWindow *popup_window = FindWindowByName(name))
		if (popup_window->WasActive)
		{
			...
			ImVec2 pos =
			    FindBestWindowPosForPopupEx(bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection,
			 ...
			 const void *p_max, const char *format, ImGuiSliderFlags flags)
{
	...
	IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) &&
		  "Invalid ImGuiSliderFlags flags! Has the legacy 'float power' argument been mistakenly cast to "
...
namespace ImStb
{
	...
	static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, ImGuiInputTextState *obj, int line_start_idx)
	{
		...
		const ImVec2 size	   = InputTextCalcTextSize(
		     obj->Ctx, text + line_start_idx, text + obj->TextLen, text + obj->TextLen, &text_remaining, NULL,
...
#define STB_TEXTEDIT_MOVEWORDRIGHT STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
	...
	static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState *obj, ImStb::STB_TexteditState *state,
						   int cursor)
	{
					     ...
					     const char *buf, const char *buf_end, int cursor_n)
{
	...
	ImVec2 offset;
	offset.x =
	    InputTextCalcTextSize(&g, line_start, cursor_ptr, buf_end, NULL, NULL, ImDrawTextFlags_WrapKeepBlanks).x;
			...
			ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void *callback_user_data)
{
	...
	if (is_wordwrap)
		wrap_width =
		    ImMax(1.0f, GetContentRegionAvail().x + (draw_window->ScrollbarY ? 0.0f : -g.Style.ScrollbarSize));
	...
	if (init_reload_from_user_buf)
	{
		...
		state->TextA.resize(
		    buf_size + 1); // we use +1 to make sure that .Data is always pointing to at least an empty string.
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		const bool is_enter =
		    Shortcut(ImGuiKey_Enter, f_repeat, id) || Shortcut(ImGuiKey_KeypadEnter, f_repeat, id);
		...
		else if (is_paste)
		{
			if (const char *clipboard = GetClipboardText())
			{
				...
				for (const char *s = clipboard; *s != 0;)
				{
					...
					memcpy(clipboard_filtered.Data + clipboard_filtered.Size - out_len, c_utf8,
					       out_len);
				}
				      ...
				      ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if (event_flag)
				{
					...
					state->Stb->select_start =
					    ImClamp(callback_data.SelectionStart, 0, callback_data.BufTextLen);
	...
	if (render_cursor || render_selection)
	{
		...
		if (state->CursorCenterY)
		{
			if (is_multiline)
				new_scroll_y =
				    cursor_offset.y - g.FontSize - (inner_size.y * 0.5f - style.FramePadding.y);
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags, const float *ref_col)
{
	...
	if (!(flags & ImGuiColorEditFlags_PickerMask_))
		flags |=
		    ((g.ColorEditOptions & ImGuiColorEditFlags_PickerMask_) ? g.ColorEditOptions
	...
	else if (flags & ImGuiColorEditFlags_PickerHueBar)
	{
		...
		draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0,
...
void ImGui::ColorTooltip(const char *text, const float *col, ImGuiColorEditFlags flags)
{
	...
	if ((flags & ImGuiColorEditFlags_InputRGB) || !(flags & ImGuiColorEditFlags_InputMask_))
	{
		if (flags & ImGuiColorEditFlags_NoAlpha)
			...
			Text("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)", cr, cg, cb, ca, cr,
			     cg, cb, ca, col[0], col[1], col[2], col[3]);
	}
	else if (flags & ImGuiColorEditFlags_InputHSV)
	{
		if (flags & ImGuiColorEditFlags_NoAlpha)
...
bool ImGui::CollapsingHeader(const char *label, bool *p_visible, ImGuiTreeNodeFlags flags)
{
	...
	if (p_visible)
		flags |=
		    ImGuiTreeNodeFlags_AllowOverlap | (ImGuiTreeNodeFlags)ImGuiTreeNodeFlags_ClipLabelForTrailingButton;
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
	...
	if (is_visible)
		RenderTextClipped(pos, ImVec2(ImMin(pos.x + size.x, window->WorkRect.Max.x), pos.y + size.y), label,
				  NULL, &label_size, style.SelectableTextAlign, &bb);
...
ImGuiTypingSelectRequest *ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
	...
	if (data->SearchBuffer[0] != 0)
	{
		...
		clear_buffer |=
		    g.ActiveId != 0 && g.NavActivateId == 0; // Allow temporary SPACE activation to not interfere
				 ...
				 const char *(*get_item_name_func)(void *, int), void *user_data, int nav_item_idx)
{
	...
	if (req->SingleCharMode && (req->Flags & ImGuiTypingSelectFlags_AllowSingleCharMode))
		idx =
		    TypingSelectFindNextSingleCharMatch(req, items_count, get_item_name_func, user_data, nav_item_idx);
	else
			...
			if (req.Selected)
			{
				...
				for (int idx = 0; idx < ms_io->ItemsCount; idx++, _SelectionOrder++)
					ImGuiSelectionBasicStorage_BatchSetItemSelected(
					    this, GetStorageIdFromIndex(idx), req.Selected, size_before_amends,
					    _SelectionOrder);
...
void ImGui::PlotHistogram(const char *label, float (*values_getter)(void *data, int idx), void *data, int values_count,
			  ...
			  ImVec2 graph_size)
{
	PlotEx(ImGuiPlotType_Histogram, label, values_getter, data, values_count, values_offset, overlay_text,
	       scale_min, scale_max, graph_size);
}
...
bool ImGui::BeginMenuBar()
{
	...
	ImRect clip_rect(IM_ROUND(bar_rect.Min.x + window->WindowBorderSize * 0.5f),
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
	...
	for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
	{
		...
		const bool has_close_button_or_unsaved_marker = (tab->Flags & ImGuiTabItemFlags_NoCloseButton) == 0 ||
		      ...
		      ImGuiWindow *docked_window)
{
	...
	if (is_visible)
	{
		    ...
		    style.TabBarOverlineSize > 0.0f)
		{
			...
			{
				display_draw_list->AddLine(tl - ImVec2(0.5f, 0.5f), tr - ImVec2(0.5f, 0.5f),
