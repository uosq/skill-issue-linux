			 ...
			 const ImVec2 &outer_size, float inner_width)
{
	...
	if (use_child_window)
	{
		...
		ImGuiWindowFlags child_window_flags =
		    (g.NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasWindowFlags) ? g.NextWindowData.WindowFlags
...
void ImGui::TableUpdateLayout(ImGuiTable *table)
{
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
	{
		...
		offset_x +=
		    column->WidthGiven + table->CellSpacingX1 + table->CellSpacingX2 + table->CellPaddingX * 2.0f;
		visible_n++;
	}
...
void ImGui::EndTable()
{
	...
	if (table->Flags & ImGuiTableFlags_ScrollX)
	{
		...
		if (table->RightMostEnabledColumn != -1)
			max_pos_x =
			    ImMax(max_pos_x, table->Columns[table->RightMostEnabledColumn].WorkMaxX +
						 table->CellPaddingX + table->OuterPaddingX - outer_padding_for_border);
	...
	else if (temp_data->UserOuterSize.x <= 0.0f)
	{
		...
		const float inner_content_max_x =
		    table->OuterRect.Min.x +
		    table
			->ColumnsAutoFitWidth; // Slightly misleading name but used for code symmetry with inner_content_max_y
		...
		outer_window->DC.IdealMaxPos.x = ImMax(
		    outer_window->DC.IdealMaxPos.x, inner_content_max_x + decoration_size - temp_data->UserOuterSize.x);
		outer_window->DC.CursorMaxPos.x =
	...
	if (temp_data->UserOuterSize.y <= 0.0f)
	{
		...
		outer_window->DC.IdealMaxPos.y = ImMax(
		    outer_window->DC.IdealMaxPos.y, inner_content_max_y + decoration_size - temp_data->UserOuterSize.y);
...
static void TableInitColumnDefaults(ImGuiTable *table, ImGuiTableColumn *column, ImGuiTableColumnFlags init_mask)
{
	...
	if (init_mask & ImGuiTableFlags_Resizable)
	{
		...
		column->WidthRequest	   = ((flags & ImGuiTableColumnFlags_WidthFixed) && init_width_or_weight > 0.0f)
...
void ImGui::TableEndRow(ImGuiTable *table)
{
	...
	if (is_visible)
	{
		...
		if (table->RowBgColor[0] != IM_COL32_DISABLE)
			...
			bg_col0 =
			    GetColorU32((table->RowBgColorCounter & 1) ? ImGuiCol_TableRowBgAlt : ImGuiCol_TableRowBg);
		...
		for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
		{
			...
			if (is_frozen_separator)
				...
				draw_y2 = draw_y2_body;
				  ...
				  (ImGuiTableFlags_NoBordersInBodyUntilResize | ImGuiTableFlags_NoBordersInBody)) == 0)
				draw_y2 = draw_y2_body;
			if (draw_y2 > draw_y1)
		...
		for (int order_n = 0; order_n < data_count; order_n++)
		{
			...
			if (pass == 0)
			{
				...
				while (label_name < label_name_end)
				{
					...
					float clip_height = ImMin(label_size.y, column->ClipRect.Max.x -
...
void ImGui::DebugNodeTable(ImGuiTable *table)
{
	...
	for (int n = 0; n < table->ColumnsCount; n++)
	{
		...
		ImFormatString(
		    ...
		    column->IsSkipItems, column->DrawChannelFrozen, column->DrawChannelUnfrozen, column->WidthGiven,
