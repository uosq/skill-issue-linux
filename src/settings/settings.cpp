...
void Settings::Load (const std::string &fullPath)
{
	...
	{
		const char *val = ini.GetValue (
		    "Settings", std::string (setting.name).c_str ());
		if (val)
			setting.load (val);
	}
}
...
void Settings::Save (const std::string &fullPath)
{
	...
	for (const auto &setting : m_entries)
		ini.SetValue ("Settings", std::string (setting.name).c_str (),
			      setting.get ().c_str ());
...
void Settings::InitBinds ()
{
	...
	AntiAim.warp_recharge_key
	    = gBinds.RegisterHotkey ("warp recharge key");
