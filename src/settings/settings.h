...
enum class MeleeMode { NONE = 0, LEGIT, RAGE };

enum class AimbotMode { INVALID = -1, PLAIN, SMOOTH, ASSISTANCE, SILENT, MAX };

enum class GenericMode { NONE = 0, LEGIT, RAGE };

enum class TeamMode { INVALID = -1, ONLYENEMY, ONLYTEAMMATE, BOTH, MAX };

enum class BacktrackMode { INVALID = -1, NONE, LAST_ONLY, ALL_RECORDS, MAX };

enum class HealthMode { INVALID = -1, NONE, TEXT, BAR, BOTH, MAX };
...
#define CONFIG_ENUM(name, var, EnumType)                                      \
	{ name,                                                               \
	  ...
	  [] (const std::string &v) {                                         \
		  var = static_cast<EnumType> (std::stoi (v));                \
	  },                                                                  \
...
#define CONFIG_FLOAT3(name, var)                                              \
	CONFIG_FLOAT (name " x", var[0]), CONFIG_FLOAT (name " y", var[1]),   \
	    CONFIG_FLOAT (name " z", var[2])

#define CONFIG_FLOAT4(name, var)                                              \
	CONFIG_FLOAT3 (name, var), CONFIG_FLOAT (name, var[3])
...
namespace Settings
{
	...
	inline SettingEntry m_entries[]{
		...
		CONFIG_BOOL ("aimbot draw fov indicator",
			     Aimbot.draw_fov_indicator),
		...
		CONFIG_BOOL ("aimbot hold minigun spin",
			     Aimbot.hold_minigun_spin),
		...
		CONFIG_FLOAT3 ("misc thirdperson offset",
			       Misc.thirdperson_offset),
